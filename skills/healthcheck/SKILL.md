---
name: healthcheck
description: Host security hardening and risk-tolerance configuration for text2llm deployments. Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure review, text2llm cron scheduling for periodic checks, or version status checks on a machine running text2llm (laptop, workstation, Pi, VPS).
---

# text2llm Host Hardening

## Overview

Assess and harden the host running text2llm, then align it to a user-defined risk tolerance without breaking access. Use text2llm security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim text2llm changes the host firewall, SSH, or OS updates; it does not.
...
5. text2llm gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if text2llm can access sensitive data.
...
### 2) Run text2llm security audits (read-only)

As part of the default read-only checks, run `text2llm security audit --deep`. Only offer alternatives if the user requests them:

1. `text2llm security audit` (faster, non-probing)
2. `text2llm security audit --json` (structured output)

Offer to apply text2llm safe defaults (numbered):

1. `text2llm security audit --fix`

Be explicit that `--fix` only tightens text2llm defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check text2llm version/update status (read-only)

As part of the default read-only checks, run `text2llm update status`.
...
- Credential hygiene notes (location of text2llm creds, prefer disk encryption)
...
- text2llm security audit (re-run)
...
After text2llm install or first hardening pass, run at least one baseline audit and version check:

- `text2llm security audit`
- `text2llm security audit --deep`
- `text2llm update status`

Ongoing monitoring is recommended. Use the text2llm cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `text2llm cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `text2llm update status`
...
Before creating, `text2llm cron list` and match on exact `name`. If found, `text2llm cron edit <id> ...`.
If not found, `text2llm cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. `text2llm update status` (preferred for source checkouts and channels)
2. `npm view text2llm version` (published npm version)

## text2llm command accuracy

Use only supported commands and flags:

- `text2llm security audit [--deep] [--fix] [--json]`
- `text2llm status` / `text2llm status --deep`
- `text2llm health --json`
- `text2llm update status`
- `text2llm cron add|list|runs|run`

Do not invent CLI flags or imply text2llm enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by text2llm compaction:
