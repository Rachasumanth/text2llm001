...
import type { Text2llmPluginApi, Text2llmPluginToolContext } from "../../../src/plugins/types.js";
import { createLobsterTool } from "./lobster-tool.js";

async function writeFakeLobsterScript(scriptBody: string, prefix = "text2llm-lobster-plugin-") {
...
function fakeApi(overrides: Partial<Text2llmPluginApi> = {}): Text2llmPluginApi {
  return {
    id: "lobster",
    name: "lobster",
    source: "test",
...
function fakeCtx(overrides: Partial<Text2llmPluginToolContext> = {}): Text2llmPluginToolContext {
  return {
    config: {},
    workspaceDir: "/tmp",
    agentDir: "/tmp",
...
describe("lobster plugin tool", () => {
  ...
  it("can be gated off in sandboxed contexts", async () => {
    const api = fakeApi();
    const factoryTool = (ctx: Text2llmPluginToolContext) => {
      if (ctx.sandboxed) {
        return null;
      }
