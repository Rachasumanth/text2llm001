import type { Text2llmPluginApi, Text2llmPluginService } from "text2llm/plugin-sdk";
...
function normalizeDenyList(cfg: Text2llmPluginApi["config"]): string[] {
  return uniqSorted([...(cfg.gateway?.nodes?.denyCommands ?? [])]);
}

function normalizeAllowList(cfg: Text2llmPluginApi["config"]): string[] {
...
export default function register(api: Text2llmPluginApi) {
  ...
  const timerService: Text2llmPluginService = {
