/**
 * Twitch channel plugin for text2llm.
 *
...
import { buildChannelConfigSchema } from "text2llm/plugin-sdk";
import type {
 ...
 * for text2llm. Supports message sending, receiving, access control, and
 * status monitoring.
 */
export const twitchPlugin: ChannelPlugin<TwitchAccountConfig> = {
  ...
  config: {
    ...
    listAccountIds: (cfg: Text2llmConfig): string[] => listAccountIds(cfg),

    /** Resolve an account config by ID */
    resolveAccount: (cfg: Text2llmConfig, accountId?: string | null): TwitchAccountConfig => {
    ...
    isConfigured: (_account: unknown, cfg: Text2llmConfig): boolean => {
