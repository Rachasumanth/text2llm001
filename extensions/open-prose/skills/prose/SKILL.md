...
metadata: { "text2llm": { "emoji": "🪶", "homepage": "https://www.prose.md" } }
---
...
- **Task tool** in the upstream spec == text2llm `sessions_spawn`
- **File I/O** == text2llm `read`/`write`
- **Remote fetch** == text2llm `web_fetch` (or `exec` with curl when POST is required)
