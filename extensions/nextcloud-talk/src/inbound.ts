...
export async function handleNextcloudTalkInbound(params: {
  ...
  const hasControlCommand = core.channel.text.hasControlCommand(rawBody, config as Text2llmConfig);
  ...
  const mentionRegexes = core.channel.mentions.buildMentionRegexes(config as Text2llmConfig);
  ...
  const envelopeOptions = core.channel.reply.resolveEnvelopeFormatOptions(config as Text2llmConfig);
