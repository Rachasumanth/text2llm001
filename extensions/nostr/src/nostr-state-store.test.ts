import type { PluginRuntime } from "text2llm/plugin-sdk";
...
async function withTempStateDir<T>(fn: (dir: string) => Promise<T>) {
  const previous = process.env.TEXT2LLM_STATE_DIR;
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "text2llm-nostr-"));
  process.env.TEXT2LLM_STATE_DIR = dir;
  setNostrRuntime({
    state: {
      resolveStateDir: (env, homedir) => {
        const override = env.TEXT2LLM_STATE_DIR?.trim() || env.TEXT2LLM_STATE_DIR?.trim();
        if (override) {
          return override;
        }
        return path.join(homedir(), ".text2llm");
      },
    },
  } as PluginRuntime);
  try {
    ...
    if (previous === undefined) {
      ...
      process.env.TEXT2LLM_STATE_DIR = previous;
    }
