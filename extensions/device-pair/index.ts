...
import { approveDevicePairing, listDevicePairing } from "text2llm/plugin-sdk";
...
function resolveGatewayPort(cfg: Text2llmPluginApi["config"]): number {
  const envRaw =
    process.env.TEXT2LLM_GATEWAY_PORT?.trim() || process.env.CLAWDBOT_GATEWAY_PORT?.trim();
  if (envRaw) {
...
async function resolveTailnetHost(api: Text2llmPluginApi): Promise<string | null> {
...
function resolveAuth(cfg: Text2llmPluginApi["config"]): ResolveAuthResult {
...
async function resolveGatewayUrl(api: Text2llmPluginApi): Promise<ResolveUrlResult> {
