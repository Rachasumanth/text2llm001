...
// NOTE: This extension is intended to be bundled with text2llm.
// When running from source (tests/dev), text2llm internals live under src/.
// When running from a built install, internals live under dist/ (no src/ tree).
// So we resolve internal imports dynamically with src-first, dist-fallback.
import type { Text2llmPluginApi } from "../../../src/plugins/types.js";
...
export function createLlmTaskTool(api: Text2llmPluginApi) {
  return {
    name: "llm-task",
    label: "LLM Task",
    description:
      "Run a generic JSON-only LLM task and return schema-validated JSON. Designed for orchestration from Lobster workflows via text2llm.invoke.",
    ...
    async execute(_id: string, params: Record<string, unknown>) {
      ...
      try {
        tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "text2llm-llm-task-"));
