...
`text2llm.invoke --each`) without adding new text2llm code per workflow.
...
This extension depends on text2llm internal modules (the embedded agent runner).
It is intended to ship as a **bundled** text2llm extension (like `lobster`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.text2llm/extensions` as a standalone plugin directory.
