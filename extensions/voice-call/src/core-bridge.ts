...
function resolveText2llmRoot(): string {
  if (coreRootCache) {
    return coreRootCache;
  }
  const override = process.env.TEXT2LLM_ROOT?.trim();
  if (override) {
  ...
  throw new Error("Unable to resolve core root. Set TEXT2LLM_ROOT to the package root.");
}
...
async function importCoreExtensionAPI(): Promise<{
  ...
  const distPath = path.join(resolveText2llmRoot(), "dist", "extensionAPI.js");
