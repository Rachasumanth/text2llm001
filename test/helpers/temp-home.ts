...
function restoreEnv(snapshot: EnvSnapshot) {
  ...
  restoreKey("TEXT2LLM_STATE_DIR", snapshot.stateDir);
}
...
function setTempHome(base: string) {
  ...
  process.env.TEXT2LLM_STATE_DIR = path.join(base, ".text2llm");
...
export async function withTempHome<T>(
  ...
  const base = await fs.mkdtemp(path.join(os.tmpdir(), opts.prefix ?? "text2llm-test-home-"));
  ...
  setTempHome(base);
  await fs.mkdir(path.join(base, ".text2llm", "agents", "main", "sessions"), { recursive: true });
  if (opts.env) {
