...
const makeTempMedia = async (ext: string) => {
  const dir = await makeTempDir("text2llm-media-e2e-");
...
describe("media understanding auto-detect (e2e)", () => {
  ...
  it("uses sherpa-onnx-offline when available", async () => {
    ...
    try {
      const binDir = await makeTempDir("text2llm-bin-sherpa-");
      const modelDir = await makeTempDir("text2llm-sherpa-model-");
      ...
      const cfg: TEXT2LLMConfig = { tools: { media: { audio: {} } } };
  ...
  it("uses whisper-cli when sherpa is missing", async () => {
    const snapshot = envSnapshot();
    try {
      const binDir = await makeTempDir("text2llm-bin-whispercpp-");
      const modelDir = await makeTempDir("text2llm-whispercpp-model-");
      tempPaths.push(binDir, modelDir);
  ...
  it("uses gemini CLI for images when available", async () => {
    const snapshot = envSnapshot();
    try {
      const binDir = await makeTempDir("text2llm-bin-gemini-");
      ...
      const cfg: TEXT2LLMConfig = { tools: { media: { image: {} } } };
