...
const spawnGatewayInstance = async (name: string): Promise<GatewayInstance> => {
  ...
  const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `text2llm-e2e-${name}-`));
  const configDir = path.join(homeDir, ".text2llm");
    ...
    child = spawn(
      ...
      ],
      {
        ...
        env: {
          ...
          TEXT2LLM_SKIP_BROWSER_CONTROL_SERVER: "1",
...
const waitForNodeStatus = async (inst: GatewayInstance, nodeId: string, timeoutMs = 10_000) => {
  ...
  while (Date.now() < deadline) {
    const list = (await runCliJson(
      ...
      {
        TEXT2LLM_GATEWAY_TOKEN: inst.gatewayToken,
        TEXT2LLM_GATEWAY_PASSWORD: "",
      },
    )) as NodeListPayload;
    ...
    async () => {
      ...
      const [healthA, healthB] = (await Promise.all([
        runCliJson(["health", "--json", "--timeout", "10000"], {
          TEXT2LLM_GATEWAY_PORT: String(gwA.port),
          TEXT2LLM_GATEWAY_TOKEN: gwA.gatewayToken,
          TEXT2LLM_GATEWAY_PASSWORD: "",
        }),
        runCliJson(["health", "--json", "--timeout", "10000"], {
          TEXT2LLM_GATEWAY_PORT: String(gwB.port),
          TEXT2LLM_GATEWAY_TOKEN: gwB.gatewayToken,
          TEXT2LLM_GATEWAY_PASSWORD: "",
        }),
