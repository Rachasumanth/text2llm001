...
struct IOSGatewayChatTransport: Text2llmChatTransport, Sendable {
    ...
    func listSessions(limit: Int?) async throws -> Text2llmChatSessionsListResponse {
        ...
        return try JSONDecoder().decode(Text2llmChatSessionsListResponse.self, from: res)
    }
    ...
    func requestHistory(sessionKey: String) async throws -> Text2llmChatHistoryPayload {
        ...
        return try JSONDecoder().decode(Text2llmChatHistoryPayload.self, from: res)
    }
        ...
        attachments: [Text2llmChatAttachmentPayload]) async throws -> Text2llmChatSendResponse
    {
        struct Params: Codable {
            ...
            var message: String
            var thinking: String
            var attachments: [Text2llmChatAttachmentPayload]?
            var timeoutMs: Int
        ...
        return try JSONDecoder().decode(Text2llmChatSendResponse.self, from: res)
    }

    func requestHealth(timeoutMs: Int) async throws -> Bool {
        let seconds = max(1, Int(ceil(Double(timeoutMs) / 1000.0)))
        let res = try await self.gateway.request(method: "health", paramsJSON: nil, timeoutSeconds: seconds)
        return (try? JSONDecoder().decode(Text2llmGatewayHealthOK.self, from: res))?.ok ?? true
    }
                ...
                for await evt in stream {
                    ...
                    case "health":
                        ...
                        let ok = (try? GatewayPayloadDecoding.decode(
                            payload,
                            as: Text2llmGatewayHealthOK.self))?.ok ?? true
                        continuation.yield(.health(ok: ok))
                    case "chat":
                        guard let payload = evt.payload else { break }
                        if let chatPayload = try? GatewayPayloadDecoding.decode(
                            payload,
                            as: Text2llmChatEventPayload.self)
                        {
                    ...
                    case "agent":
                        ...
                        if let agentPayload = try? GatewayPayloadDecoding.decode(
                            payload,
                            as: Text2llmAgentEventPayload.self)
                        {
