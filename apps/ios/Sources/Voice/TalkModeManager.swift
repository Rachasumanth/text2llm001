...
final class TalkModeManager: NSObject {
    ...
    private var pttCompletion: CheckedContinuation<Text2llmTalkPTTStopPayload, Never>?
    ...
    func stop() {
        ...
        if pendingPTT {
            let payload = Text2llmTalkPTTStopPayload(
    ...
    func beginPushToTalk() async throws -> Text2llmTalkPTTStartPayload {
        guard self.gatewayConnected else {
            self.statusText = "Offline"
        ...
        if self.isPushToTalkActive, let captureId = self.activePTTCaptureId {
            return Text2llmTalkPTTStartPayload(captureId: captureId)
        }
    ...
    func endPushToTalk() async -> Text2llmTalkPTTStopPayload {
    ...
    func runPushToTalkOnce(maxDurationSeconds: TimeInterval = 12) async throws -> Text2llmTalkPTTStopPayload {
        ...
        if self.isPushToTalkActive {
            let captureId = self.activePTTCaptureId ?? UUID().uuidString
            return Text2llmTalkPTTStopPayload(
                captureId: captureId,
                transcript: nil,
                status: "busy")
        }
    ...
    func cancelPushToTalk() async -> Text2llmTalkPTTStopPayload {
        let captureId = self.activePTTCaptureId ?? UUID().uuidString
        guard self.isPushToTalkActive else {
            let payload = Text2llmTalkPTTStopPayload(
                captureId: captureId,
    ...
    private func finishPTTOnce(_ payload: Text2llmTalkPTTStopPayload) {
    ...
    private func streamAssistant(runId: String, gateway: GatewayNodeSession) async {
        ...
        for await evt in stream {
            ...
            guard let agentEvent = try? GatewayPayloadDecoding.decode(payload, as: Text2llmAgentEventPayload.self) else {
                continue
            }
