...
final class ContactsService: ContactsServicing {
    ...
    func search(params: Text2llmContactsSearchParams) async throws -> Text2llmContactsSearchPayload {
        ...
        return Text2llmContactsSearchPayload(contacts: payload)
    }
    ...
    func add(params: Text2llmContactsAddParams) async throws -> Text2llmContactsAddPayload {
        ...
        if !phoneNumbers.isEmpty || !emails.isEmpty {
            if let existing = try Self.findExistingContact(
                ...
                emails: emails)
            {
                return Text2llmContactsAddPayload(contact: Self.payload(from: existing))
            }
        }
        ...
        return Text2llmContactsAddPayload(contact: Self.payload(from: persisted))
    }
    ...
    private static func payload(from contact: CNContact) -> Text2llmContactPayload {
        Text2llmContactPayload(
