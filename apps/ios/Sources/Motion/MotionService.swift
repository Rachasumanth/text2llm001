...
final class MotionService: MotionServicing {
    func activities(params: Text2llmMotionActivityParams) async throws -> Text2llmMotionActivityPayload {
        ...
        let mapped = try await withCheckedThrowingContinuation { (cont: CheckedContinuation<[Text2llmMotionActivityEntry], Error>) in
        ...
        return Text2llmMotionActivityPayload(activities: mapped)
    }
    ...
    func pedometer(params: Text2llmPedometerParams) async throws -> Text2llmPedometerPayload {
        ...
        let payload = try await withCheckedThrowingContinuation { (cont: CheckedContinuation<Text2llmPedometerPayload, Error>) in
            pedometer.queryPedometerData(from: start, to: end) { data, error in
                if let error {
                    cont.resume(throwing: error)
                } else {
                    let formatter = ISO8601DateFormatter()
                    let payload = Text2llmPedometerPayload(
