...
final class RemindersService: RemindersServicing {
    func list(params: Text2llmRemindersListParams) async throws -> Text2llmRemindersListPayload {
        ...
        let payload = try await withCheckedThrowingContinuation { (cont: CheckedContinuation<[Text2llmReminderPayload], Error>) in
            store.fetchReminders(matching: predicate) { items in
                ...
                let payload = selected.map { reminder in
                    ...
                    return Text2llmReminderPayload(
        ...
        return Text2llmRemindersListPayload(reminders: payload)
    }
    ...
    func add(params: Text2llmRemindersAddParams) async throws -> Text2llmRemindersAddPayload {
        ...
        let formatter = ISO8601DateFormatter()
        let due = reminder.dueDateComponents.flatMap { Calendar.current.date(from: $0) }
        let payload = Text2llmReminderPayload(
            identifier: reminder.calendarItemIdentifier,
            title: reminder.title,
            dueISO: due.map { formatter.string(from: $0) },
            completed: reminder.isCompleted,
        ...
        return Text2llmRemindersAddPayload(reminder: payload)
    }
