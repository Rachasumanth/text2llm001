...
enum GatewaySettingsStore {
    private static let gatewayService = "ai.text2llm.gateway"
    private static let nodeService = "ai.text2llm.node"
...
enum GatewayDiagnostics {
    private static let logger = Logger(subsystem: "ai.text2llm.ios", category: "GatewayDiag")
    private static let queue = DispatchQueue(label: "ai.text2llm.gateway.diagnostics")
    ...
    private static var fileURL: URL? {
        FileManager.default.urls(for: .documentDirectory, in: .userDomainMask).first?
            .appendingPathComponent("text2llm-gateway.log")
    }
