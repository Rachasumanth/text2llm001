...
This is an **alpha** iOS app that connects to an text2llm Gateway as a `role: node`.
...
3. On iOS: text2llm → Settings → Gateway → paste setup code → Connect
...
cd apps/ios
xcodegen generate
xcodebuild test -project text2llm.xcodeproj -scheme text2llm -destination "platform=iOS Simulator,name=iPhone 17"
```

## Shared Code

- `apps/shared/Text2llmKit` contains the shared transport/types used by the iOS app.
