...
import Foundation
import Testing
@testable import Text2llmChatUI
...
private final class TestChatTransport: @unchecked Sendable, Text2llmChatTransport {
    private let state = TestChatTransportState()
    private let historyResponses: [Text2llmChatHistoryPayload]
    private let sessionsResponses: [Text2llmChatSessionsListResponse]

    private let stream: AsyncStream<Text2llmChatTransportEvent>
    private let continuation: AsyncStream<Text2llmChatTransportEvent>.Continuation

    init(
        historyResponses: [Text2llmChatHistoryPayload],
        sessionsResponses: [Text2llmChatSessionsListResponse] = [])
    {
        self.historyResponses = historyResponses
        self.sessionsResponses = sessionsResponses
        var cont: AsyncStream<Text2llmChatTransportEvent>.Continuation!
    ...
    func requestHistory(sessionKey: String) async throws -> Text2llmChatHistoryPayload {
        ...
        return self.historyResponses.last ?? Text2llmChatHistoryPayload(
        ...
        attachments _: [Text2llmChatAttachmentPayload]) async throws -> Text2llmChatSendResponse
    {
        ...
        return Text2llmChatSendResponse(runId: idempotencyKey, status: "ok")
    }
    ...
    func listSessions(limit _: Int?) async throws -> Text2llmChatSessionsListResponse {
        ...
        return self.sessionsResponses.last ?? Text2llmChatSessionsListResponse(
            ts: nil,
            path: nil,
            count: 0,
            defaults: nil,
    ...
    func emit(_ evt: Text2llmChatTransportEvent) {
...
@Suite struct ChatViewModelTests {
    @Test func streamsAssistantAndClearsOnFinal() async throws {
        ...
        let history1 = Text2llmChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
        let history2 = Text2llmChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [
                AnyCodable([
        ...
        let vm = await MainActor.run { Text2llmChatViewModel(sessionKey: "main", transport: transport) }
    ...
    @Test func clearsStreamingOnExternalFinalEvent() async throws {
        let sessionId = "sess-main"
        let history = Text2llmChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
    ...
    @Test func sessionChoicesPreferMainAndRecent() async throws {
        ...
        let history = Text2llmChatHistoryPayload(
            ...
            messages: [],
            thinkingLevel: "off")
        let sessions = Text2llmChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 4,
            defaults: nil,
    ...
    @Test func sessionChoicesIncludeCurrentWhenMissing() async throws {
        ...
        let transport = TestChatTransport(
            historyResponses: [history],
            sessionsResponses: [sessions])
        let vm = await MainActor.run { Text2llmChatViewModel(sessionKey: "custom", transport: transport) }
        await MainActor.run { vm.load() }
        try await waitUntil("sessions loaded") { await MainActor.run { !vm.sessions.isEmpty } }
