...
struct ChatMessageBubble: View {
    let message: Text2llmChatMessage
    let style: Text2llmChatView.Style
...
private struct ChatMessageBody: View {
    ...
    var body: some View {
        ...
        let textColor = self.isUser ? Text2llmChatTheme.userText : Text2llmChatTheme.assistantText
    ...
    private var inlineAttachments: [Text2llmChatMessageContent] {
    ...
    private var toolCalls: [Text2llmChatMessageContent] {
        self.message.content.filter { content in
    ...
    private var inlineToolResults: [Text2llmChatMessageContent] {
        self.message.content.filter { content in
            let kind = (content.type ?? "").lowercased()
    ...
    private var bubbleFillColor: Color {
        if self.isUser {
            return self.userAccent ?? Text2llmChatTheme.userBubble
        }
        if self.style == .onboarding {
            return Text2llmChatTheme.onboardingAssistantBubble
        }
        return Text2llmChatTheme.assistantBubble
    }
    ...
    private var bubbleBorderColor: Color {
        ...
        if self.style == .onboarding {
            return Text2llmChatTheme.onboardingAssistantBorder
        }
...
private struct AttachmentRow: View {
    let att: Text2llmChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        HStack(spacing: 8) {
            ...
            Text(self.att.fileName ?? "Attachment")
                .font(.footnote)
                .lineLimit(1)
                .foregroundStyle(self.isUser ? Text2llmChatTheme.userText : Text2llmChatTheme.assistantText)
            Spacer()
        }
        .padding(10)
...
private struct ToolCallCard: View {
    let content: Text2llmChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 12, style: .continuous)
                .fill(Text2llmChatTheme.subtleCard)
                .overlay(
                    RoundedRectangle(cornerRadius: 12, style: .continuous)
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .fill(Text2llmChatTheme.assistantBubble))
        .overlay(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .strokeBorder(Color.white.opacity(0.08), lineWidth: 1))
...
struct ChatPendingToolsBubble: View {
    let toolCalls: [Text2llmChatPendingToolCall]
        ...
        VStack(alignment: .leading, spacing: 10) {
            ForEach(segments) { segment in
                ...
                ChatMarkdownRenderer(
                    ...
                    variant: self.markdownVariant,
                    font: font,
                    textColor: Text2llmChatTheme.assistantText)
            }
        }
    }
}
