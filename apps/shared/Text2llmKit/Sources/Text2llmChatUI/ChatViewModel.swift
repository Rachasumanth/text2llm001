...
private let chatUILogger = Logger(subsystem: "ai.text2llm", category: "Text2llmChatUI")

@MainActor
@Observable
public final class Text2llmChatViewModel {
    public private(set) var messages: [Text2llmChatMessage] = []
    ...
    public var attachments: [Text2llmPendingAttachment] = []
    ...
    public private(set) var pendingToolCalls: [Text2llmChatPendingToolCall] = []
    public private(set) var sessions: [Text2llmChatSessionEntry] = []
    private let transport: any Text2llmChatTransport
    ...
    private var pendingToolCallsById: [String: Text2llmChatPendingToolCall] = [:] {
        didSet {
    ...
    public init(sessionKey: String, transport: any Text2llmChatTransport) {
    ...
    public var sessionChoices: [Text2llmChatSessionEntry] {
        let now = Date().timeIntervalSince1970 * 1000
        let cutoff = now - (24 * 60 * 60 * 1000)
        let sorted = self.sessions.sorted { ($0.updatedAt ?? 0) > ($1.updatedAt ?? 0) }
        var seen = Set<String>()
        var recent: [Text2llmChatSessionEntry] = []
        for entry in sorted {
        ...
        var result: [Text2llmChatSessionEntry] = []
    ...
    public func removeAttachment(_ id: Text2llmPendingAttachment.ID) {
    ...
    private static func decodeMessages(_ raw: [AnyCodable]) -> [Text2llmChatMessage] {
        let decoded = raw.compactMap { item in
            (try? ChatPayloadDecoding.decode(item, as: Text2llmChatMessage.self))
        }
        return Self.dedupeMessages(decoded)
    }

    private static func dedupeMessages(_ messages: [Text2llmChatMessage]) -> [Text2llmChatMessage] {
        var result: [Text2llmChatMessage] = []
    ...
    private static func dedupeKey(for message: Text2llmChatMessage) -> String? {
    ...
    private func performSend() async {
        ...
        var userContent: [Text2llmChatMessageContent] = [
        ...
        let encodedAttachments = self.attachments.map { att -> Text2llmChatAttachmentPayload in
            Text2llmChatAttachmentPayload(
                type: att.type,
    ...
    private func placeholderSession(key: String) -> Text2llmChatSessionEntry {
        Text2llmChatSessionEntry(
            key: key,
            kind: nil,
            displayName: nil,
            surface: nil,
    ...
    private func handleTransportEvent(_ evt: Text2llmChatTransportEvent) {
        switch evt {
        case let .health(ok):
            self.healthOK = ok
        case .tick:
    ...
    private func handleChatEvent(_ chat: Text2llmChatEventPayload) {
    ...
    private func handleAgentEvent(_ evt: Text2llmAgentEventPayload) {
        ...
        case "tool":
            ...
            if phase == "start" {
                ...
                self.pendingToolCallsById[toolCallId] = Text2llmChatPendingToolCall(
    ...
    private static func previewImage(data: Data) -> Text2llmPlatformImage? {
        #if canImport(AppKit)
        NSImage(data: data)
        #elseif canImport(UIKit)
        UIImage(data: data)
