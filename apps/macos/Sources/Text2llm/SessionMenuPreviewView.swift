...
enum SessionMenuPreviewLoader {
    private static let logger = Logger(subsystem: "ai.text2llm", category: "SessionPreview")
    ...
    private static func requestPreview(
        keys: [String],
        maxItems: Int) async throws -> Text2llmSessionsPreviewPayload
    {
    ...
    private static func snapshot(
        from entry: Text2llmSessionPreviewEntry,
    ...
    private static func cache(payload: Text2llmSessionsPreviewPayload, maxItems: Int) async {
    ...
    private static func previewItems(
        from payload: Text2llmChatHistoryPayload,
        maxItems: Int) -> [SessionPreviewItem]
    {
        let boundedItems = self.normalizeMaxItems(maxItems)
        let raw: [Text2llmKit.AnyCodable] = payload.messages ?? []
    ...
    private static func decodeMessages(_ raw: [Text2llmKit.AnyCodable]) -> [Text2llmChatMessage] {
        raw.compactMap { item in
            guard let data = try? JSONEncoder().encode(item) else { return nil }
            return try? JSONDecoder().decode(Text2llmChatMessage.self, from: data)
        }
    }
    ...
    private static func previewText(for message: Text2llmChatMessage) -> String? {
    ...
    private static func isToolCall(_ message: Text2llmChatMessage) -> Bool {
    ...
    private static func toolNames(for message: Text2llmChatMessage) -> [String] {
        var names: [String] = []
    ...
    private static func mediaSummary(for message: Text2llmChatMessage) -> String? {
