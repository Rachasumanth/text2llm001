...
struct TailscaleIntegrationSection: View {
    ...
    private var authFields: some View {
        ...
        Text("Stored in ~/.text2llm/text2llm.json. Prefer TEXT2LLM_GATEWAY_PASSWORD for production.")
            .font(.caption)
            .foregroundStyle(.secondary)
    ...
    private func applySettings() async {
        ...
        if self.connectionMode == .local, !self.isPaused {
            self.statusMessage = "Saved to ~/.text2llm/text2llm.json. Restarting gateway…"
        } else {
            self.statusMessage = "Saved to ~/.text2llm/text2llm.json. Restart the gateway to apply."
        }
