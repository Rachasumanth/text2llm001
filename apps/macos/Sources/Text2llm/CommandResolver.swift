...
enum CommandResolver {
    private static let projectRootDefaultsKey = "text2llm.gatewayProjectRootPath"
    private static let helperName = "text2llm"

    static func gatewayEntrypoint(in root: URL) -> String? {
        let distEntry = root.appendingPathComponent("dist/index.js").path
        if FileManager().isReadableFile(atPath: distEntry) { return distEntry }
        let text2llmEntry = root.appendingPathComponent("text2llm.mjs").path
        if FileManager().isReadableFile(atPath: text2llmEntry) { return text2llmEntry }
        let binEntry = root.appendingPathComponent("bin/text2llm.js").path
    ...
    static func projectRoot() -> URL {
        ...
        let fallback = FileManager().homeDirectoryForCurrentUser
            .appendingPathComponent("Projects/text2llm")
    ...
    static func preferredPaths(home: URL, current: [String], projectRoot: URL) -> [String] {
        ...
        #endif
        let text2llmPaths = self.text2llmManagedPaths(home: home)
        if !text2llmPaths.isEmpty {
            extras.insert(contentsOf: text2llmPaths, at: 1)
        }
        extras.insert(contentsOf: self.nodeManagerBinPaths(home: home), at: 1 + text2llmPaths.count)
        var seen = Set<String>()
        // Preserve order while stripping duplicates so PATH lookups remain deterministic.
        return (extras + current).filter { seen.insert($0).inserted }
    }

    private static func text2llmManagedPaths(home: URL) -> [String] {
        let bases = [
            home.appendingPathComponent(".text2llm"),
        ]
        var paths: [String] = []
        for base in bases {
    ...
    static func text2llmExecutable(searchPaths: [String]? = nil) -> String? {
        self.findExecutable(named: self.helperName, searchPaths: searchPaths)
    }

    static func projectText2llmExecutable(projectRoot: URL? = nil) -> String? {
        #if DEBUG
    ...
    static func nodeCliPath() -> String? {
        ...
        let candidates = [
            root.appendingPathComponent("text2llm.mjs").path,
            root.appendingPathComponent("bin/text2llm.js").path,
        ]
    ...
    static func hasAnyText2llmInvoker(searchPaths: [String]? = nil) -> Bool {
        if self.text2llmExecutable(searchPaths: searchPaths) != nil { return true }
    ...
    static func text2llmNodeCommand(
        subcommand: String,
        extraArgs: [String] = [],
        ...
        searchPaths: [String]? = nil) -> [String]
    {
        ...
        case let .success(runtime):
            let root = self.projectRoot()
            if let text2llmPath = self.projectText2llmExecutable(projectRoot: root) {
                return [text2llmPath, subcommand] + extraArgs
            }
            ...
            if let pnpm = self.findExecutable(named: "pnpm", searchPaths: searchPaths) {
                ...
                return [pnpm, "--silent", "text2llm", subcommand] + extraArgs
            }
            if let text2llmPath = self.text2llmExecutable(searchPaths: searchPaths) {
                return [text2llmPath, subcommand] + extraArgs
            }

            let missingEntry = """
            text2llm entrypoint missing (looked for dist/index.js or text2llm.mjs); run pnpm build.
            """
    ...
    private static func sshNodeCommand(subcommand: String, extraArgs: [String], settings: RemoteSettings) -> [String]? {
        ...
        // Run the real text2llm CLI on the remote host.
        let exportedPath = [
            "/opt/homebrew/bin",
            "/usr/local/bin",
            "/usr/bin",
        ...
        let projectSection = if userPRJ.isEmpty {
            """
            DEFAULT_PRJ="$HOME/Projects/text2llm"
        ...
        \(cliSection)
        \(projectSection)
        if command -v text2llm >/dev/null 2>&1; then
          CLI="$(command -v text2llm)"
          text2llm \(quotedArgs);
        ...
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/text2llm.mjs" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/text2llm.mjs"
            node "$PRJ/text2llm.mjs" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/bin/text2llm.js" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/bin/text2llm.js"
            node "$PRJ/bin/text2llm.js" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif command -v pnpm >/dev/null 2>&1; then
          CLI="pnpm --silent text2llm"
          pnpm --silent text2llm \(quotedArgs);
        else
          echo "text2llm CLI missing on remote host"; exit 127;
        fi
        """
        let options: [String] = [
            "-o", "BatchMode=yes",
    ...
    {
        let root = configRoot ?? Text2llmConfigFile.loadDict()
