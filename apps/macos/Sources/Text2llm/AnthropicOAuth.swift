...
enum AnthropicAuthMode: Equatable {
    ...
    var shortLabel: String {
        switch self {
        case .oauthFile: "OAuth (text2llm token file)"
...
enum AnthropicAuthResolver {
    static func resolve(
        ...
        oauthStatus: Text2llmOAuthStore.AnthropicOAuthStatus = Text2llmOAuthStore
...
enum AnthropicOAuth {
    private static let logger = Logger(subsystem: "ai.text2llm", category: "anthropic-oauth")
...
enum Text2llmOAuthStore {
    static let oauthFilename = "oauth.json"
    private static let providerKey = "anthropic"
    private static let text2llmOAuthDirEnv = "TEXT2LLM_OAUTH_DIR"
    ...
    enum AnthropicOAuthStatus: Equatable {
        ...
        var shortDescription: String {
            switch self {
            case .missingFile: "text2llm OAuth token file not found"
            case .unreadableFile: "text2llm OAuth token file not readable"
            case .invalidJSON: "text2llm OAuth token file invalid"
            case .missingProviderEntry: "No Anthropic entry in text2llm OAuth token file"
            case .missingTokens: "Anthropic entry missing tokens"
            case .connected: "text2llm OAuth credentials found"
            }
        }
    }

    static func oauthDir() -> URL {
        if let override = ProcessInfo.processInfo.environment[self.text2llmOAuthDirEnv]?
        ...
        let preferred = home.appendingPathComponent(".text2llm", isDirectory: true)
