...
actor RemoteTunnelManager {
    ...
    private let logger = Logger(subsystem: "ai.text2llm", category: "remote-tunnel")
    ...
    func controlTunnelPortIfRunning() async -> UInt16? {
        ...
        // If a previous text2llm run already has an SSH listener on the expected port (common after restarts),
