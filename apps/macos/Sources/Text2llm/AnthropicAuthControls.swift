...
struct AnthropicAuthControls: View {
    ...
    @State private var oauthStatus: Text2llmOAuthStore.AnthropicOAuthStatus = Text2llmOAuthStore.anthropicOAuthStatus()
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            HStack(spacing: 10) {
                ...
                .disabled(!FileManager().fileExists(atPath: Text2llmOAuthStore.oauthURL().path))
    ...
    private func refresh() {
        let imported = Text2llmOAuthStore.importLegacyAnthropicOAuthIfNeeded()
        self.oauthStatus = Text2llmOAuthStore.anthropicOAuthStatus()
        if imported != nil {
    ...
    private func finishOAuth() async {
        ...
        do {
            ...
            try Text2llmOAuthStore.saveAnthropicOAuth(creds)
            self.refresh()
            self.pkce = nil
            self.code = ""
            self.statusText = "Connected. text2llm can now use Claude via OAuth."
        } catch {
...
extension AnthropicAuthControls {
    init(
        ...
        oauthStatus: Text2llmOAuthStore.AnthropicOAuthStatus,
