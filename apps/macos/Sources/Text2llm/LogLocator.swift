...
enum LogLocator {
    private static var logDir: URL {
        if let override = ProcessInfo.processInfo.environment["TEXT2LLM_LOG_DIR"],
           !override.isEmpty
        {
            return URL(fileURLWithPath: override)
        }
        let preferred = URL(fileURLWithPath: "/tmp/text2llm")
        return preferred
    }

    private static var stdoutLog: URL {
        logDir.appendingPathComponent("text2llm-stdout.log")
    }

    private static var gatewayLog: URL {
        logDir.appendingPathComponent("text2llm-gateway.log")
    }
    ...
    /// Returns the newest log file under /tmp/text2llm/ (rolling or stdout), or nil if none exist.
