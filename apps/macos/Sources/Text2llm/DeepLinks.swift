...
private let deepLinkLogger = Logger(subsystem: "ai.text2llm", category: "DeepLink")
...
final class DeepLinkHandler {
    ...
    func handle(url: URL) async {
        ...
        guard !AppStateStore.shared.isPaused else {
            self.presentAlert(title: "text2llm is paused", message: "Unpause text2llm to run agent actions.")
            return
        }
    ...
    private func handleAgent(link: AgentDeepLink, originalURL: URL) async {
        ...
        if !allowUnattended {
            ...
            guard self.confirm(title: "Run text2llm agent?", message: body) else { return }
        }
