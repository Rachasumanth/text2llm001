...
private let webChatSwiftLogger = Logger(subsystem: "ai.text2llm", category: "WebChatSwiftUI")
...
struct MacGatewayChatTransport: Text2llmChatTransport, Sendable {
    ...
    func listSessions(limit: Int?) async throws -> Text2llmChatSessionsListResponse {
        ...
        let data = try await GatewayConnection.shared.request(
            ...
            params: params,
            timeoutMs: 15000)
        return try JSONDecoder().decode(Text2llmChatSessionsListResponse.self, from: data)
    }
    ...
    static func mapPushToTransportEvent(_ push: GatewayPush) -> Text2llmChatTransportEvent? {
        switch push {
        case let .snapshot(hello):
            let ok = (try? JSONDecoder().decode(
                Text2llmGatewayHealthOK.self,
        ...
        case let .event(evt):
            ...
            case "agent":
                ...
                guard let agent = try? JSONDecoder().decode(
                    Text2llmAgentEventPayload.self,
                    from: JSONEncoder().encode(payload))
                else {
                    return nil
                }
...
final class WebChatSwiftUIWindowController {
    ...
    private let hosting: NSHostingController<Text2llmChatView>
    ...
    init(sessionKey: String, presentation: WebChatPresentation, transport: any Text2llmChatTransport) {
        self.sessionKey = sessionKey
        self.presentation = presentation
        let vm = Text2llmChatViewModel(sessionKey: sessionKey, transport: transport)
        let accent = Self.color(fromHex: AppStateStore.shared.seamColorHex)
        self.hosting = NSHostingController(rootView: Text2llmChatView(
            viewModel: vm,
    ...
    private static func makeContentController(
        for presentation: WebChatPresentation,
        hosting: NSHostingController<Text2llmChatView>) -> NSViewController
    {
