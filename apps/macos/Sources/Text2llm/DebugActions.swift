...
enum DebugActions {
    private static let verboseDefaultsKey = "text2llm.debug.verboseMain"
    private static let sessionMenuLimit = 12
    private static let onboardingSeenKey = "text2llm.onboardingSeen"
    ...
    static func openConfigFolder() {
        let url = Text2llmPaths.stateDirURL
        NSWorkspace.shared.activateFileViewerSelecting([url])
    }
    ...
    static func sendTestNotification() async {
        _ = await NotificationManager().send(title: "text2llm", body: "Test notification", sound: nil)
    }
    ...
    private static func resolveSessionStorePath() -> String {
        ...
        let configURL = Text2llmPaths.configURL
        guard
