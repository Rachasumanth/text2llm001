...
final class PeekabooBridgeHostCoordinator {
    ...
    private let logger = Logger(subsystem: "ai.text2llm", category: "PeekabooBridge")

    private var host: PeekabooBridgeHost?
    private var services: Text2llmPeekabooBridgeServices?
    private static var text2llmSocketPath: String {
        let fileManager = FileManager.default
        let base = fileManager.urls(for: .applicationSupportDirectory, in: .userDomainMask).first
            ?? fileManager.homeDirectoryForCurrentUser.appendingPathComponent("Library/Application Support")
        let directory = base.appendingPathComponent("text2llm", isDirectory: true)
    ...
    private func startIfNeeded() async {
        ...
        let services = Text2llmPeekabooBridgeServices()
        ...
        let host = PeekabooBridgeHost(
            socketPath: Self.text2llmSocketPath,
            server: server,
        ...
        await host.start()
        self.logger
            .info("PeekabooBridge host started at \(Self.text2llmSocketPath, privacy: .public)")
    }
...
@MainActor
private final class Text2llmPeekabooBridgeServices: PeekabooBridgeServiceProviding {
    ...
    init() {
        let logging = LoggingService(subsystem: "ai.text2llm.peekaboo")
