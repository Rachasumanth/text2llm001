...
enum Text2llmLogging {
    private static let labelSeparator = "::"

    private static let didBootstrap: Void = {
        LoggingSystem.bootstrap { label in
            let (subsystem, category) = Self.parseLabel(label)
            let osHandler = Text2llmOSLogHandler(subsystem: subsystem, category: category)
            let fileHandler = Text2llmFileLogHandler(label: label)
...
extension Logging.Logger {
    init(subsystem: String, category: String) {
        Text2llmLogging.bootstrapIfNeeded()
        let label = Text2llmLogging.makeLabel(subsystem: subsystem, category: category)
...
struct Text2llmOSLogHandler: LogHandler {
...
struct Text2llmFileLogHandler: LogHandler {
    let label: String
    var metadata: Logger.Metadata = [:]
        ...
        line: UInt)
    {
        ...
        let (subsystem, category) = Text2llmLogging.parseLabel(self.label)
