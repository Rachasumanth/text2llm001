...
struct ControlAgentEvent: Codable, Sendable, Identifiable {
    ...
    let stream: String
    let ts: Double
    let data: [String: Text2llmProtocol.AnyCodable]
    let summary: String?
}
...
final class ControlChannel {
    ...
    private let logger = Logger(subsystem: "ai.text2llm", category: "control")
    ...
    {
        do {
            let rawParams = params?.reduce(into: [String: Text2llmKit.AnyCodable]()) {
                $0[$1.key] = Text2llmKit.AnyCodable($1.value.base)
            }
    ...
    private static func bridgeToProtocolArgs(
        _ value: Text2llmProtocol.AnyCodable?) -> [String: Text2llmProtocol.AnyCodable]?
    {
        guard let value else { return nil }
        if let dict = value.value as? [String: Text2llmProtocol.AnyCodable] {
            return dict
        }
        if let dict = value.value as? [String: Text2llmKit.AnyCodable],
           let data = try? JSONEncoder().encode(dict),
           let decoded = try? JSONDecoder().decode([String: Text2llmProtocol.AnyCodable].self, from: data)
        {
            return decoded
        }
...
extension Notification.Name {
    static let controlHeartbeat = Notification.Name("text2llm.control.heartbeat")
    static let controlAgentEvent = Notification.Name("text2llm.control.agent")
}
