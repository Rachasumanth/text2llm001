...
public struct CanvasShowResult: Codable, Sendable {
    /// Session directory on disk (e.g. `~/Library/Application Support/text2llm/canvas/<session>/`).
...
public let controlSocketPath: String = {
    ...
    let preferred = home
        .appendingPathComponent("Library/Application Support/text2llm/control.sock")
        .path
    return preferred
}()
