...
@Suite struct MacGatewayChatTransportMappingTests {
    @Test func snapshotMapsToHealth() {
        let snapshot = Snapshot(
            presence: [],
            health: Text2llmProtocol.AnyCodable(["ok": Text2llmProtocol.AnyCodable(false)]),
    ...
    @Test func healthEventMapsToHealth() {
        let frame = EventFrame(
            type: "event",
            event: "health",
            payload: Text2llmProtocol.AnyCodable(["ok": Text2llmProtocol.AnyCodable(true)]),
            seq: 1,
            stateversion: nil)
    ...
    @Test func chatEventMapsToChat() {
        let payload = Text2llmProtocol.AnyCodable([
            "runId": Text2llmProtocol.AnyCodable("run-1"),
            "sessionKey": Text2llmProtocol.AnyCodable("main"),
            "state": Text2llmProtocol.AnyCodable("final"),
        ])
    ...
    @Test func unknownEventMapsToNil() {
        let frame = EventFrame(
            type: "event",
            event: "unknown",
            payload: Text2llmProtocol.AnyCodable(["a": Text2llmProtocol.AnyCodable(1)]),
            seq: 1,
            stateversion: nil)
        let mapped = MacGatewayChatTransport.mapPushToTransportEvent(.event(frame))
        #expect(mapped == nil)
