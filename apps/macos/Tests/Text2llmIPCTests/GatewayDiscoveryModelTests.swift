...
struct GatewayDiscoveryModelTests {
    ...
    @Test func localGatewayMatchesDisplayName() {
        ...
        #expect(GatewayDiscoveryModel.isLocalGateway(
            lanHost: nil,
            tailnetDns: nil,
            displayName: "Peter's Mac Studio (text2llm)",
            serviceName: nil,
            local: local))
    }
    ...
    @Test func serviceNameDoesNotFalsePositiveOnSubstringHostToken() {
        ...
        #expect(!GatewayDiscoveryModel.isLocalGateway(
            ...
            tailnetDns: nil,
            displayName: nil,
            serviceName: "steipetacstudio (text2llm)",
            local: local))
        #expect(GatewayDiscoveryModel.isLocalGateway(
            lanHost: nil,
            tailnetDns: nil,
            displayName: nil,
            serviceName: "steipete (text2llm)",
            local: local))
    }
    ...
    @Test func parsesGatewayTXTFields() {
        ...
        #expect(parsed.cliPath == "/opt/text2llm")
    }
