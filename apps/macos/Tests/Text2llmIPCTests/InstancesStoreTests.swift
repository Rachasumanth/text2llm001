...
@Suite struct InstancesStoreTests {
    ...
    func presenceEventPayloadDecodesViaJSONEncoder() {
        ...
        let entry: [String: Text2llmProtocol.AnyCodable] = [
            "host": .init("gw"),
            "ip": .init("10.0.0.1"),
            "version": .init("2.0.0"),
            "mode": .init("gateway"),
        ...
        let payloadMap: [String: Text2llmProtocol.AnyCodable] = [
            "presence": .init([Text2llmProtocol.AnyCodable(entry)]),
        ]
        let payload = Text2llmProtocol.AnyCodable(payloadMap)
