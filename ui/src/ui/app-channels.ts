...
export async function handleWhatsAppStart(host: TEXT2LLMApp, force: boolean) {
  await startWhatsAppLogin(host, force);
  await loadChannels(host, true);
}

export async function handleWhatsAppWait(host: TEXT2LLMApp) {
  await waitWhatsAppLogin(host);
  await loadChannels(host, true);
}

export async function handleWhatsAppLogout(host: TEXT2LLMApp) {
  await logoutWhatsApp(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigSave(host: TEXT2LLMApp) {
  await saveConfig(host);
  await loadConfig(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigReload(host: TEXT2LLMApp) {
  await loadConfig(host);
  await loadChannels(host, true);
}
...
function resolveNostrAccountId(host: TEXT2LLMApp): string {
...
function resolveGatewayHttpAuthHeader(host: TEXT2LLMApp): string | null {
...
function buildGatewayHttpHeaders(host: TEXT2LLMApp): Record<string, string> {
...
export function handleNostrProfileCancel(host: TEXT2LLMApp) {
...
export function handleNostrProfileToggleAdvanced(host: TEXT2LLMApp) {
  const state = host.nostrProfileFormState;
  if (!state) {
    return;
  }
...
export async function handleNostrProfileSave(host: TEXT2LLMApp) {
  const state = host.nostrProfileFormState;
...
export async function handleNostrProfileImport(host: TEXT2LLMApp) {
  const state = host.nostrProfileFormState;
