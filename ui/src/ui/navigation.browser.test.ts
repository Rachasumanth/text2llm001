...
describe("control UI routing", () => {
  ...
  it("infers nested base paths", async () => {
    const app = mountApp("/apps/text2llm/cron");
    await app.updateComplete;

    expect(app.basePath).toBe("/apps/text2llm");
    expect(app.tab).toBe("cron");
    expect(window.location.pathname).toBe("/apps/text2llm/cron");
  });

  it("honors explicit base path overrides", async () => {
    window.__TEXT2LLM_CONTROL_UI_BASE_PATH__ = "/text2llm";
    const app = mountApp("/text2llm/sessions");
    await app.updateComplete;

    expect(app.basePath).toBe("/text2llm");
    expect(app.tab).toBe("sessions");
    expect(window.location.pathname).toBe("/text2llm/sessions");
  });
