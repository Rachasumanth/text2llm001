 ...
 * `text2llm update`. Use it to verify the completion installation flow
...
\`text2llm update\`, \`text2llm doctor\`, and \`text2llm onboard\`.
...
async function main() {
  ...
  if (status.profileInstalled && status.cacheExists && !options.force) {
    ...
    console.log(
      theme.muted("  1. Remove the '# text2llm Completion' block from your shell profile"),
    );
  ...
  if (isCancel(shouldInstall) || !shouldInstall) {
    console.log(theme.muted(`Skipped. Run \`text2llm completion --install\` later to enable.`));
    return;
  }
