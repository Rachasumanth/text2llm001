...
ZIP=${1:?"Usage: $0 text2llm-<ver>.zip"}
FEED_URL=${2:-"https://raw.githubusercontent.com/text2llm/text2llm/main/appcast.xml"}
...
if [[ -z "$VERSION" ]]; then
  if [[ "$ZIP_NAME" =~ ^text2llm-([0-9]+(\.[0-9]+){1,2}([-.][^.]*)?)\.zip$ ]]; then
...
DOWNLOAD_URL_PREFIX=${SPARKLE_DOWNLOAD_URL_PREFIX:-"https://github.com/text2llm/text2llm/releases/download/v${VERSION}/"}
