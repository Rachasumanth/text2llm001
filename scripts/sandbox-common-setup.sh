...
BASE_IMAGE="${BASE_IMAGE:-text2llm-sandbox:bookworm-slim}"
TARGET_IMAGE="${TARGET_IMAGE:-text2llm-sandbox-common:bookworm-slim}"
...
If you want a clean re-create, remove old sandbox containers:
  docker rm -f \$(docker ps -aq --filter label=text2llm.sandbox=1)
NOTE
