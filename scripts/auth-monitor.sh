...
#
# Suggested cron: */30 * * * * /home/admin/text2llm/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send text2llm notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., text2llm-alerts)
...
STATE_FILE="$HOME/.text2llm/auth-monitor-state"
...
send_notification() {
    ...
    # Send via text2llm if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use text2llm
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via text2llm to $NOTIFY_PHONE..."
            text2llm send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! text2llm is down. Run: ssh l36 '~/text2llm/scripts/mobile-reauth.sh'" "urgent"
    exit 1
