...
# Build and bundle text2llm into a minimal .app we can open.
# Outputs to dist/text2llm.app

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
APP_ROOT="$ROOT_DIR/dist/text2llm.app"
BUILD_ROOT="$ROOT_DIR/apps/macos/.build"
PRODUCT="text2llm"
BUNDLE_ID="${BUNDLE_ID:-ai.text2llm.mac.debug}"
...
SPARKLE_FEED_URL="${SPARKLE_FEED_URL:-https://raw.githubusercontent.com/text2llm/text2llm/main/appcast.xml}"
AUTO_CHECKS=true
...
INFO_PLIST_SRC="$ROOT_DIR/apps/macos/Sources/text2llm/Resources/Info.plist"
...
/usr/libexec/PlistBuddy -c "Set :TEXT2LLMBuildTimestamp ${BUILD_TS}" "$APP_ROOT/Contents/Info.plist" || true
/usr/libexec/PlistBuddy -c "Set :TEXT2LLMGitCommit ${GIT_COMMIT}" "$APP_ROOT/Contents/Info.plist" || true
...
echo "🚚 Copying binary"
cp "$BIN_PRIMARY" "$APP_ROOT/Contents/MacOS/text2llm"
if [[ "${#BUILD_ARCHS[@]}" -gt 1 ]]; then
  BIN_INPUTS=()
  for arch in "${BUILD_ARCHS[@]}"; do
    BIN_INPUTS+=("$(bin_for_arch "$arch")")
  done
  /usr/bin/lipo -create "${BIN_INPUTS[@]}" -output "$APP_ROOT/Contents/MacOS/text2llm"
fi
chmod +x "$APP_ROOT/Contents/MacOS/text2llm"
# SwiftPM outputs ad-hoc signed binaries; strip the signature before install_name_tool to avoid warnings.
/usr/bin/codesign --remove-signature "$APP_ROOT/Contents/MacOS/text2llm" 2>/dev/null || true
...
cp "$ROOT_DIR/apps/macos/Sources/text2llm/Resources/text2llm.icns" "$APP_ROOT/Contents/Resources/text2llm.icns"

echo "📦 Copying device model resources"
rm -rf "$APP_ROOT/Contents/Resources/DeviceModels"
cp -R "$ROOT_DIR/apps/macos/Sources/text2llm/Resources/DeviceModels" "$APP_ROOT/Contents/Resources/DeviceModels"
...
echo "📦 Copying TEXT2LLMKit resources"
TEXT2LLMKIT_BUNDLE="$(build_path_for_arch "$PRIMARY_ARCH")/$BUILD_CONFIG/TEXT2LLMKit_TEXT2LLMKit.bundle"
if [ -d "$TEXT2LLMKIT_BUNDLE" ]; then
  rm -rf "$APP_ROOT/Contents/Resources/TEXT2LLMKit_TEXT2LLMKit.bundle"
  cp -R "$TEXT2LLMKIT_BUNDLE" "$APP_ROOT/Contents/Resources/TEXT2LLMKit_TEXT2LLMKit.bundle"
else
  echo "WARN: TEXT2LLMKit resource bundle not found at $TEXT2LLMKIT_BUNDLE (continuing)" >&2
fi
...
echo "⏹  Stopping any running text2llm"
killall -q text2llm 2>/dev/null || true
