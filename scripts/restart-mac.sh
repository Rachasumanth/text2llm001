#!/usr/bin/env bash
# Reset text2llm like Trimmy: kill running instances, rebuild, repackage, relaunch, verify.

set -euo pipefail

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
APP_BUNDLE="${TEXT2LLM_APP_BUNDLE:-}"
APP_PROCESS_PATTERN="text2llm.app/Contents/MacOS/text2llm"
DEBUG_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build/debug/text2llm"
LOCAL_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build-local/debug/text2llm"
RELEASE_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build/release/text2llm"
LAUNCH_AGENT="${HOME}/Library/LaunchAgents/ai.text2llm.mac.plist"
LOCK_KEY="$(printf '%s' "${ROOT_DIR}" | shasum -a 256 | cut -c1-8)"
LOCK_DIR="${TMPDIR:-/tmp}/text2llm-restart-${LOCK_KEY}"
LOCK_PID_FILE="${LOCK_DIR}/pid"
WAIT_FOR_LOCK=0
LOG_PATH="${TEXT2LLM_RESTART_LOG:-/tmp/text2llm-restart.log}"
NO_SIGN=0
SIGN=0
AUTO_DETECT_SIGNING=1
GATEWAY_WAIT_SECONDS="${TEXT2LLM_GATEWAY_WAIT_SECONDS:-0}"
LAUNCHAGENT_DISABLE_MARKER="${HOME}/.text2llm/disable-launchagent"
ATTACH_ONLY=1
...
for arg in "$@"; do
  case "${arg}" in
    ...
    --help|-h)
      ...
      log ""
      log "Env:"
      log "  TEXT2LLM_GATEWAY_WAIT_SECONDS=0  Wait time before gateway port check (unsigned only)"
      log ""
      log "Unsigned recovery:"
      log "  node text2llm.mjs daemon install --force --runtime node"
      log "  node text2llm.mjs daemon restart"
      log ""
      log "Reset unsigned overrides:"
      log "  rm ~/.text2llm/disable-launchagent"
      log ""
...
stop_launch_agent() {
  launchctl bootout gui/"$UID"/ai.text2llm.mac 2>/dev/null || true
}

# 1) Kill all running instances first.
log "==> Killing existing text2llm instances"
kill_all_TEXT2LLM
stop_launch_agent
...
run_step "swift build" bash -lc "cd '${ROOT_DIR}/apps/macos' && swift build -q --product text2llm"
...
choose_app_bundle() {
  ...
  if [[ -d "/Applications/text2llm.app" ]]; then
    APP_BUNDLE="/Applications/text2llm.app"
    return 0
  fi

  if [[ -d "${ROOT_DIR}/dist/text2llm.app" ]]; then
    APP_BUNDLE="${ROOT_DIR}/dist/text2llm.app"
    if [[ ! -d "${APP_BUNDLE}/Contents/Frameworks/Sparkle.framework" ]]; then
      fail "dist/text2llm.app missing Sparkle after packaging"
    fi
    return 0
  fi

  fail "App bundle not found. Set TEXT2LLM_APP_BUNDLE to your installed text2llm.app"
}
...
if [ "$NO_SIGN" -eq 1 ] && [ "$ATTACH_ONLY" -ne 1 ]; then
  run_step "install gateway launch agent (unsigned)" bash -lc "cd '${ROOT_DIR}' && node text2llm.mjs daemon install --force --runtime node"
  run_step "restart gateway daemon (unsigned)" bash -lc "cd '${ROOT_DIR}' && node text2llm.mjs daemon restart"
  ...
  GATEWAY_PORT="$(
    node -e '
      ...
      try {
        const raw = fs.readFileSync(path.join(process.env.HOME, ".text2llm", "text2llm.json"), "utf8");
