...
IMAGE_NAME="text2llm-onboard-e2e"
...
docker run --rm -t "$IMAGE_NAME" bash -lc '
  ...
  export PATH="/tmp/text2llm-bin:$PATH"
  mkdir -p /tmp/text2llm-bin
  cat > /tmp/text2llm-bin/trash <<'"'"'TRASH'"'"'
#!/usr/bin/env bash
set -euo pipefail
...
TRASH
  chmod +x /tmp/text2llm-bin/trash
  ...
  wait_for_log() {
    ...
    while true; do
	  ...
	  start_gateway() {
	    node "$TEXT2LLM_ENTRY" gateway --port 18789 --bind loopback --allow-unconfigured > /tmp/gateway-e2e.log 2>&1 &
	    GATEWAY_PID="$!"
	  }
  ...
  run_wizard_cmd() {
    ...
    input_fifo="$(mktemp -u "/tmp/text2llm-onboard-${case_name}.XXXXXX")"
    mkfifo "$input_fifo"
    local log_path="/tmp/text2llm-onboard-${case_name}.log"
  ...
  run_wizard() {
    ...
    local validate_fn="${4:-}"
	    ...
	    run_wizard_cmd "$case_name" "$home_dir" "node \"$TEXT2LLM_ENTRY\" onboard $ONBOARD_FLAGS" "$send_fn" true "$validate_fn"
	  }

  make_home() {
    mktemp -d "/tmp/text2llm-e2e-$1.XXXXXX"
  }
  ...
  send_skills_flow() {
    ...
    workspace_dir="$HOME/.text2llm/workspace"
    config_path="$HOME/.text2llm/text2llm.json"
    sessions_dir="$HOME/.text2llm/agents/main/sessions"
...
NODE
  ...
  run_case_remote_non_interactive() {
    ...
    export HOME="$home_dir"
	    ...
	    node "$TEXT2LLM_ENTRY" onboard --non-interactive --accept-risk \
	      --mode remote \
...
NODE
  ...
  run_case_reset() {
    ...
    # Seed a remote config to exercise reset path.
	    cat > "$HOME/.text2llm/text2llm.json" <<'"'"'JSON'"'"'
{
...
NODE
  ...
  run_case_channels() {
	    ...
	    run_wizard_cmd channels "$home_dir" "node \"$TEXT2LLM_ENTRY\" configure --section channels" send_channels_flow
...
JSON
	    ...
	    run_wizard_cmd skills "$home_dir" "node \"$TEXT2LLM_ENTRY\" configure --section skills" send_skills_flow
