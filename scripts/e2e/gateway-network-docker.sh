...
IMAGE_NAME="text2llm-gateway-network-e2e"

PORT="18789"
TOKEN="e2e-$(date +%s)-$$"
NET_NAME="text2llm-net-e2e-$$"
GW_NAME="text2llm-gateway-e2e-$$"
...
echo "Starting gateway container..."
	docker run --rm -d \
	  --name "$GW_NAME" \
	  --network "$NET_NAME" \
	  -e "TEXT2LLM_GATEWAY_TOKEN=$TOKEN" \
	  -e "TEXT2LLM_SKIP_CHANNELS=1" \
	  -e "TEXT2LLM_SKIP_GMAIL_WATCHER=1" \
	  -e "TEXT2LLM_SKIP_CRON=1" \
	  -e "TEXT2LLM_SKIP_CANVAS_HOST=1" \
	  "$IMAGE_NAME" \
