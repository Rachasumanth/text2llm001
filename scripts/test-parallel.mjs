...
const useVmForks =
  process.env.TEXT2LLM_TEST_VM_FORKS === "1" ||
  (process.env.TEXT2LLM_TEST_VM_FORKS !== "0" && !isWindows);
const runs = [
  ...(useVmForks
    ? [
        {
...
const shardOverride = Number.parseInt(process.env.TEXT2LLM_TEST_SHARDS ?? "", 10);
...
const silentArgs =
  process.env.TEXT2LLM_TEST_SHOW_PASSED_LOGS === "1" ? [] : ["--silent=passed-only"];
const rawPassthroughArgs = process.argv.slice(2);
const passthroughArgs =
  rawPassthroughArgs[0] === "--" ? rawPassthroughArgs.slice(1) : rawPassthroughArgs;
const overrideWorkers = Number.parseInt(process.env.TEXT2LLM_TEST_WORKERS ?? "", 10);
const resolvedOverride =
  Number.isFinite(overrideWorkers) && overrideWorkers > 0 ? overrideWorkers : null;
// Keep gateway serial on Windows CI and CI by default; run in parallel locally
// for lower wall-clock time. CI can opt in via TEXT2LLM_TEST_PARALLEL_GATEWAY=1.
const keepGatewaySerial =
  isWindowsCi ||
  process.env.TEXT2LLM_TEST_SERIAL_GATEWAY === "1" ||
  (isCI && process.env.TEXT2LLM_TEST_PARALLEL_GATEWAY !== "1");
...
function resolveReportDir() {
  const raw = process.env.TEXT2LLM_VITEST_REPORT_DIR?.trim();
  if (!raw) {
    return null;
  }
  try {
