...
APP_BUNDLE="${1:-dist/text2llm.app}"
IDENTITY="${SIGN_IDENTITY:-}"
TIMESTAMP_MODE="${CODESIGN_TIMESTAMP:-auto}"
DISABLE_LIBRARY_VALIDATION="${DISABLE_LIBRARY_VALIDATION:-0}"
SKIP_TEAM_ID_CHECK="${SKIP_TEAM_ID_CHECK:-0}"
ENT_TMP_BASE=$(mktemp -t text2llm-entitlements-base.XXXXXX)
ENT_TMP_APP_BASE=$(mktemp -t text2llm-entitlements-app-base.XXXXXX)
ENT_TMP_RUNTIME=$(mktemp -t text2llm-entitlements-runtime.XXXXXX)
...
# Sign main binary
if [ -f "$APP_BUNDLE/Contents/MacOS/text2llm" ]; then
  echo "Signing main binary"; sign_item "$APP_BUNDLE/Contents/MacOS/text2llm" "$APP_ENTITLEMENTS"
fi
