...
#
# Env:
#   DMG_VOLUME_NAME        default: CFBundleName (or "text2llm")
...
APP_NAME=$(/usr/libexec/PlistBuddy -c "Print CFBundleName" "$APP_PATH/Contents/Info.plist" 2>/dev/null || echo "text2llm")
...
DMG_TEMP="$(mktemp -d /tmp/text2llm-dmg.XXXXXX)"
...
if [[ "${SKIP_DMG_STYLE:-0}" != "1" ]]; then
  ...
  ICON_SRC="$ROOT_DIR/apps/macos/Sources/text2llm/Resources/text2llm.icns"
  if [[ -f "$ICON_SRC" ]]; then
...
hdiutil resize -limits "$DMG_RW_PATH" >/tmp/text2llm-dmg-limits.txt 2>/dev/null || true
MIN_SECTORS="$(tail -n 1 /tmp/text2llm-dmg-limits.txt 2>/dev/null | awk '{print $1}')"
rm -f /tmp/text2llm-dmg-limits.txt
