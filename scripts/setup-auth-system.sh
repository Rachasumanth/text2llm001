#!/bin/bash
# Setup text2llm Auth Management System
...
echo "=== text2llm Auth System Setup ==="
echo ""
...
echo "  2. Subscribe to a topic (e.g., 'text2llm-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  text2llm message: Send warning via text2llm itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/text2llm-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/text2llm-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/text2llm-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now text2llm-auth-monitor.timer
...
echo "  - Auth monitor: systemctl --user status text2llm-auth-monitor.timer"
echo ""
echo "Quick commands:"
