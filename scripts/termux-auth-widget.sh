...
# text2llm Auth Widget for Termux
...
# Check auth status
termux-toast "Checking text2llm auth..."

STATUS=$(ssh "$SERVER" '$HOME/text2llm/scripts/claude-auth-status.sh simple' 2>&1)
EXIT_CODE=$?

case "$STATUS" in
    OK)
        # Get remaining time
        DETAILS=$(ssh "$SERVER" '$HOME/text2llm/scripts/claude-auth-status.sh json' 2>&1)
    ...
    CLAUDE_EXPIRING|TEXT2LLM_EXPIRING|CLAWDBOT_EXPIRING)
...
3. SSH to server and paste key"
                ...
                termux-toast "Run: ssh $SERVER '$HOME/text2llm/scripts/mobile-reauth.sh'"
                ;;
            *)
    ...
    CLAUDE_EXPIRED|TEXT2LLM_EXPIRED|CLAWDBOT_EXPIRED)
...
2. Return here and tap OK to SSH"
                ...
                ;;
            *)
                termux-toast "Warning: text2llm won't work until re-auth"
                ;;
        esac
        ;;
