...
INSTALL_URL="${TEXT2LLM_INSTALL_URL:-https://text2llm.bot/install.sh}"
SMOKE_PREVIOUS_VERSION="${TEXT2LLM_INSTALL_SMOKE_PREVIOUS:-}"
SKIP_PREVIOUS="${TEXT2LLM_INSTALL_SMOKE_SKIP_PREVIOUS:-0}"
DEFAULT_PACKAGE="text2llm"
PACKAGE_NAME="${TEXT2LLM_INSTALL_PACKAGE:-$DEFAULT_PACKAGE}"
...
if [[ "$SKIP_PREVIOUS" == "1" ]]; then
  echo "==> Skip preinstall previous (TEXT2LLM_INSTALL_SMOKE_SKIP_PREVIOUS=1)"
else
...
if [[ -n "${TEXT2LLM_INSTALL_LATEST_OUT:-}" ]]; then
  printf "%s" "$LATEST_VERSION" > "${TEXT2LLM_INSTALL_LATEST_OUT:-}"
fi
