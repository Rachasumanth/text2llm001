...
# Checks both Claude Code and text2llm auth status

set -euo pipefail

CLAUDE_CREDS="$HOME/.claude/.credentials.json"
TEXT2LLM_AUTH="$HOME/.text2llm/agents/main/agent/auth-profiles.json"
...
fetch_models_status_json() {
    text2llm models status --json 2>/dev/null || true
}
...
check_TEXT2LLM_auth() {
    ...
    expires=$(jq -r '
        ...
        | max // 0
    ' "$TEXT2LLM_AUTH" 2>/dev/null || echo "0")
...
if [ "$OUTPUT_MODE" = "json" ]; then
    ...
    TEXT2LLM_status=$(check_TEXT2LLM_auth 2>/dev/null || true)

    claude_expires=0
    TEXT2LLM_expires=0
    if [ "$USE_JSON" -eq 1 ]; then
        claude_expires=$(json_expires_for_claude_cli)
        TEXT2LLM_expires=$(json_expires_for_anthropic_any)
    else
        claude_expires=$(jq -r '.claudeAiOauth.expiresAt // 0' "$CLAUDE_CREDS" 2>/dev/null || echo "0")
        TEXT2LLM_expires=$(jq -r '.profiles["anthropic:default"].expires // 0' "$TEXT2LLM_AUTH" 2>/dev/null || echo "0")
    fi

    jq -n \
        --arg cs "$claude_status" \
        --arg ce "$claude_expires" \
        --arg bs "$TEXT2LLM_status" \
        --arg be "$TEXT2LLM_expires" \
        '{
            claude_code: {status: $cs, expires_at_ms: ($ce | tonumber)},
            text2llm: {status: $bs, expires_at_ms: ($be | tonumber)},
...
if [ "$OUTPUT_MODE" = "simple" ]; then
    ...
    if [[ "$claude_status" == EXPIRED* ]] || [[ "$claude_status" == MISSING* ]]; then
        echo "CLAUDE_EXPIRED"
        exit 1
    elif [[ "$TEXT2LLM_status" == EXPIRED* ]] || [[ "$TEXT2LLM_status" == MISSING* ]]; then
        echo "TEXT2LLM_EXPIRED"
        exit 1
    elif [[ "$claude_status" == EXPIRING* ]]; then
        echo "CLAUDE_EXPIRING"
        exit 2
    elif [[ "$TEXT2LLM_status" == EXPIRING* ]]; then
        echo "TEXT2LLM_EXPIRING"
        exit 2
    else
        echo "OK"
        exit 0
...
echo ""
echo "text2llm Auth (~/.text2llm/agents/main/agent/auth-profiles.json):"
if [ "$USE_JSON" -eq 1 ]; then
    ...
    best_profile=$(jq -r '
        ...
        | .[0].key // "none"
    ' "$TEXT2LLM_AUTH" 2>/dev/null || echo "none")
    expires=$(jq -r '
        [.profiles | to_entries[] | select(.value.provider == "anthropic") | .value.expires]
        | max // 0
    ' "$TEXT2LLM_AUTH" 2>/dev/null || echo "0")
...
if [ "$expires" -le 0 ] && [ "$api_keys" -gt 0 ]; then
    ...
    echo -e "  Status: ${RED}NOT FOUND${NC}"
    echo "  Note: Run 'text2llm doctor --yes' to sync from Claude Code"
else
    now_ms=$(( $(date +%s) * 1000 ))
...
echo ""
echo "=== Service Status ==="
if systemctl --user is-active text2llm >/dev/null 2>&1; then
    echo -e "text2llm service: ${GREEN}running${NC}"
else
    echo -e "text2llm service: ${RED}NOT running${NC}"
fi
