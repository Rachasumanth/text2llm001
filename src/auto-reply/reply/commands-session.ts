...
import { scheduleGatewaySigusr1Restart, triggerTEXT2LLMRestart } from "../../infra/restart.js";
...
export const handleRestartCommand: CommandHandler = async (params, allowTextCommands) => {
  ...
  if (hasSigusr1Listener) {
    ...
    return {
      shouldContinue: false,
      reply: {
        text: "⚙️ Restarting text2llm in-process (SIGUSR1); back in a few seconds.",
      },
    };
  }
  const restartMethod = triggerTEXT2LLMRestart();
  if (!restartMethod.ok) {
  ...
  return {
    shouldContinue: false,
    reply: {
      text: `⚙️ Restarting text2llm via ${restartMethod.method}; give me a few seconds to come back online.`,
    },
  };
};
