...
export function isCommandEnabled(cfg: TEXT2LLMConfig, commandKey: string): boolean {
...
function resolveDefaultCommandContext(cfg?: TEXT2LLMConfig): {
  provider: string;
  model: string;
} {
  const resolved = resolveConfiguredModelRef({
    cfg: cfg ?? ({} as TEXT2LLMConfig),
...
export function getCommandDetection(_cfg?: TEXT2LLMConfig): CommandDetection {
  const commands = getChatCommands();
...
export function maybeResolveTextAlias(raw: string, cfg?: TEXT2LLMConfig) {
