...
import { loadTEXT2LLMPlugins } from "./loader.js";

type TempPlugin = { dir: string; file: string; id: string };

const tempDirs: string[] = [];
const prevBundledDir = process.env.TEXT2LLM_BUNDLED_PLUGINS_DIR;
const EMPTY_PLUGIN_SCHEMA = { type: "object", additionalProperties: false, properties: {} };

function makeTempDir() {
  const dir = path.join(os.tmpdir(), `text2llm-plugin-${randomUUID()}`);
...
function writePlugin(params: {
  ...
  fs.writeFileSync(
    path.join(dir, "text2llm.plugin.json"),
    JSON.stringify(
      {
        id: params.id,
...
afterEach(() => {
  ...
  if (prevBundledDir === undefined) {
    ...
    process.env.TEXT2LLM_BUNDLED_PLUGINS_DIR = prevBundledDir;
  }
});

describe("loadTEXT2LLMPlugins", () => {
  it("disables bundled plugins by default", () => {
    ...
    process.env.TEXT2LLM_BUNDLED_PLUGINS_DIR = bundledDir;

    const registry = loadTEXT2LLMPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
    ...
    const enabledRegistry = loadTEXT2LLMPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
  ...
  it("loads plugins from config paths", () => {
    process.env.TEXT2LLM_BUNDLED_PLUGINS_DIR = "/nonexistent/bundled/plugins";
