...
function makeTempDir() {
  const dir = path.join(os.tmpdir(), `text2llm-plugin-install-${randomUUID()}`);
...
describe("installPluginFromArchive", () => {
  it("installs into ~/.text2llm/extensions and uses unscoped id", async () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        name: "@text2llm/voice-call",
        version: "0.0.1",
        text2llm: { extensions: ["./dist/index.js"] },
      }),
      "utf-8",
    );
  ...
  it("rejects packages without text2llm.extensions", async () => {
    const stateDir = makeTempDir();
    const workDir = makeTempDir();
    const pkgDir = path.join(workDir, "package");
    ...
    fs.writeFileSync(
      path.join(pkgDir, "package.json"),
      JSON.stringify({ name: "@text2llm/nope", version: "0.0.1" }),
      "utf-8",
    );
    ...
    expect(result.error).toContain("text2llm.extensions");
  });
