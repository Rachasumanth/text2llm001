...
homepage: https://docs.text2llm.ai/hooks#command-logger
metadata:
  {
    "text2llm":
      {
...
2. **Appends to log file** - Writes a JSON line to `~/.text2llm/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.text2llm/logs/commands.log ~/.text2llm/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/text2llm`:
   ```
   /home/username/.text2llm/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
