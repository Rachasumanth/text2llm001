...
describe("loader", () => {
  ...
  beforeEach(async () => {
    ...
    tmpDir = path.join(os.tmpdir(), `text2llm-test-${Date.now()}`);
    await fs.mkdir(tmpDir, { recursive: true });

    // Disable bundled hooks during tests by setting env var to non-existent directory
    originalBundledDir = process.env.TEXT2LLM_BUNDLED_HOOKS_DIR;
    process.env.TEXT2LLM_BUNDLED_HOOKS_DIR = "/nonexistent/bundled/hooks";
  });
  ...
  afterEach(async () => {
    ...
    if (originalBundledDir === undefined) {
      delete process.env.TEXT2LLM_BUNDLED_HOOKS_DIR;
    } else {
      process.env.TEXT2LLM_BUNDLED_HOOKS_DIR = originalBundledDir;
    }
