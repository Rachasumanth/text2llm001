...
function makeTempDir() {
  const dir = path.join(os.tmpdir(), `text2llm-hook-install-${randomUUID()}`);
...
describe("installHooksFromArchive", () => {
  it("installs hook packs from zip archives", async () => {
    ...
    zip.file(
      ...
      JSON.stringify({
        name: "@text2llm/zip-hooks",
        version: "0.0.1",
        text2llm: { hooks: ["./hooks/zip-hook"] },
      }),
    );
    zip.file(
      ...
      [
        ...
        'metadata: {"text2llm":{"events":["command:new"]}}',
        "---",
        "",
        "# Zip Hook",
      ].join("\n"),
  ...
  it("installs hook packs from tar archives", async () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        name: "@text2llm/tar-hooks",
        version: "0.0.1",
        text2llm: { hooks: ["./hooks/tar-hook"] },
      }),
      "utf-8",
    );
    fs.writeFileSync(
  ...
  it("rejects hook packs with traversal-like ids", async () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        name: "@evil/..",
        version: "0.0.1",
        text2llm: { hooks: ["./hooks/evil-hook"] },
      }),
      "utf-8",
    );
    fs.writeFileSync(
  ...
  it("rejects hook packs with reserved ids", async () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        name: "@evil/.",
        version: "0.0.1",
        text2llm: { hooks: ["./hooks/reserved-hook"] },
      }),
      "utf-8",
    );
    fs.writeFileSync(
...
describe("installHooksFromPath", () => {
  it("uses --ignore-scripts for dependency install", async () => {
    ...
    fs.writeFileSync(
      path.join(pkgDir, "package.json"),
      JSON.stringify({
        name: "@text2llm/test-hooks",
        version: "0.0.1",
        text2llm: { hooks: ["./hooks/one-hook"] },
