...
async function createDockerSetupSandbox(): Promise<DockerSetupSandbox> {
  const rootDir = await mkdtemp(join(tmpdir(), "text2llm-docker-setup-"));
...
function createEnv(
  ...
  return {
    ...
    TEXT2LLM_GATEWAY_TOKEN: "test-token",
    TEXT2LLM_CONFIG_DIR: join(sandbox.rootDir, "config"),
    TEXT2LLM_WORKSPACE_DIR: join(sandbox.rootDir, "text2llm"),
    ...overrides,
  };
}
...
describe("docker-setup.sh", () => {
  it("handles unset optional env vars under strict mode", async () => {
    ...
    const env = createEnv(sandbox, {
      TEXT2LLM_DOCKER_APT_PACKAGES: undefined,
      TEXT2LLM_EXTRA_MOUNTS: undefined,
      TEXT2LLM_HOME_VOLUME: undefined,
    });
    ...
    expect(envFile).toContain("TEXT2LLM_DOCKER_APT_PACKAGES=");
    expect(envFile).toContain("TEXT2LLM_EXTRA_MOUNTS=");
    expect(envFile).toContain("TEXT2LLM_HOME_VOLUME=");
  });
  ...
  it("supports a home volume when extra mounts are empty", async () => {
    ...
    const env = createEnv(sandbox, {
      ...
      TEXT2LLM_HOME_VOLUME: "text2llm-home",
    });
    ...
    expect(extraCompose).toContain("text2llm-home:/home/node");
    expect(extraCompose).toContain("volumes:");
    expect(extraCompose).toContain("text2llm-home:");
  });
  ...
  it("plumbs TEXT2LLM_DOCKER_APT_PACKAGES into .env and docker build args", async () => {
    const sandbox = await createDockerSetupSandbox();
    const env = createEnv(sandbox, {
      TEXT2LLM_DOCKER_APT_PACKAGES: "ffmpeg build-essential",
      TEXT2LLM_EXTRA_MOUNTS: "",
      TEXT2LLM_HOME_VOLUME: "",
    });
    ...
    const envFile = await readFile(join(sandbox.rootDir, ".env"), "utf8");
    expect(envFile).toContain("TEXT2LLM_DOCKER_APT_PACKAGES=ffmpeg build-essential");

    const log = await readFile(sandbox.logPath, "utf8");
    expect(log).toContain("--build-arg TEXT2LLM_DOCKER_APT_PACKAGES=ffmpeg build-essential");
  });
