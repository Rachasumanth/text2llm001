...
function summarizeGroupPolicy(cfg: TEXT2LLMConfig): {
  open: number;
  allowlist: number;
  other: number;
} {
...
function isGatewayRemotelyExposed(cfg: TEXT2LLMConfig): boolean {
...
function collectModels(cfg: TEXT2LLMConfig): ModelRef[] {
  const out: ModelRef[] = [];
...
function hasWebSearchKey(cfg: TEXT2LLMConfig, env: NodeJS.ProcessEnv): boolean {
...
function isWebSearchEnabled(cfg: TEXT2LLMConfig, env: NodeJS.ProcessEnv): boolean {
...
function isWebFetchEnabled(cfg: TEXT2LLMConfig): boolean {
...
function isBrowserEnabled(cfg: TEXT2LLMConfig): boolean {
  try {
...
function listGroupPolicyOpen(cfg: TEXT2LLMConfig): string[] {
  const out: string[] = [];
  const channels = cfg.channels as Record<string, unknown> | undefined;
  if (!channels || typeof channels !== "object") {
    return out;
...
export function collectAttackSurfaceSummaryFindings(cfg: TEXT2LLMConfig): SecurityAuditFinding[] {
...
export function collectSyncedFolderFindings(params: {
  ...
  if (isProbablySyncedPath(params.stateDir) || isProbablySyncedPath(params.configPath)) {
    findings.push({
      ...
      remediation: `Keep TEXT2LLM_STATE_DIR on a local-only volume and re-run "${formatCliCommand("text2llm security audit --fix")}".`,
    });
  }
  return findings;
}

export function collectSecretsInConfigFindings(cfg: TEXT2LLMConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
  const password =
  ...
  if (password && !looksLikeEnvRef(password)) {
    findings.push({
      ...
      remediation:
        "Prefer TEXT2LLM_GATEWAY_PASSWORD (env) and remove gateway.auth.password from disk.",
    });
  }
...
export function collectHooksHardeningFindings(cfg: TEXT2LLMConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectModelHygieneFindings(cfg: TEXT2LLMConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectExposureMatrixFindings(cfg: TEXT2LLMConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
