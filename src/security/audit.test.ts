...
describe("security audit", () => {
  ...
  it("treats Windows ACL-only perms as secure", async () => {
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "text2llm-security-audit-win-"));
    const stateDir = path.join(tmp, "state");
    await fs.mkdir(stateDir, { recursive: true });
    const configPath = path.join(stateDir, "text2llm.json");
  ...
  it("flags Windows ACLs when Users can read the state dir", async () => {
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "text2llm-security-audit-win-open-"));
    const stateDir = path.join(tmp, "state");
    await fs.mkdir(stateDir, { recursive: true });
    const configPath = path.join(stateDir, "text2llm.json");
    await fs.writeFile(configPath, "{}\n", "utf-8");
  ...
  it("warns when multiple DM senders share the main session", async () => {
    const cfg: TEXT2LLMConfig = { session: { dmScope: "main" } };
  ...
  it("flags Discord native commands without a guild user allowlist", async () => {
    ...
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "text2llm-security-audit-discord-"));
    process.env.TEXT2LLM_STATE_DIR = tmp;
  ...
  it("does not flag Discord slash commands when dm.allowFrom includes a Discord snowflake id", async () => {
    ...
    const tmp = await fs.mkdtemp(
      path.join(os.tmpdir(), "text2llm-security-audit-discord-allowfrom-snowflake-"),
    );
    process.env.TEXT2LLM_STATE_DIR = tmp;
    await fs.mkdir(path.join(tmp, "credentials"), { recursive: true, mode: 0o700 });
    try {
  ...
  it("flags Discord slash commands when access-group enforcement is disabled and no users allowlist exists", async () => {
    const prevStateDir = process.env.TEXT2LLM_STATE_DIR;
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "text2llm-security-audit-discord-open-"));
    process.env.TEXT2LLM_STATE_DIR = tmp;
    await fs.mkdir(path.join(tmp, "credentials"), { recursive: true, mode: 0o700 });
    try {
      const cfg: TEXT2LLMConfig = {
  ...
  it("flags Slack slash commands without a channel users allowlist", async () => {
    const prevStateDir = process.env.TEXT2LLM_STATE_DIR;
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "text2llm-security-audit-slack-"));
    process.env.TEXT2LLM_STATE_DIR = tmp;
    await fs.mkdir(path.join(tmp, "credentials"), { recursive: true, mode: 0o700 });
    try {
      const cfg: TEXT2LLMConfig = {
  ...
  it("flags Slack slash commands when access-group enforcement is disabled", async () => {
    const prevStateDir = process.env.TEXT2LLM_STATE_DIR;
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "text2llm-security-audit-slack-open-"));
    process.env.TEXT2LLM_STATE_DIR = tmp;
    await fs.mkdir(path.join(tmp, "credentials"), { recursive: true, mode: 0o700 });
    try {
      const cfg: TEXT2LLMConfig = {
  ...
  it("flags Telegram group commands without a sender allowlist", async () => {
    const prevStateDir = process.env.TEXT2LLM_STATE_DIR;
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "text2llm-security-audit-telegram-"));
    process.env.TEXT2LLM_STATE_DIR = tmp;
    await fs.mkdir(path.join(tmp, "credentials"), { recursive: true, mode: 0o700 });
    try {
      const cfg: TEXT2LLMConfig = {
  ...
  it("adds a warning when deep probe fails", async () => {
    const cfg: TEXT2LLMConfig = { gateway: { mode: "local" } };
  ...
  it("warns when hooks token reuses the gateway env token", async () => {
    const prevToken = process.env.TEXT2LLM_GATEWAY_TOKEN;
    process.env.TEXT2LLM_GATEWAY_TOKEN = "shared-gateway-token-1234567890";
    const cfg: TEXT2LLMConfig = {
  ...
  it("warns when state/config look like a synced folder", async () => {
    ...
    const res = await runSecurityAudit({
      ...
      includeFilesystem: false,
      includeChannelSecurity: false,
      stateDir: "/Users/test/Dropbox/.text2llm",
      configPath: "/Users/test/Dropbox/.text2llm/text2llm.json",
    });
  ...
  it("flags group/world-readable config include files", async () => {
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "text2llm-security-audit-"));
    const stateDir = path.join(tmp, "state");
    ...
    try {
      const cfg: TEXT2LLMConfig = { logging: { redactSensitive: "off" } };
      const user = "DESKTOP-TEST\\Tester";
  ...
  it("flags extensions without plugins.allow", async () => {
    ...
    try {
      ...
      const res = await runSecurityAudit({
        ...
        includeChannelSecurity: false,
        stateDir,
        configPath: path.join(stateDir, "text2llm.json"),
      });
  ...
  it("flags plugins with dangerous code patterns (deep audit)", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "text2llm-audit-scanner-"));
    ...
    await fs.writeFile(
      ...
      JSON.stringify({
        name: "evil-plugin",
        text2llm: { extensions: [".hidden/index.js"] },
      }),
    );
    await fs.writeFile(
...
`,
  ...
  it("flags plugin extension entry path traversal in deep audit", async () => {
    ...
    await fs.writeFile(
      ...
      JSON.stringify({
        name: "escape-plugin",
        text2llm: { extensions: ["../outside.js"] },
      }),
    );
  ...
  describe("maybeProbeGateway auth selection", () => {
    const originalEnvToken = process.env.TEXT2LLM_GATEWAY_TOKEN;
    const originalEnvPassword = process.env.TEXT2LLM_GATEWAY_PASSWORD;
    ...
    afterEach(() => {
      if (originalEnvToken == null) {
        ...
        process.env.TEXT2LLM_GATEWAY_TOKEN = originalEnvToken;
      }
      if (originalEnvPassword == null) {
        delete process.env.TEXT2LLM_GATEWAY_PASSWORD;
      } else {
        process.env.TEXT2LLM_GATEWAY_PASSWORD = originalEnvPassword;
      }
    });
    ...
    it("prefers env token over local config token", async () => {
      process.env.TEXT2LLM_GATEWAY_TOKEN = "env-token";
      let capturedAuth: { token?: string; password?: string } | undefined;
      const cfg: TEXT2LLMConfig = {
        gateway: {
          mode: "local",
    ...
    it("prefers env password over remote password", async () => {
      process.env.TEXT2LLM_GATEWAY_PASSWORD = "env-pass";
      let capturedAuth: { token?: string; password?: string } | undefined;
      const cfg: TEXT2LLMConfig = {
        gateway: {
          mode: "remote",
