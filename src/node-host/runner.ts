...
const execHostEnforced = process.env.TEXT2LLM_NODE_EXEC_HOST?.trim().toLowerCase() === "app";
const execHostFallbackAllowed =
  process.env.TEXT2LLM_NODE_EXEC_FALLBACK?.trim().toLowerCase() !== "0";
...
function ensureNodePathEnv(): string {
  ensureTEXT2LLMCliOnPath({ pathEnv: process.env.PATH ?? "" });
