...
vi.mock("../config/sessions.js", async (importOriginal) => {
  ...
  return {
    ...actual,
    resolveStorePath: vi.fn(() => "/tmp/text2llm-sessions.json"),
...
describe("discord tool result dispatch", () => {
      ...
      const cfg = {
        agents: {
          defaults: {
            ...
            workspace: "/tmp/text2llm",
          },
        },
        session: { store: "/tmp/text2llm-sessions.json" },
        channels: {
          discord: {
        ...
        messages: {
          responsePrefix: "PFX",
          groupChat: { mentionPatterns: ["\\bTEXT2LLM\\b"] },
        },
    ...
    await handler(
      {
        message: {
          ...
          referencedMessage: {
            ...
            mentionedUsers: [],
            mentionedRoles: [],
            author: { id: "bot-id", bot: true, username: "text2llm" },
          },
        },
        author: { id: "u1", bot: false, username: "Ada" },
        member: { nickname: "Ada" },
  ...
  it("treats forum threads as distinct sessions without channel payloads", async () => {
    ...
    const cfg = {
      agent: { model: "anthropic/claude-opus-4-5", workspace: "/tmp/text2llm" },
      session: { store: "/tmp/text2llm-sessions.json" },
      channels: {
        discord: {
          dm: { enabled: true, policy: "open" },
