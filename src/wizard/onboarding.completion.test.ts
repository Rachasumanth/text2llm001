...
describe("setupOnboardingShellCompletion", () => {
  it("QuickStart: installs without prompting", async () => {
    ...
    const deps = {
      resolveCliName: () => "text2llm",
      checkShellCompletionStatus: vi.fn(async () => ({
        shell: "zsh",
        profileInstalled: false,
        cacheExists: false,
        cachePath: "/tmp/text2llm.zsh",
    ...
    expect(deps.ensureCompletionCacheExists).toHaveBeenCalledWith("text2llm");
    expect(deps.installCompletion).toHaveBeenCalledWith("zsh", true, "text2llm");
