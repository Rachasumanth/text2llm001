...
export function triggerTEXT2LLMRestart(): RestartAttempt {
  ...
  if (process.platform !== "darwin") {
    if (process.platform === "linux") {
      const unit = normalizeSystemdUnit(
        process.env.TEXT2LLM_SYSTEMD_UNIT,
        process.env.TEXT2LLM_PROFILE,
      );
  ...
  const label =
    process.env.TEXT2LLM_LAUNCHD_LABEL ||
    resolveGatewayLaunchAgentLabel(process.env.TEXT2LLM_PROFILE);
