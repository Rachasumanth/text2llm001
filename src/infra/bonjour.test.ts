...
describe("gateway bonjour advertiser", () => {
  ...
  it("does not block on advertise and publishes expected txt keys", async () => {
    ...
    process.env.TEXT2LLM_MDNS_HOSTNAME = "test-host";
    process.env.TEXT2LLM_MDNS_HOSTNAME = "test-host";
    ...
    expect(gatewayCall?.[0]?.type).toBe("text2llm-gw");
  ...
  it("normalizes hostnames with domains for service names", async () => {
    ...
    expect(gatewayCall?.[0]?.name).toBe("text2llm (text2llm)");
    expect(gatewayCall?.[0]?.domain).toBe("local");
    expect(gatewayCall?.[0]?.hostname).toBe("text2llm");
    expect((gatewayCall?.[0]?.txt as Record<string, string>)?.lanHost).toBe("text2llm.local");
