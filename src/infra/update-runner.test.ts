...
describe("runGatewayUpdate", () => {
  ...
  beforeEach(async () => {
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "text2llm-update-"));
    await fs.writeFile(path.join(tempDir, "text2llm.mjs"), "export {};\n", "utf-8");
  });
  ...
  it("skips git update when worktree is dirty", async () => {
    ...
    await fs.writeFile(
      ...
      JSON.stringify({ name: "text2llm", version: "1.0.0" }),
      "utf-8",
    );
  ...
  it("uses stable tag when beta tag is older than release", async () => {
    ...
    await fs.writeFile(
      path.join(tempDir, "package.json"),
      JSON.stringify({ name: "text2llm", version: "1.0.0", packageManager: "pnpm@8.0.0" }),
      "utf-8",
    );
    ...
    const { runner, calls } = createRunner({
      ...
      [`${process.execPath} ${path.join(tempDir, "text2llm.mjs")} doctor --non-interactive`]: {
        stdout: "",
      },
    });
  ...
  it("skips update when no git root", async () => {
    await fs.writeFile(
      path.join(tempDir, "package.json"),
      JSON.stringify({ name: "text2llm", packageManager: "pnpm@8.0.0" }),
      "utf-8",
    );
  ...
  it("updates global npm installs when detected", async () => {
    ...
    const pkgRoot = path.join(nodeModules, "text2llm");
    ...
    const runCommand = async (argv: string[]) => {
      ...
      if (key === "npm i -g text2llm@latest") {
        await fs.writeFile(
          path.join(pkgRoot, "package.json"),
          JSON.stringify({ name: "text2llm", version: "2.0.0" }),
          "utf-8",
        );
    ...
    expect(calls.some((call) => call === "npm i -g text2llm@latest")).toBe(true);
  });
  ...
  it("uses update channel for global npm installs when tag is omitted", async () => {
    ...
    expect(result.before?.version).toBe("1.0.0");
    expect(result.after?.version).toBe("2.0.0");
    expect(calls.some((call) => call === "npm i -g text2llm@beta")).toBe(true);
  });

  it("cleans stale npm rename dirs before global update", async () => {
    const nodeModules = path.join(tempDir, "node_modules");
    const pkgRoot = path.join(nodeModules, "text2llm");
    const staleDir = path.join(nodeModules, ".text2llm-stale");
  ...
  it("updates global bun installs when detected", async () => {
    ...
    try {
      ...
      const pkgRoot = path.join(bunGlobalRoot, "text2llm");
      await fs.mkdir(pkgRoot, { recursive: true });
      await fs.writeFile(
        path.join(pkgRoot, "package.json"),
        JSON.stringify({ name: "text2llm", version: "1.0.0" }),
      ...
      const runCommand = async (argv: string[]) => {
        ...
        if (key === "bun add -g text2llm@latest") {
          await fs.writeFile(
            path.join(pkgRoot, "package.json"),
            JSON.stringify({ name: "text2llm", version: "2.0.0" }),
            "utf-8",
      ...
      expect(result.before?.version).toBe("1.0.0");
      expect(result.after?.version).toBe("2.0.0");
      expect(calls.some((call) => call === "bun add -g text2llm@latest")).toBe(true);
    } finally {
  ...
  it("rejects git roots that are not a text2llm checkout", async () => {
    await fs.mkdir(path.join(tempDir, ".git"));
    ...
    expect(result.status).toBe("error");
    expect(result.reason).toBe("not-text2llm-root");
    expect(calls.some((call) => call.includes("status --porcelain"))).toBe(false);
  });

  it("fails with a clear reason when text2llm.mjs is missing", async () => {
    await fs.mkdir(path.join(tempDir, ".git"));
    await fs.writeFile(
      path.join(tempDir, "package.json"),
      JSON.stringify({ name: "text2llm", version: "1.0.0", packageManager: "pnpm@8.0.0" }),
    ...
    await fs.rm(path.join(tempDir, "text2llm.mjs"), { force: true });
    ...
    expect(result.steps.at(-1)?.name).toBe("text2llm doctor entry");
  });
  ...
  it("repairs UI assets when doctor run removes control-ui files", async () => {
    ...
    const runCommand = async (argv: string[]) => {
      ...
      if (
        key === `${process.execPath} ${path.join(tempDir, "text2llm.mjs")} doctor --non-interactive`
      ) {
