...
import { resolveTEXT2LLMPackageRoot, resolveTEXT2LLMPackageRootSync } from "./text2llm-root.js";
...
export async function resolveControlUiDistIndexPath(
  ...
  const packageRoot = await resolveTEXT2LLMPackageRoot({ argv1: normalized, moduleUrl });
  if (packageRoot) {
    return path.join(packageRoot, "dist", "control-ui", "index.html");
  }

  // Fallback: traverse up and find package.json with name "text2llm" + dist/control-ui/index.html
...
export function resolveControlUiRootSync(opts: ControlUiRootResolveOptions = {}): string | null {
  ...
  if (argv1Dir) {
    // text2llm.mjs or dist/<bundle>.js
