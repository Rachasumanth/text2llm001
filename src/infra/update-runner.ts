...
export async function runGatewayUpdate(opts: UpdateRunnerOptions = {}): Promise<UpdateRunResult> {
  ...
  if (gitRoot && pkgRoot && path.resolve(gitRoot) === path.resolve(pkgRoot)) {
    ...
    if (channel === "dev") {
      ...
      const preflightRoot = await fs.mkdtemp(path.join(os.tmpdir(), "text2llm-update-preflight-"));
    ...
    const doctorEntry = path.join(gitRoot, "text2llm.mjs");
    ...
    const doctorStep = await runStep(
      step("text2llm doctor", doctorArgv, gitRoot, { TEXT2LLM_UPDATE_IN_PROGRESS: "1" }),
    );
