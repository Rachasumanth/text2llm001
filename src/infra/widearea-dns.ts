...
export function resolveWideAreaDiscoveryDomain(params?: {
  ...
  const candidate = params?.configDomain ?? env.TEXT2LLM_WIDE_AREA_DOMAIN ?? null;
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*text2llm-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "text2llm";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "text2llm");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "text2llm-gw");
  ...
  records.push(`_TEXT2LLM-gw._tcp IN PTR ${instanceLabel}._TEXT2LLM-gw._tcp`);
  records.push(`${instanceLabel}._TEXT2LLM-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._TEXT2LLM-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; text2llm-content-hash: ${contentHash}\n${contentBody}`;
}
