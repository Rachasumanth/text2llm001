...
describe("session cost usage", () => {
  it("aggregates daily totals with log cost and pricing fallback", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "text2llm-cost-"));
    ...
    const originalState = process.env.TEXT2LLM_STATE_DIR;
    process.env.TEXT2LLM_STATE_DIR = root;
    try {
      ...
      if (originalState === undefined) {
        ...
        process.env.TEXT2LLM_STATE_DIR = originalState;
      }
    }
  });

  it("summarizes a single session file", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "text2llm-cost-session-"));
  ...
  it("captures message counts, tool usage, and model usage", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "text2llm-cost-session-meta-"));
    const sessionFile = path.join(root, "session.jsonl");
  ...
  it("does not exclude sessions with mtime after endMs during discovery", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "text2llm-discover-"));
    const sessionsDir = path.join(root, "agents", "main", "sessions");
    await fs.mkdir(sessionsDir, { recursive: true });
