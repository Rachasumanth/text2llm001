...
async function withTempDir<T>(run: (dir: string) => Promise<T>): Promise<T> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "text2llm-run-node-"));
  try {
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      await withTempDir(async (tmp) => {
        ...
        await fs.writeFile(
          path.join(tmp, "text2llm.mjs"),
          "#!/usr/bin/env node\nif (process.argv.includes('--version')) console.log('9.9.9-test');\n",
          "utf-8",
        );
        await fs.chmod(path.join(tmp, "text2llm.mjs"), 0o755);
...
`;
        ...
        const env = {
          ...
          TEXT2LLM_FORCE_BUILD: "1",
          TEXT2LLM_RUNNER_LOG: "0",
        };
