...
function candidateBinDirs(opts: EnsureTEXT2LLMPathOpts): string[] {
  ...
  // Bundled macOS app: `text2llm` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const siblingCli = path.join(execDir, "text2llm");
  ...
  // Project-local installs (best effort): if a `node_modules/.bin/text2llm` exists near cwd,
  // include it. This helps when running under launchd or other minimal PATH environments.
  const localBinDir = path.join(cwd, "node_modules", ".bin");
  if (isExecutable(path.join(localBinDir, "text2llm"))) {
...
/**
 * Best-effort PATH bootstrap so skills that require the `text2llm` CLI can run
 * under launchd/minimal environments (and inside the macOS app bundle).
 */
export function ensureTEXT2LLMCliOnPath(opts: EnsureTEXT2LLMPathOpts = {}) {
  if (isTruthyEnvValue(process.env.TEXT2LLM_PATH_BOOTSTRAPPED)) {
    return;
  }
  process.env.TEXT2LLM_PATH_BOOTSTRAPPED = "1";
