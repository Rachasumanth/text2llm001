...
export type ExtraGatewayService = {
  ...
  detail: string;
  scope: "user" | "system";
  marker?: "text2llm" | "clawdbot" | "moltbot";
  legacy?: boolean;
};
...
const EXTRA_MARKERS = ["text2llm", "clawdbot", "moltbot"] as const;
const execFileAsync = promisify(execFile);

export function renderGatewayServiceCleanupHints(
  env: Record<string, string | undefined> = process.env as Record<string, string | undefined>,
): string[] {
  const profile = env.TEXT2LLM_PROFILE;
...
function hasGatewayServiceMarker(content: string): boolean {
  const lower = content.toLowerCase();
  const markerKeys = ["TEXT2LLM_service_marker"];
  const kindKeys = ["TEXT2LLM_service_kind"];
...
function isTEXT2LLMGatewayLaunchdService(label: string, contents: string): boolean {
  ...
  return label.startsWith("ai.text2llm.");
}

function isTEXT2LLMGatewaySystemdService(name: string, contents: string): boolean {
  if (hasGatewayServiceMarker(contents)) {
    return true;
  }
  if (!name.startsWith("text2llm-gateway")) {
    return false;
  }
  return contents.toLowerCase().includes("gateway");
}

function isTEXT2LLMGatewayTaskName(name: string): boolean {
  const normalized = name.trim().toLowerCase();
  if (!normalized) {
    return false;
  }
  const defaultName = resolveGatewayWindowsTaskName().toLowerCase();
  return normalized === defaultName || normalized.startsWith("text2llm gateway");
}
...
async function scanLaunchdDir(params: {
  ...
  for (const entry of entries) {
    ...
    if (marker === "text2llm" && isTEXT2LLMGatewayLaunchdService(label, contents)) {
      continue;
    }
    results.push({
      platform: "darwin",
      ...
      scope: params.scope,
      marker,
      legacy: marker !== "text2llm" || isLegacyLabel(label),
    });
  }
...
async function scanSystemdDir(params: {
  ...
  for (const entry of entries) {
    ...
    if (marker === "text2llm" && isTEXT2LLMGatewaySystemdService(name, contents)) {
      continue;
    }
    results.push({
      platform: "linux",
...
export async function findExtraGatewayServices(
  ...
  if (process.platform === "win32") {
    ...
    for (const task of tasks) {
      ...
      if (isTEXT2LLMGatewayTaskName(name)) {
        continue;
      }
