...
function resolveTaskName(env: Record<string, string | undefined>): string {
  const override = env.TEXT2LLM_WINDOWS_TASK_NAME?.trim();
  if (override) {
    return override;
  }
  return resolveGatewayWindowsTaskName(env.TEXT2LLM_PROFILE);
}

export function resolveTaskScriptPath(env: Record<string, string | undefined>): string {
  const override = env.TEXT2LLM_TASK_SCRIPT?.trim();
  if (override) {
    return override;
  }
  const scriptName = env.TEXT2LLM_TASK_SCRIPT_NAME?.trim() || "gateway.cmd";
