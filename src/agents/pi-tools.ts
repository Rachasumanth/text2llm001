...
function resolveExecConfig(cfg: TEXT2LLMConfig | undefined) {
...
export function createTEXT2LLMCodingTools(options?: {
  ...
  const base = (codingTools as unknown as AnyAgentTool[]).flatMap((tool) => {
    if (tool.name === readTool.name) {
      ...
      return [createTEXT2LLMReadTool(freshReadTool)];
    }
