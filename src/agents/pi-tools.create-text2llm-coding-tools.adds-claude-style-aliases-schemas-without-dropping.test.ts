...
import { __testing, createTEXT2LLMCodingTools } from "./pi-tools.js";
...
describe("createTEXT2LLMCodingTools", () => {
  ...
  it("keeps raw core tool schemas union-free", () => {
    const tools = createTEXT2LLMTools();
  ...
  it("does not expose provider-specific message tools", () => {
    const tools = createTEXT2LLMCodingTools({ messageProvider: "discord" });
  ...
  it("applies sandbox path guards to file_path alias", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "text2llm-sbx-"));
    const outsidePath = path.join(os.tmpdir(), "text2llm-outside.txt");
