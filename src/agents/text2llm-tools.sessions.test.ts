...
describe("sessions tools", () => {
  ...
  it("sessions_list filters kinds and includes messages", async () => {
    ...
    const tool = createTEXT2LLMTools().find((candidate) => candidate.name === "sessions_list");
    expect(tool).toBeDefined();
    if (!tool) {
  ...
  it("sessions_history filters tool messages by default", async () => {
    ...
    const tool = createTEXT2LLMTools().find((candidate) => candidate.name === "sessions_history");
    expect(tool).toBeDefined();
    if (!tool) {
