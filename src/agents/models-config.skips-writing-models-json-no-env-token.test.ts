...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeBase(fn, { prefix: "text2llm-models-" });
}

const MODELS_CONFIG: TEXT2LLMConfig = {
  models: {
    providers: {
      "custom-proxy": {
...
describe("models-config", () => {
  ...
  it("skips writing models.json when no env token or profile exists", async () => {
    await withTempHome(async (home) => {
      ...
      try {
        vi.resetModules();
        const { ensureTEXT2LLMModelsJson } = await import("./models-config.js");

        const agentDir = path.join(home, "agent-empty");
        const result = await ensureTEXT2LLMModelsJson(
          {
            models: { providers: {} },
          },
          agentDir,
  ...
  it("writes models.json for configured providers", async () => {
    await withTempHome(async () => {
      vi.resetModules();
      const { ensureTEXT2LLMModelsJson } = await import("./models-config.js");
      const { resolveTEXT2LLMAgentDir } = await import("./agent-paths.js");

      await ensureTEXT2LLMModelsJson(MODELS_CONFIG);

      const modelPath = path.join(resolveTEXT2LLMAgentDir(), "models.json");
  ...
  it("adds minimax provider when MINIMAX_API_KEY is set", async () => {
    await withTempHome(async () => {
      ...
      try {
        ...
        await ensureTEXT2LLMModelsJson({});
