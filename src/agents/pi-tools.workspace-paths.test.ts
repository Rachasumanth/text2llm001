...
describe("workspace path resolution", () => {
  it("reads relative paths against workspaceDir even after cwd changes", async () => {
    await withTempDir("text2llm-ws-", async (workspaceDir) => {
      await withTempDir("text2llm-cwd-", async (otherDir) => {
        ...
        try {
          const tools = createTEXT2LLMCodingTools({ workspaceDir });
    ...
    await withTempDir("text2llm-ws-", async (workspaceDir) => {
      await withTempDir("text2llm-cwd-", async (otherDir) => {
        ...
        try {
          ...
          expect(updated).toBe("hello text2llm");
        } finally {
          cwdSpy.mockRestore();
        }
      });
  ...
  it("defaults exec cwd to workspaceDir when workdir is omitted", async () => {
    await withTempDir("text2llm-ws-", async (workspaceDir) => {
      const tools = createTEXT2LLMCodingTools({ workspaceDir, exec: { host: "gateway" } });
  ...
  it("lets exec workdir override the workspace default", async () => {
    await withTempDir("text2llm-ws-", async (workspaceDir) => {
      await withTempDir("text2llm-override-", async (overrideDir) => {
        const tools = createTEXT2LLMCodingTools({ workspaceDir, exec: { host: "gateway" } });
        const execTool = tools.find((tool) => tool.name === "exec");
        expect(execTool).toBeDefined();
...
describe("sandboxed workspace paths", () => {
  it("uses sandbox workspace for relative read/write/edit", async () => {
    await withTempDir("text2llm-sandbox-", async (sandboxDir) => {
      await withTempDir("text2llm-workspace-", async (workspaceDir) => {
        const sandbox = {
          enabled: true,
          ...
          workspaceAccess: "rw",
          containerName: "text2llm-sbx-test",
        ...
        const tools = createTEXT2LLMCodingTools({ workspaceDir, sandbox });
        const readTool = tools.find((tool) => tool.name === "read");
        const writeTool = tools.find((tool) => tool.name === "write");
        const editTool = tools.find((tool) => tool.name === "edit");
