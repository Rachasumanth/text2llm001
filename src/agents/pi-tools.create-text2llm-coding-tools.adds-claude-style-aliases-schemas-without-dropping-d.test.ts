...
const defaultTools = createTEXT2LLMCodingTools();

describe("createTEXT2LLMCodingTools", () => {
  it("keeps read tool image metadata intact", async () => {
    const readTool = defaultTools.find((tool) => tool.name === "read");
    expect(readTool).toBeDefined();

    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "text2llm-read-"));
    try {
  ...
  it("returns text content without image blocks for text files", async () => {
    ...
    try {
      ...
      const contents = "Hello from text2llm read tool.";
  ...
  it("filters tools by sandbox policy", () => {
    const sandbox = {
      ...
      workspaceDir: path.join(os.tmpdir(), "text2llm-sandbox"),
      agentWorkspaceDir: path.join(os.tmpdir(), "text2llm-workspace"),
    ...
    const tools = createTEXT2LLMCodingTools({ sandbox });
