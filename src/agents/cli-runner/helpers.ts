...
/**
 * Cleanup suspended text2llm CLI processes that have accumulated.
...
function buildModelAliasLines(cfg?: TEXT2LLMConfig) {
...
export async function writeCliImages(
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "text2llm-cli-images-"));
  const paths: string[] = [];
