...
const makeConfig = (opts?: { fallbacks?: string[]; apiKey?: string }): TEXT2LLMConfig =>
  ({
    agents: {
      defaults: {
        model: {
...
describe("runEmbeddedPiAgent auth profile rotation", () => {
  it("rotates for auto-pinned profiles", async () => {
    const agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "text2llm-agent-"));
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "text2llm-workspace-"));
    try {
