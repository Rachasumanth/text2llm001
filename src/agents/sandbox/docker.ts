...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "text2llm.sandbox=1");
  args.push("--label", `text2llm.sessionKey=${params.scopeKey}`);
  args.push("--label", `text2llm.createdAtMs=${createdAtMs}`);
  if (params.configHash) {
    args.push("--label", `text2llm.configHash=${params.configHash}`);
  }
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  ...
  return await readLabel("text2llm.configHash");
}

function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`text2llm sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`text2llm sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("text2llm sandbox recreate --all");
}
