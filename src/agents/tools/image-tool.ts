...
function resolveDefaultModelRef(cfg?: TEXT2LLMConfig): {
  provider: string;
  model: string;
} {
  if (cfg) {
...
function pickMaxBytes(cfg?: TEXT2LLMConfig, maxBytesMb?: number): number | undefined {
...
async function runImagePrompt(params: {
  ...
  const effectiveCfg: TEXT2LLMConfig | undefined = params.cfg
    ? {
        ...params.cfg,
        agents: {
          ...params.cfg.agents,
  ...
  await ensureTEXT2LLMModelsJson(effectiveCfg, params.agentDir);
