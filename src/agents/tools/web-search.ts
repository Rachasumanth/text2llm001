...
type WebSearchConfig = NonNullable<TEXT2LLMConfig["tools"]>["web"] extends infer Web
...
function resolveSearchConfig(cfg?: TEXT2LLMConfig): WebSearchConfig {
...
function missingSearchKeyPayload(provider: (typeof SEARCH_PROVIDERS)[number]) {
  ...
  return {
    ...
    message: `web_search needs a Brave Search API key. Run \`${formatCliCommand("text2llm configure --section web")}\` to store it, or set BRAVE_API_KEY in the Gateway environment.`,
    docs: "https://docs.text2llm.ai/tools/web",
  };
}
...
async function runPerplexitySearch(params: {
  ...
  const res = await fetch(endpoint, {
    ...
    headers: {
      ...
      "HTTP-Referer": "https://text2llm.ai",
      "X-Title": "text2llm Web Search",
    },
    body: JSON.stringify({
      model,
      messages: [
