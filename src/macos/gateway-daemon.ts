...
async function main() {
  if (hasFlag(args, "--version") || hasFlag(args, "-v")) {
    // Match `text2llm --version` behavior for Swift env/version checks.
  ...
  const portRaw =
    argValue(args, "--port") ??
    process.env.TEXT2LLM_GATEWAY_PORT ??
  ...
  const bindRaw =
    argValue(args, "--bind") ??
    process.env.TEXT2LLM_GATEWAY_BIND ??
...
void main().catch((err) => {
  console.error(
    "[text2llm] Gateway daemon failed:",
