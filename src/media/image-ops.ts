...
function prefersSips(): boolean {
  return (
    process.env.TEXT2LLM_IMAGE_BACKEND === "sips" ||
    (process.env.TEXT2LLM_IMAGE_BACKEND !== "sharp" && isBun() && process.platform === "darwin")
  );
}
...
async function withTempDir<T>(fn: (dir: string) => Promise<T>): Promise<T> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "text2llm-img-"));
  try {
    return await fn(dir);
  } finally {
