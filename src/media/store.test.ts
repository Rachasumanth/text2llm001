...
describe("media store", () => {
  ...
  const snapshotEnv = () => {
    for (const key of ["HOME", "USERPROFILE", "HOMEDRIVE", "HOMEPATH", "TEXT2LLM_STATE_DIR"]) {
  ...
  beforeAll(async () => {
    snapshotEnv();
    home = await fs.mkdtemp(path.join(os.tmpdir(), "text2llm-test-home-"));
    process.env.HOME = home;
    process.env.USERPROFILE = home;
    process.env.TEXT2LLM_STATE_DIR = path.join(home, ".text2llm");
  ...
  it("creates and returns media directory", async () => {
    await withTempStore(async (store, home) => {
      ...
      expect(path.normalize(dir)).toContain(`${path.sep}.text2llm${path.sep}media`);
