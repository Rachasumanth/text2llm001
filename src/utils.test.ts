...
describe("ensureDir", () => {
  it("creates nested directory", async () => {
    const tmp = await fs.promises.mkdtemp(path.join(os.tmpdir(), "text2llm-test-"));
...
describe("jidToE164", () => {
  ...
  it("maps @lid from authDir mapping files", () => {
    const authDir = fs.mkdtempSync(path.join(os.tmpdir(), "text2llm-auth-"));
  ...
  it("falls back through lidMappingDirs in order", () => {
    const first = fs.mkdtempSync(path.join(os.tmpdir(), "text2llm-lid-a-"));
    const second = fs.mkdtempSync(path.join(os.tmpdir(), "text2llm-lid-b-"));
...
describe("resolveConfigDir", () => {
  it("prefers ~/.text2llm when legacy dir is missing", async () => {
    const root = await fs.promises.mkdtemp(path.join(os.tmpdir(), "text2llm-config-dir-"));
    try {
...
describe("resolveHomeDir", () => {
  it("prefers TEXT2LLM_HOME over HOME", () => {
    vi.stubEnv("TEXT2LLM_HOME", "/srv/text2llm-home");
    vi.stubEnv("HOME", "/home/other");

    expect(resolveHomeDir()).toBe(path.resolve("/srv/text2llm-home"));
...
describe("shortenHomePath", () => {
  it("uses $TEXT2LLM_HOME prefix when TEXT2LLM_HOME is set", () => {
    vi.stubEnv("TEXT2LLM_HOME", "/srv/text2llm-home");
    vi.stubEnv("HOME", "/home/other");

    expect(shortenHomePath(`${path.resolve("/srv/text2llm-home")}/.text2llm/text2llm.json`)).toBe(
      "$TEXT2LLM_HOME/.text2llm/text2llm.json",
    );
...
describe("shortenHomeInString", () => {
  it("uses $TEXT2LLM_HOME replacement when TEXT2LLM_HOME is set", () => {
    vi.stubEnv("TEXT2LLM_HOME", "/srv/text2llm-home");
    vi.stubEnv("HOME", "/home/other");

    expect(
      shortenHomeInString(`config: ${path.resolve("/srv/text2llm-home")}/.text2llm/text2llm.json`),
    ).toBe("config: $TEXT2LLM_HOME/.text2llm/text2llm.json");
...
describe("resolveUserPath", () => {
  ...
  it("expands ~/ to home dir", () => {
    expect(resolveUserPath("~/text2llm")).toBe(path.resolve(os.homedir(), "text2llm"));
  });
  ...
  it("prefers TEXT2LLM_HOME for tilde expansion", () => {
    vi.stubEnv("TEXT2LLM_HOME", "/srv/text2llm-home");
    vi.stubEnv("HOME", "/home/other");

    expect(resolveUserPath("~/text2llm")).toBe(path.resolve("/srv/text2llm-home", "text2llm"));
