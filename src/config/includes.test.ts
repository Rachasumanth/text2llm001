...
const ETC_TEXT2LLM_DIR = path.join(ROOT_DIR, "etc", "text2llm");
const SHARED_DIR = path.join(ROOT_DIR, "shared");

const DEFAULT_BASE_PATH = path.join(CONFIG_DIR, "text2llm.json");

function configPath(...parts: string[]) {
  return path.join(CONFIG_DIR, ...parts);
}

function etcTEXT2LLMPath(...parts: string[]) {
  return path.join(ETC_TEXT2LLM_DIR, ...parts);
}
...
describe("resolveConfigIncludes", () => {
  ...
  it("resolves absolute path $include", () => {
    const absolute = etcTEXT2LLMPath("agents.json");
  ...
  it("resolves parent directory references", () => {
    ...
    expect(resolve(obj, files, configPath("sub", "text2llm.json"))).toEqual({
      shared: true,
    });
  });
});
