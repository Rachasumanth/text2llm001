...
describe("resolveStorePath", () => {
  ...
  it("uses TEXT2LLM_HOME for tilde expansion", () => {
    vi.stubEnv("TEXT2LLM_HOME", "/srv/text2llm-home");
    vi.stubEnv("HOME", "/home/other");

    const resolved = resolveStorePath("~/.text2llm/agents/{agentId}/sessions/sessions.json", {
      agentId: "research",
    });

    expect(resolved).toBe(
      path.resolve("/srv/text2llm-home/.text2llm/agents/research/sessions/sessions.json"),
    );
  });
});
...
describe("session path safety", () => {
  ...
  it("resolves transcript path inside an explicit sessions dir", () => {
    const sessionsDir = "/tmp/text2llm/agents/main/sessions";
