...
describe("config env vars", () => {
  ...
  it("loads ${VAR} substitutions from ~/.text2llm/.env on repeated runtime loads", async () => {
    await withTempHome(async (home) => {
      await withEnvOverride(
        {
          TEXT2LLM_STATE_DIR: path.join(home, ".text2llm"),
          CLAWDBOT_STATE_DIR: undefined,
          TEXT2LLM_HOME: undefined,
          CLAWDBOT_HOME: undefined,
          BRAVE_API_KEY: undefined,
          TEXT2LLM_DISABLE_CONFIG_CACHE: "1",
        },
        async () => {
