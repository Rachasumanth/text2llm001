...
describe("sessions", () => {
  ...
  it("builds discord display name with guild+channel slugs", () => {
    expect(
      buildGroupDisplayName({
        ...
        space: "friends-of-text2llm",
        id: "123",
        key: "discord:group:123",
      }),
    ).toBe("discord:friends-of-text2llm#general");
  });
  ...
  it("updateLastRoute persists channel and target", async () => {
    ...
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "text2llm-sessions-"));
  ...
  it("derives session transcripts dir from TEXT2LLM_STATE_DIR", () => {
    const dir = resolveSessionTranscriptsDir(
      { TEXT2LLM_STATE_DIR: "/custom/state" } as NodeJS.ProcessEnv,
      () => "/home/ignored",
    );
  ...
  it("includes topic ids in session transcript filenames", () => {
    const prev = process.env.TEXT2LLM_STATE_DIR;
    process.env.TEXT2LLM_STATE_DIR = "/custom/state";
    try {
      ...
      if (prev === undefined) {
        ...
        process.env.TEXT2LLM_STATE_DIR = prev;
      }
    }
  });
