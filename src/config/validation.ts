...
import type { TEXT2LLMConfig, ConfigValidationIssue } from "./types.js";
...
function validateIdentityAvatar(config: TEXT2LLMConfig): ConfigValidationIssue[] {
...
export function validateConfigObjectRaw(
  raw: unknown,
): { ok: true; config: TEXT2LLMConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const validated = TEXT2LLMSchema.safeParse(raw);
  ...
  const duplicates = findDuplicateAgentDirs(validated.data as TEXT2LLMConfig);
  ...
  const avatarIssues = validateIdentityAvatar(validated.data as TEXT2LLMConfig);
  if (avatarIssues.length > 0) {
    return { ok: false, issues: avatarIssues };
  }
  return {
    ok: true,
    config: validated.data as TEXT2LLMConfig,
  };
}
