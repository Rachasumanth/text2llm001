...
import type { TEXT2LLMConfig, ModelProviderConfig } from "../config/types.js";
import { resolveTEXT2LLMAgentDir } from "../agents/agent-paths.js";
...
import { ensureTEXT2LLMModelsJson } from "../agents/models-config.js";
...
const GATEWAY_LIVE = isTruthyEnvValue(process.env.TEXT2LLM_LIVE_GATEWAY);
const ZAI_FALLBACK = isTruthyEnvValue(process.env.TEXT2LLM_LIVE_GATEWAY_ZAI_FALLBACK);
const PROVIDERS = parseFilter(process.env.TEXT2LLM_LIVE_GATEWAY_PROVIDERS);
...
function sanitizeAuthConfig(params: {
  ...
  let profiles: NonNullable<TEXT2LLMConfig["auth"]>["profiles"] | undefined;
  if (auth.profiles) {
    profiles = {};
...
async function runGatewayModelSuite(params: GatewayModelSuiteParams) {
  const previous = {
    ...
    agentDir: process.env.TEXT2LLM_AGENT_DIR,
    piAgentDir: process.env.PI_CODING_AGENT_DIR,
    stateDir: process.env.TEXT2LLM_STATE_DIR,
  };
  ...
  const hostAgentDir = resolveTEXT2LLMAgentDir();
  ...
  tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "text2llm-live-state-"));
  ...
  process.env.TEXT2LLM_AGENT_DIR = tempAgentDir;
  ...
  const toolProbePath = path.join(workspaceDir, `.text2llm-live-tool-probe.${nonceA}.txt`);
  await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);

  const agentDir = resolveTEXT2LLMAgentDir();
  const sanitizedCfg: TEXT2LLMConfig = {
    ...params.cfg,
  ...
  try {
        ...
        try {
          ...
          const toolProbe = await client.request<AgentFinalPayload>(
            "agent",
            {
              ...
              message:
                "text2llm live tool probe (local, safe): " +
    ...
    process.env.TEXT2LLM_AGENT_DIR = previous.agentDir;
    process.env.PI_CODING_AGENT_DIR = previous.piAgentDir;
    process.env.TEXT2LLM_STATE_DIR = previous.stateDir;
  }
}
...
describeLive("gateway live (dev agent, profile keys)", () => {
  it(
    ...
    async () => {
      ...
      await ensureTEXT2LLMModelsJson(cfg);
      ...
      const rawModels = process.env.TEXT2LLM_LIVE_GATEWAY_MODELS?.trim();
  ...
  it("z.ai fallback handles anthropic tool history", async () => {
    ...
    const nonceA = randomUUID();
    const nonceB = randomUUID();
    const toolProbePath = path.join(workspaceDir, `.text2llm-live-zai-fallback.${nonceA}.txt`);
    await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);
