...
beforeAll(async () => {
  previousToken = process.env.TEXT2LLM_GATEWAY_TOKEN;
...
afterAll(async () => {
  ...
  if (previousToken === undefined) {
    ...
    process.env.TEXT2LLM_GATEWAY_TOKEN = previousToken;
  }
});
...
describe("gateway server sessions", () => {
  ...
  test("sessions.preview returns transcript previews", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "text2llm-sessions-preview-"));
    const storePath = path.join(dir, "sessions.json");
    testState.sessionStorePath = storePath;
