...
describe("gateway e2e", () => {
  it(
    ...
    async () => {
      const prev = {
        ...
        skipBrowser: process.env.TEXT2LLM_SKIP_BROWSER_CONTROL_SERVER,
      };

      const { baseUrl: openaiBaseUrl, restore } = installOpenAiResponsesMock();

      const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "text2llm-gw-mock-home-"));
      ...
      const workspaceDir = path.join(tempHome, "text2llm");
      await fs.mkdir(workspaceDir, { recursive: true });

      const nonceA = randomUUID();
      const nonceB = randomUUID();
      const toolProbePath = path.join(workspaceDir, `.text2llm-tool-probe.${nonceA}.txt`);
      await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);

      const configDir = path.join(tempHome, ".text2llm");
      await fs.mkdir(configDir, { recursive: true });
      const configPath = path.join(configDir, "text2llm.json");
      ...
      try {
        ...
        process.env.TEXT2LLM_CONFIG_PATH = prev.configPath;
        process.env.TEXT2LLM_GATEWAY_TOKEN = prev.token;
        process.env.TEXT2LLM_SKIP_CHANNELS = prev.skipChannels;
        process.env.TEXT2LLM_SKIP_GMAIL_WATCHER = prev.skipGmail;
        process.env.TEXT2LLM_SKIP_CRON = prev.skipCron;
        process.env.TEXT2LLM_SKIP_CANVAS_HOST = prev.skipCanvas;
        process.env.TEXT2LLM_SKIP_BROWSER_CONTROL_SERVER = prev.skipBrowser;
      }
    },
  );
  ...
  it("runs wizard over ws and writes auth token config", { timeout: 90_000 }, async () => {
    ...
    const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "text2llm-wizard-home-"));
    process.env.HOME = tempHome;
    ...
    try {
      ...
      await fs.rm(tempHome, { recursive: true, force: true });
      process.env.HOME = prev.home;
      process.env.TEXT2LLM_STATE_DIR = prev.stateDir;
      process.env.TEXT2LLM_CONFIG_PATH = prev.configPath;
      process.env.TEXT2LLM_GATEWAY_TOKEN = prev.token;
      process.env.TEXT2LLM_SKIP_CHANNELS = prev.skipChannels;
      process.env.TEXT2LLM_SKIP_GMAIL_WATCHER = prev.skipGmail;
