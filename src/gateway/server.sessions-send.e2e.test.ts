...
import { createTEXT2LLMTools } from "../agents/text2llm-tools.js";
...
beforeAll(async () => {
  ...
  prevGatewayToken = process.env.TEXT2LLM_GATEWAY_TOKEN;
  gatewayPort = await getFreePort();
  testState.gatewayAuth = { mode: "token", token: gatewayToken };
  process.env.TEXT2LLM_GATEWAY_PORT = String(gatewayPort);
  process.env.TEXT2LLM_GATEWAY_TOKEN = gatewayToken;
...
afterAll(async () => {
  ...
  if (prevGatewayToken === undefined) {
    ...
    process.env.TEXT2LLM_GATEWAY_TOKEN = prevGatewayToken;
  }
});
...
describe("sessions_send gateway loopback", () => {
  it("returns reply when lifecycle ends before agent.wait", async () => {
    ...
    const tool = createTEXT2LLMTools().find((candidate) => candidate.name === "sessions_send");
    if (!tool) {
