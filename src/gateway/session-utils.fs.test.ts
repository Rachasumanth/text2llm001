...
describe("readFirstUserMessageFromTranscript", () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "text2llm-session-fs-test-"));
...
describe("readSessionMessages", () => {
  ...
  test("includes synthetic compaction markers for compaction entries", () => {
    ...
    const marker = out[1] as {
      ...
      __TEXT2LLM?: { kind?: string; id?: string };
      timestamp?: number;
    };
    expect(marker.role).toBe("system");
    expect(marker.content?.[0]?.text).toBe("Compaction");
    expect(marker.__TEXT2LLM?.kind).toBe("compaction");
    expect(marker.__TEXT2LLM?.id).toBe("comp-1");
...
describe("readSessionPreviewItemsFromTranscript", () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "text2llm-session-preview-test-"));
    storePath = path.join(tmpDir, "sessions.json");
  });
...
describe("resolveSessionTranscriptCandidates", () => {
  ...
  test("fallback candidate uses TEXT2LLM_HOME instead of os.homedir()", () => {
    ...
    expect(fallback).toBe(
      path.join(path.resolve("/srv/text2llm-home"), ".text2llm", "sessions", "sess-1.jsonl"),
    );
  });
});
...
describe("resolveSessionTranscriptCandidates safety", () => {
  test("drops unsafe session IDs instead of producing traversal paths", () => {
    const candidates = resolveSessionTranscriptCandidates(
      "../etc/passwd",
      "/tmp/text2llm/agents/main/sessions/sessions.json",
    );
  ...
  test("drops unsafe sessionFile candidates and keeps safe fallbacks", () => {
    const storePath = "/tmp/text2llm/agents/main/sessions/sessions.json";
    const candidates = resolveSessionTranscriptCandidates(
      "sess-safe",
      storePath,
      "../../etc/passwd",
