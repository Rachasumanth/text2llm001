...
describe("gateway hot reload", () => {
  ...
  beforeEach(() => {
    prevSkipChannels = process.env.TEXT2LLM_SKIP_CHANNELS;
    prevSkipGmail = process.env.TEXT2LLM_SKIP_GMAIL_WATCHER;
    process.env.TEXT2LLM_SKIP_CHANNELS = "0";
  ...
  afterEach(() => {
    if (prevSkipChannels === undefined) {
      ...
      process.env.TEXT2LLM_SKIP_CHANNELS = prevSkipChannels;
    }
    if (prevSkipGmail === undefined) {
      delete process.env.TEXT2LLM_SKIP_GMAIL_WATCHER;
    } else {
      process.env.TEXT2LLM_SKIP_GMAIL_WATCHER = prevSkipGmail;
    }
  });
