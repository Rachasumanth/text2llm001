...
describe("gateway config.patch", () => {
  it("merges patches without clobbering unrelated config", async () => {
    ...
    expect(get2Res.payload?.config?.channels?.telegram?.botToken).toBe("__TEXT2LLM_REDACTED__");
  ...
  it("preserves credentials on config.set when raw contains redacted sentinels", async () => {
    ...
    expect(rawRedacted).toContain("__TEXT2LLM_REDACTED__");
  ...
  it("writes config, stores sentinel, and schedules restart", async () => {
    ...
    const sentinelPath = path.join(os.homedir(), ".text2llm", "restart-sentinel.json");
...
describe("gateway server sessions", () => {
  it("filters sessions by agentId", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "text2llm-sessions-agents-"));
