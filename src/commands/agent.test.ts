...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeBase(fn, { prefix: "text2llm-agent-" });
}

function mockConfig(
  home: string,
  storePath: string,
  agentOverrides?: Partial<NonNullable<NonNullable<TEXT2LLMConfig["agents"]>["defaults"]>>,
  telegramOverrides?: Partial<NonNullable<TEXT2LLMConfig["telegram"]>>,
