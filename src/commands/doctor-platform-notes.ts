...
export async function noteMacLaunchAgentOverrides() {
  ...
  const markerCandidates = [path.join(home, ".text2llm", "disable-launchagent")];
...
function hasConfigGatewayCreds(cfg: TEXT2LLMConfig): boolean {
  const localToken =
...
export async function noteMacLaunchctlGatewayEnvOverrides(
  ...
  if (deprecatedLaunchctlEntries.length > 0) {
    const lines = [
      ...
      ...deprecatedLaunchctlEntries.map(
        ([key]) =>
          `- \`${key}\` is set; use \`TEXT2LLM_${key.slice(key.indexOf("_") + 1)}\` instead.`,
      ),
    ];
  ...
  const tokenEntries = [
    ["TEXT2LLM_GATEWAY_TOKEN", await getenv("TEXT2LLM_GATEWAY_TOKEN")],
  ] as const;
  const passwordEntries = [
    ["TEXT2LLM_GATEWAY_PASSWORD", await getenv("TEXT2LLM_GATEWAY_PASSWORD")],
  ] as const;
  ...
  const lines = [
    ...
    envToken && envTokenKey
      ...
      : undefined,
    envPassword
      ? `- \`${envPasswordKey ?? "TEXT2LLM_GATEWAY_PASSWORD"}\` is set; it overrides config passwords.`
      : undefined,
...
export function noteDeprecatedLegacyEnvVars(
  ...
  const lines = [
    ...
    "- Use TEXT2LLM_* equivalents instead:",
    ...entries.map((key) => {
      const suffix = key.slice(key.indexOf("_") + 1);
      return `  ${key} -> TEXT2LLM_${suffix}`;
    }),
  ];
  (deps?.noteFn ?? note)(lines.join("\n"), "Environment");
}
