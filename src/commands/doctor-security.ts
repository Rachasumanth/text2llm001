...
import type { TEXT2LLMConfig, GatewayBindMode } from "../config/config.js";
...
export async function noteSecurityWarnings(cfg: TEXT2LLMConfig) {
  const warnings: string[] = [];
  const auditHint = `- Run: ${formatCliCommand("text2llm security audit --deep")}`;
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("text2llm configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("text2llm config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("text2llm doctor --fix")} to generate a token`,
      ...
      warnings.push(
        ...
        `  Fix: ${formatCliCommand("text2llm config set gateway.bind loopback")}`,
        ...authFixLines,
      );
    } else {
