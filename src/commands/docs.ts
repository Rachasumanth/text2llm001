...
const SEARCH_TOOL = "https://docs.text2llm.ai/mcp.SearchTEXT2LLM";
...
export async function docsSearchCommand(queryParts: string[], runtime: RuntimeEnv) {
  ...
  if (!query) {
    const docs = formatDocsLink("/", "docs.text2llm.ai");
    if (isRich()) {
      runtime.log(`${theme.muted("Docs:")} ${docs}`);
      runtime.log(`${theme.muted("Search:")} ${formatCliCommand('text2llm docs "your query"')}`);
    } else {
      runtime.log("Docs: https://docs.text2llm.ai/");
      runtime.log(`Search: ${formatCliCommand('text2llm docs "your query"')}`);
    }
    return;
  }
