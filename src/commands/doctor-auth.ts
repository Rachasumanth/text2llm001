...
function pruneAuthProfiles(
  cfg: TEXT2LLMConfig,
  profileIds: Set<string>,
): { next: TEXT2LLMConfig; changed: boolean } {
...
export async function maybeRemoveDeprecatedCliAuthProfiles(
  ...
  if (deprecated.has(CLAUDE_CLI_PROFILE_ID)) {
    lines.push(
      `- ${CLAUDE_CLI_PROFILE_ID} (Anthropic): use setup-token → ${formatCliCommand("text2llm models auth setup-token")}`,
    );
  }
  if (deprecated.has(CODEX_CLI_PROFILE_ID)) {
    lines.push(
      `- ${CODEX_CLI_PROFILE_ID} (OpenAI Codex): use OAuth → ${formatCliCommand(
        "text2llm models auth login --provider openai-codex",
      )}`,
    );
  }
  note(lines.join("\n"), "Auth profiles");
...
function formatAuthIssueHint(issue: AuthIssue): string | null {
  if (issue.provider === "anthropic" && issue.profileId === CLAUDE_CLI_PROFILE_ID) {
    return `Deprecated profile. Use ${formatCliCommand("text2llm models auth setup-token")} or ${formatCliCommand(
      "text2llm configure",
    )}.`;
  }
  if (issue.provider === "openai-codex" && issue.profileId === CODEX_CLI_PROFILE_ID) {
    return `Deprecated profile. Use ${formatCliCommand(
      "text2llm models auth login --provider openai-codex",
    )} or ${formatCliCommand("text2llm configure")}.`;
  }
  return `Re-auth via \`${formatCliCommand("text2llm configure")}\` or \`${formatCliCommand("text2llm onboard")}\`.`;
}
