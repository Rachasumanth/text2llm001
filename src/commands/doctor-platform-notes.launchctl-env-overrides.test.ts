...
describe("noteMacLaunchctlGatewayEnvOverrides", () => {
  it("prints clear unsetenv instructions for token override", async () => {
    ...
    const getenv = vi.fn(async (name: string) =>
      name === "TEXT2LLM_GATEWAY_TOKEN" ? "launchctl-token" : undefined,
    );
    const cfg = {
      gateway: {
        auth: {
    ...
    expect(message).toContain("TEXT2LLM_GATEWAY_TOKEN");
    expect(message).toContain("launchctl unsetenv TEXT2LLM_GATEWAY_TOKEN");
    expect(message).not.toContain("TEXT2LLM_GATEWAY_PASSWORD");
  });
