...
beforeAll(() => {
  previousProfile = process.env.TEXT2LLM_PROFILE;
  process.env.TEXT2LLM_PROFILE = "isolated";
});
...
afterAll(() => {
  if (previousProfile === undefined) {
    ...
    process.env.TEXT2LLM_PROFILE = previousProfile;
  }
});
...
vi.mock("../infra/text2llm-root.js", () => ({
  resolveTEXT2LLMPackageRoot: vi.fn().mockResolvedValue("/tmp/text2llm"),
}));
...
vi.mock("../infra/update-check.js", () => ({
  checkUpdateStatus: vi.fn().mockResolvedValue({
    ...
    deps: {
      manager: "pnpm",
      status: "ok",
      lockfilePath: "/tmp/text2llm/pnpm-lock.yaml",
      markerPath: "/tmp/text2llm/node_modules/.modules.yaml",
    },
...
describe("statusCommand", () => {
  ...
  it("prints formatted lines otherwise", async () => {
    ...
    await statusCommand({}, runtime as never);
    const logs = (runtime.log as vi.Mock).mock.calls.map((c) => String(c[0]));
    expect(logs.some((l) => l.includes("text2llm status"))).toBe(true);
    ...
    expect(
      logs.some(
        (l) =>
          l.includes("text2llm status --all") ||
          l.includes("text2llm --profile isolated status --all") ||
          l.includes("text2llm status --all") ||
          l.includes("text2llm --profile isolated status --all"),
      ),
    ).toBe(true);
  ...
  it("shows gateway auth when reachable", async () => {
    ...
    process.env.TEXT2LLM_GATEWAY_TOKEN = "abcd1234";
    try {
