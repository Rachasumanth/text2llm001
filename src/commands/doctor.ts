...
function resolveMode(cfg: TEXT2LLMConfig): "local" | "remote" {
...
export async function doctorCommand(
  ...
  intro("text2llm doctor");

  const root = await resolveTEXT2LLMPackageRoot({
  ...
  let cfg: TEXT2LLMConfig = configResult.cfg;

  const configPath = configResult.path ?? CONFIG_PATH;
  if (!cfg.gateway?.mode) {
    const lines = [
      "gateway.mode is unset; gateway start will be blocked.",
      `Fix: run ${formatCliCommand("text2llm configure")} and set Gateway mode (local/remote).`,
      `Or set directly: ${formatCliCommand("text2llm config set gateway.mode local")}`,
    ];
    if (!fs.existsSync(configPath)) {
      lines.push(`Missing config: run ${formatCliCommand("text2llm setup")} first.`);
    }
    note(lines.join("\n"), "Gateway");
  }
  ...
  if (shouldWriteConfig) {
    ...
    runtime.log(`Run "${formatCliCommand("text2llm doctor --fix")}" to apply changes.`);
  }
