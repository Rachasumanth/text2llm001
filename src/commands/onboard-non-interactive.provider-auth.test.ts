...
function captureEnv(): EnvSnapshot {
  return {
    ...
    password: process.env.TEXT2LLM_GATEWAY_PASSWORD,
    customApiKey: process.env.CUSTOM_API_KEY,
    disableConfigCache: process.env.TEXT2LLM_DISABLE_CONFIG_CACHE,
  };
}
...
function restoreEnv(prev: EnvSnapshot): void {
  ...
  restoreEnvVar("TEXT2LLM_STATE_DIR", prev.stateDir);
  restoreEnvVar("TEXT2LLM_CONFIG_PATH", prev.configPath);
  restoreEnvVar("TEXT2LLM_SKIP_CHANNELS", prev.skipChannels);
  restoreEnvVar("TEXT2LLM_SKIP_GMAIL_WATCHER", prev.skipGmail);
  restoreEnvVar("TEXT2LLM_SKIP_CRON", prev.skipCron);
  restoreEnvVar("TEXT2LLM_SKIP_CANVAS_HOST", prev.skipCanvas);
  restoreEnvVar("TEXT2LLM_GATEWAY_TOKEN", prev.token);
  restoreEnvVar("TEXT2LLM_GATEWAY_PASSWORD", prev.password);
  restoreEnvVar("CUSTOM_API_KEY", prev.customApiKey);
  restoreEnvVar("TEXT2LLM_DISABLE_CONFIG_CACHE", prev.disableConfigCache);
}
...
async function withOnboardEnv(
  ...
  const configPath = path.join(tempHome, "text2llm.json");
  process.env.HOME = tempHome;
  process.env.TEXT2LLM_STATE_DIR = tempHome;
...
describe("onboard (non-interactive): provider auth", () => {
  it("stores Z.AI API key and uses global baseUrl by default", async () => {
    await withOnboardEnv("text2llm-onboard-zai-", async ({ configPath, runtime }) => {
  ...
  it("supports Z.AI CN coding endpoint auth choice", async () => {
    await withOnboardEnv("text2llm-onboard-zai-cn-", async ({ configPath, runtime }) => {
      await runNonInteractive(
        {
          nonInteractive: true,
  ...
  it("stores xAI API key and sets default model", async () => {
    await withOnboardEnv("text2llm-onboard-xai-", async ({ configPath, runtime }) => {
  ...
  it("stores Vercel AI Gateway API key and sets default model", async () => {
    await withOnboardEnv("text2llm-onboard-ai-gateway-", async ({ configPath, runtime }) => {
      await runNonInteractive(
        {
          nonInteractive: true,
  ...
  it("stores token auth profile", async () => {
    await withOnboardEnv("text2llm-onboard-token-", async ({ configPath, runtime }) => {
  ...
  it("stores OpenAI API key and sets OpenAI default model", async () => {
    await withOnboardEnv("text2llm-onboard-openai-", async ({ configPath, runtime }) => {
      await runNonInteractive(
        {
          nonInteractive: true,
  ...
  it("stores Cloudflare AI Gateway API key and metadata", async () => {
    await withOnboardEnv("text2llm-onboard-cf-gateway-", async ({ configPath, runtime }) => {
      await runNonInteractive(
        {
          nonInteractive: true,
  ...
  it("infers Cloudflare auth choice from API key flags", async () => {
    await withOnboardEnv("text2llm-onboard-cf-gateway-infer-", async ({ configPath, runtime }) => {
      await runNonInteractive(
        {
          nonInteractive: true,
          cloudflareAiGatewayAccountId: "cf-account-id",
  ...
  it("configures a custom provider from non-interactive flags", async () => {
    await withOnboardEnv("text2llm-onboard-custom-provider-", async ({ configPath, runtime }) => {
      await runNonInteractive(
        {
          nonInteractive: true,
  ...
  it("infers custom provider auth choice from custom flags", async () => {
    await withOnboardEnv(
      "text2llm-onboard-custom-provider-infer-",
  ...
  it("uses CUSTOM_API_KEY env fallback for non-interactive custom provider auth", async () => {
    await withOnboardEnv(
      "text2llm-onboard-custom-provider-env-fallback-",
      async ({ configPath, runtime }) => {
  ...
  it("uses matching profile fallback for non-interactive custom provider auth", async () => {
    await withOnboardEnv(
      "text2llm-onboard-custom-provider-profile-fallback-",
      async ({ configPath, runtime }) => {
  ...
  it("fails custom provider auth when compatibility is invalid", async () => {
    await withOnboardEnv(
      "text2llm-onboard-custom-provider-invalid-compat-",
      async ({ runtime }) => {
        await expect(
          runNonInteractive(
            {
  ...
  it("fails custom provider auth when explicit provider id is invalid", async () => {
    await withOnboardEnv("text2llm-onboard-custom-provider-invalid-id-", async ({ runtime }) => {
      await expect(
        runNonInteractive(
          {
            nonInteractive: true,
  ...
  it("fails inferred custom auth when required flags are incomplete", async () => {
    await withOnboardEnv(
      "text2llm-onboard-custom-provider-missing-required-",
      async ({ runtime }) => {
        await expect(
          runNonInteractive(
            {
