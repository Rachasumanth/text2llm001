...
import { readConfigFileSnapshot, type TEXT2LLMConfig } from "../../config/config.js";
...
function applyDefaultModel(cfg: TEXT2LLMConfig, model: string): TEXT2LLMConfig {
...
export async function modelsAuthLoginCommand(opts: LoginOptions, runtime: RuntimeEnv) {
  ...
  if (providers.length === 0) {
    throw new Error(
      `No provider plugins found. Install one via \`${formatCliCommand("text2llm plugins install")}\`.`,
    );
  }
