...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveTEXT2LLMAgentDir: vi.fn().mockReturnValue("/tmp/text2llm-agent"),
    resolveAgentDir: vi.fn().mockReturnValue("/tmp/text2llm-agent"),
    ...
    resolveAuthStorePathForDisplay: vi
      .fn()
      .mockReturnValue("/tmp/text2llm-agent/auth-profiles.json"),
...
vi.mock("../../agents/agent-paths.js", () => ({
  resolveTEXT2LLMAgentDir: mocks.resolveTEXT2LLMAgentDir,
}));
...
describe("modelsStatusCommand auth overview", () => {
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(mocks.resolveTEXT2LLMAgentDir).toHaveBeenCalled();
    expect(payload.defaultModel).toBe("anthropic/claude-opus-4-5");
    expect(payload.auth.storePath).toBe("/tmp/text2llm-agent/auth-profiles.json");
  ...
  it("uses agent overrides and reports sources", async () => {
    ...
    mocks.resolveAgentDir.mockReturnValue("/tmp/text2llm-agent-custom");
    ...
    try {
      ...
      expect(payload.agentDir).toBe("/tmp/text2llm-agent-custom");
