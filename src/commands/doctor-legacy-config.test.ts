...
describe("normalizeLegacyConfigValues", () => {
  ...
  beforeEach(() => {
    previousOauthDir = process.env.TEXT2LLM_OAUTH_DIR;
    tempOauthDir = fs.mkdtempSync(path.join(os.tmpdir(), "text2llm-oauth-"));
    process.env.TEXT2LLM_OAUTH_DIR = tempOauthDir;
  });
  ...
  afterEach(() => {
    if (previousOauthDir === undefined) {
      ...
      process.env.TEXT2LLM_OAUTH_DIR = previousOauthDir;
    }
    if (tempOauthDir) {
  ...
  it("copies legacy ack reaction when authDir override exists", () => {
    const customDir = fs.mkdtempSync(path.join(os.tmpdir(), "text2llm-wa-auth-"));
    try {
