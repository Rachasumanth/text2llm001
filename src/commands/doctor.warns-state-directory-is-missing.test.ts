...
describe("doctor command", () => {
  it("warns when the state directory is missing", async () => {
    ...
    const missingDir = fs.mkdtempSync(path.join(os.tmpdir(), "text2llm-missing-state-"));
    fs.rmSync(missingDir, { recursive: true, force: true });
    process.env.TEXT2LLM_STATE_DIR = missingDir;
    note.mockClear();
  ...
  it("skips gateway auth warning when TEXT2LLM_GATEWAY_TOKEN is set", async () => {
    readConfigFileSnapshot.mockResolvedValue({
      path: "/tmp/text2llm.json",
      exists: true,
      raw: "{}",
    ...
    process.env.TEXT2LLM_GATEWAY_TOKEN = "env-token-1234567890";
    note.mockClear();
