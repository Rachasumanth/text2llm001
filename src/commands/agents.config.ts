...
export function listAgentEntries(cfg: TEXT2LLMConfig): AgentEntry[] {
...
function resolveAgentName(cfg: TEXT2LLMConfig, agentId: string) {
...
function resolveAgentModel(cfg: TEXT2LLMConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: TEXT2LLMConfig): AgentSummary[] {
