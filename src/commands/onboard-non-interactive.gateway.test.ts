...
describe("onboard (non-interactive): gateway and remote auth", () => {
  ...
  beforeAll(async () => {
    ...
    tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "text2llm-onboard-"));
  ...
  afterAll(async () => {
    ...
    process.env.TEXT2LLM_GATEWAY_PASSWORD = prev.password;
  });
  ...
  it("writes gateway token auth into config and gateway enforces it", async () => {
    ...
    const workspace = path.join(stateDir, "text2llm");
  ...
  it("auto-generates token auth when binding LAN and persists the token", async () => {
    ...
    process.env.TEXT2LLM_STATE_DIR = stateDir;
    process.env.TEXT2LLM_CONFIG_PATH = path.join(stateDir, "text2llm.json");
