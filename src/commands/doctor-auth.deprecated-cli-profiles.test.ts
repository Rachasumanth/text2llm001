...
beforeEach(() => {
  originalAgentDir = process.env.TEXT2LLM_AGENT_DIR;
  originalPiAgentDir = process.env.PI_CODING_AGENT_DIR;
  tempAgentDir = fs.mkdtempSync(path.join(os.tmpdir(), "text2llm-auth-"));
...
afterEach(() => {
  if (originalAgentDir === undefined) {
    ...
    process.env.TEXT2LLM_AGENT_DIR = originalAgentDir;
  }
