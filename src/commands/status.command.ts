...
export async function statusCommand(
  ...
  runtime.log(theme.heading("text2llm status"));
  runtime.log("");
  ...
  runtime.log(theme.muted(`Full report: ${formatCliCommand("text2llm security audit")}`));
  runtime.log(theme.muted(`Deep probe: ${formatCliCommand("text2llm security audit --deep")}`));
  ...
  runtime.log("");
  runtime.log("FAQ: https://docs.text2llm.ai/faq");
  runtime.log("Troubleshooting: https://docs.text2llm.ai/troubleshooting");
  runtime.log("");
  ...
  runtime.log(`  Need to share?      ${formatCliCommand("text2llm status --all")}`);
  runtime.log(`  Need to debug live? ${formatCliCommand("text2llm logs --follow")}`);
  if (gatewayReachable) {
    runtime.log(`  Need to test channels? ${formatCliCommand("text2llm status --deep")}`);
  } else {
    runtime.log(`  Fix reachability first: ${formatCliCommand("text2llm gateway probe")}`);
  }
}
