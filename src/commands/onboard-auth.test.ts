...
const requireAgentDir = () => {
  const agentDir = process.env.TEXT2LLM_AGENT_DIR;
  if (!agentDir) {
    throw new Error("TEXT2LLM_AGENT_DIR not set");
  }
  return agentDir;
};
...
describe("writeOAuthCredentials", () => {
  ...
  afterEach(async () => {
    ...
    delete process.env.TEXT2LLM_OAUTH_DIR;
  });

  it("writes auth-profiles.json under TEXT2LLM_AGENT_DIR when set", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "text2llm-oauth-"));
    process.env.TEXT2LLM_STATE_DIR = tempStateDir;
    process.env.TEXT2LLM_AGENT_DIR = path.join(tempStateDir, "agent");
    process.env.PI_CODING_AGENT_DIR = process.env.TEXT2LLM_AGENT_DIR;
...
describe("setMinimaxApiKey", () => {
  ...
  it("writes to TEXT2LLM_AGENT_DIR when set", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "text2llm-minimax-"));
    process.env.TEXT2LLM_STATE_DIR = tempStateDir;
    process.env.TEXT2LLM_AGENT_DIR = path.join(tempStateDir, "custom-agent");
    process.env.PI_CODING_AGENT_DIR = process.env.TEXT2LLM_AGENT_DIR;
