  ...
  return {
    ...
    loadConfig: () => ({
      browser: {
        ...
        headless: true,
        defaultProfile: "text2llm",
        profiles: {
          text2llm: { cdpPort: testPort + 1, color: "#FF4500" },
        },
      },
    }),
...
vi.mock("./chrome.js", () => ({
  ...
  launchTEXT2LLMChrome: vi.fn(async (_resolved: unknown, profile: { cdpPort: number }) => {
    launchCalls.push({ port: profile.cdpPort });
    reachable = true;
    return {
      pid: 123,
      exe: { kind: "chrome", path: "/fake/chrome" },
      userDataDir: "/tmp/text2llm",
      cdpPort: profile.cdpPort,
      startedAt: Date.now(),
      proc,
    };
  }),
  resolveTEXT2LLMUserDataDir: vi.fn(() => "/tmp/text2llm"),
  stopTEXT2LLMChrome: vi.fn(async () => {
    reachable = false;
  }),
}));
...
describe("browser control server", () => {
  beforeEach(async () => {
    ...
    prevGatewayPort = process.env.TEXT2LLM_GATEWAY_PORT;
    process.env.TEXT2LLM_GATEWAY_PORT = String(testPort - 2);
  ...
  afterEach(async () => {
    ...
    if (prevGatewayPort === undefined) {
      ...
      process.env.TEXT2LLM_GATEWAY_PORT = prevGatewayPort;
    }
...
describe("backward compatibility (profile parameter)", () => {
  ...
  it("GET / without profile uses default profile", async () => {
    ...
    // Should use default profile (text2llm)
    expect(status.profile).toBe("text2llm");
  });
  ...
  it("POST /start without profile uses default profile", async () => {
    ...
    expect(result.profile).toBe("text2llm");
  });
  ...
  it("GET /profiles returns list of profiles", async () => {
    ...
    // Should at least have the default text2llm profile
    expect(result.profiles.some((p) => p.name === "text2llm")).toBe(true);
  });

  it("GET /tabs?profile=text2llm returns tabs for specified profile", async () => {
    const { startBrowserControlServerFromConfig } = await import("./server.js");
    await startBrowserControlServerFromConfig();
    const base = `http://127.0.0.1:${testPort}`;
    ...
    const result = (await realFetch(`${base}/tabs?profile=text2llm`).then((r) => r.json())) as {
      running: boolean;
      tabs: unknown[];
    };
    expect(result.running).toBe(true);
  ...
  it("POST /tabs/open?profile=text2llm opens tab in specified profile", async () => {
    const { startBrowserControlServerFromConfig } = await import("./server.js");
    await startBrowserControlServerFromConfig();
    const base = `http://127.0.0.1:${testPort}`;
    ...
    const result = (await realFetch(`${base}/tabs/open?profile=text2llm`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ url: "https://example.com" }),
    }).then((r) => r.json())) as { targetId?: string };
