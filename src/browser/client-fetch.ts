...
function withLoopbackBrowserAuth(
  ...
  if (headers.has("authorization") || headers.has("x-text2llm-password")) {
    return { ...init, headers };
  }
  ...
  try {
    ...
    if (auth.token) {
      ...
      headers.set("x-text2llm-password", auth.password);
    }
  } catch {
...
function enhanceBrowserFetchError(url: string, err: unknown, timeoutMs: number): Error {
  const hint = isAbsoluteHttp(url)
    ...
    : `Start (or restart) the text2llm gateway (text2llm.app menubar, or \`${formatCliCommand("text2llm gateway")}\`) and try again.`;
  ...
  if (looksLikeTimeout) {
    return new Error(
      `Can't reach the text2llm browser control service (timed out after ${timeoutMs}ms). ${hint}`,
    );
  }
  return new Error(`Can't reach the text2llm browser control service. ${hint} (${msg})`);
}
