...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".text2llm-profile-decorated");
}
 ...
 */
export function decorateTEXT2LLMProfile(
  userDataDir: string,
  opts?: { name?: string; color?: string },
) {
  const desiredName = opts?.name ?? DEFAULT_TEXT2LLM_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_TEXT2LLM_BROWSER_COLOR).toUpperCase();
