...
describe("browser config", () => {
  it("defaults to enabled with loopback defaults and lobster-orange color", () => {
    ...
    const text2llm = resolveProfile(resolved, "text2llm");
    expect(text2llm?.driver).toBe("text2llm");
    expect(text2llm?.cdpPort).toBe(18800);
    expect(text2llm?.cdpUrl).toBe("http://127.0.0.1:18800");
    expect(resolved.remoteCdpTimeoutMs).toBe(1500);
    expect(resolved.remoteCdpHandshakeTimeoutMs).toBe(3000);
  });

  it("derives default ports from TEXT2LLM_GATEWAY_PORT when unset", () => {
    const prev = process.env.TEXT2LLM_GATEWAY_PORT;
    process.env.TEXT2LLM_GATEWAY_PORT = "19001";
    try {
      const resolved = resolveBrowserConfig(undefined);
      ...
      const text2llm = resolveProfile(resolved, "text2llm");
      expect(text2llm?.cdpPort).toBe(19012);
      expect(text2llm?.cdpUrl).toBe("http://127.0.0.1:19012");
    } finally {
      if (prev === undefined) {
        delete process.env.TEXT2LLM_GATEWAY_PORT;
      } else {
        process.env.TEXT2LLM_GATEWAY_PORT = prev;
      }
    }
  });
  ...
  it("derives default ports from gateway.port when env is unset", () => {
    ...
    try {
      ...
      const text2llm = resolveProfile(resolved, "text2llm");
      expect(text2llm?.cdpPort).toBe(19022);
      expect(text2llm?.cdpUrl).toBe("http://127.0.0.1:19022");
    } finally {
      if (prev === undefined) {
        delete process.env.TEXT2LLM_GATEWAY_PORT;
      } else {
  ...
  it("treats non-loopback cdpUrl as remote", () => {
    ...
    const profile = resolveProfile(resolved, "text2llm");
  ...
  it("does not add the built-in chrome extension profile if the derived relay port is already used", () => {
    const resolved = resolveBrowserConfig({
      profiles: {
        text2llm: { cdpPort: 18792, color: "#FF4500" },
      },
    });
    expect(resolveProfile(resolved, "chrome")).toBe(null);
    expect(resolved.defaultProfile).toBe("text2llm");
  });
});
