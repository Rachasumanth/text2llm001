...
const RELAY_AUTH_HEADER = "x-text2llm-relay-token";
  ...
  const routeCdpCommand = async (cmd: CdpCommand): Promise<unknown> => {
    switch (cmd.method) {
      case "Browser.getVersion":
        return {
          protocolVersion: "1.3",
          product: "Chrome/text2llm-Extension-Relay",
          revision: "0",
          userAgent: "text2llm-Extension-Relay",
          jsVersion: "V8",
        };
  ...
  const server = createServer((req, res) => {
    ...
    if (
      ...
      const payload: Record<string, unknown> = {
        Browser: "text2llm/extension-relay",
