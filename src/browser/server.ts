...
function isAuthorizedBrowserRequest(
  ...
  if (auth.password) {
    const passwordHeader = firstHeaderValue(req.headers["x-text2llm-password"]).trim();
...
export async function startBrowserControlServerFromConfig(): Promise<BrowserServerState | null> {
  ...
  const server = await new Promise<Server>((resolve, reject) => {
    ...
    logServer.error(`text2llm browser server failed to bind 127.0.0.1:${port}: ${String(err)}`);
    return null;
  });
...
export async function stopBrowserControlServer(): Promise<void> {
  ...
  try {
    ...
    logServer.warn(`text2llm browser stop failed: ${String(err)}`);
  }
