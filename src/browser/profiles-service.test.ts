...
vi.mock("./chrome.js", () => ({
  resolveTEXT2LLMUserDataDir: vi.fn(() => "/tmp/text2llm-test/text2llm/user-data"),
}));

import { loadConfig, writeConfigFile } from "../config/config.js";
import { resolveTEXT2LLMUserDataDir } from "./chrome.js";
...
describe("BrowserProfilesService", () => {
  ...
  it("deletes local profiles and moves data to Trash", async () => {
    ...
    const tempDir = fs.mkdtempSync(path.join("/tmp", "text2llm-profile-"));
    const userDataDir = path.join(tempDir, "work", "user-data");
    fs.mkdirSync(path.dirname(userDataDir), { recursive: true });
    vi.mocked(resolveTEXT2LLMUserDataDir).mockReturnValue(userDataDir);
