...
function ensureExperimentalWarningSuppressed(): boolean {
  if (isTruthyEnvValue(process.env.TEXT2LLM_NO_RESPAWN)) {
    return false;
  }
  if (isTruthyEnvValue(process.env.TEXT2LLM_NODE_OPTIONS_READY)) {
    return false;
  }
  ...
  process.env.TEXT2LLM_NODE_OPTIONS_READY = "1";
  ...
  child.once("error", (error) => {
    console.error(
      "[text2llm] Failed to respawn CLI:",
...
if (!ensureExperimentalWarningSuppressed()) {
  ...
  if (!parsed.ok) {
    ...
    console.error(`[text2llm] ${parsed.error}`);
    process.exit(2);
  }
