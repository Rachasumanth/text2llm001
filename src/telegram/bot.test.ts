...
const { sessionStorePath } = vi.hoisted(() => ({
  sessionStorePath: `/tmp/text2llm-telegram-bot-${Math.random().toString(16).slice(2)}.json`,
}));
...
describe("createTelegramBot", () => {
  ...
  it("requests pairing by default for unknown DM senders", async () => {
    ...
    expect(pairingText).toContain("text2llm pairing approve telegram PAIRME12");
    ...
    await handler({
      message: {
        ...
        reply_to_message: {
          message_id: 42,
          text: "original reply",
          from: { id: 999, first_name: "text2llm" },
        },
      },
      me: { id: 999, username: "TEXT2LLM_bot" },
      getFile: async () => ({ download: async () => new Uint8Array() }),
    });
  ...
  it("honors routed group activation from session store", async () => {
    ...
    const replySpy = replyModule.__replySpy as unknown as ReturnType<typeof vi.fn>;
    replySpy.mockReset();
    const storeDir = fs.mkdtempSync(path.join(os.tmpdir(), "text2llm-telegram-"));
