...
describe("canvas host", () => {
  it("injects live reload script", () => {
    ...
    expect(out).toContain("TEXT2LLMCanvasA2UIAction");
    expect(out).toContain("TEXT2LLMSendUserAction");
  });

  it("creates a default index.html when missing", async () => {
    const dir = await fs.mkdtemp(path.join(os.tmpdir(), "text2llm-canvas-"));
    ...
    try {
      ...
      expect(html).toContain("TEXT2LLMSendUserAction");
  ...
  it("serves the gateway-hosted A2UI scaffold", async () => {
    ...
    try {
      ...
      await fs.writeFile(bundlePath, "window.TEXT2LLMA2UI = {};", "utf8");
      createdBundle = true;
    }
    ...
    try {
      const res = await fetch(`http://127.0.0.1:${server.port}/__TEXT2LLM__/a2ui/`);
      const html = await res.text();
      expect(res.status).toBe(200);
      expect(html).toContain("text2llm-a2ui-host");
      expect(html).toContain("TEXT2LLMCanvasA2UIAction");

      const bundleRes = await fetch(
        `http://127.0.0.1:${server.port}/__TEXT2LLM__/a2ui/a2ui.bundle.js`,
      );
      const js = await bundleRes.text();
      expect(bundleRes.status).toBe(200);
      expect(js).toContain("TEXT2LLMA2UI");
    } finally {
      await server.close();
      if (createdBundle) {
