...
(() => {
  ...
  const hasIOS = () =>
    !!(
      ...
      window.webkit.messageHandlers.TEXT2LLMCanvasA2UIAction
    );
  const hasAndroid = () =>
    !!(
      (window.TEXT2LLMCanvasA2UIAction &&
        typeof window.TEXT2LLMCanvasA2UIAction.postMessage === "function")
    );
  const hasHelper = () => typeof window.TEXT2LLMSendUserAction === "function";
  statusEl.innerHTML =
    "Bridge: " +
  ...
  window.addEventListener("text2llm:a2ui-action-status", onStatus);

  function send(name, sourceComponentId) {
    if (!hasHelper()) {
      log("No action bridge found. Ensure you're viewing this on an iOS/Android text2llm node canvas.");
      return;
    }
    const sendUserAction =
      typeof window.TEXT2LLMSendUserAction === "function"
        ? window.TEXT2LLMSendUserAction
        : undefined;
...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.TEXT2LLM_SKIP_CANVAS_HOST)) {
    return true;
  }
  if (isTruthyEnvValue(process.env.TEXT2LLM_SKIP_CANVAS_HOST)) {
    return true;
    ...
    try {
      ...
      if (!opened) {
        if (urlPath === "/" || urlPath.endsWith("/")) {
          ...
          res.end(
            `<!doctype html><meta charset="utf-8" /><title>text2llm Canvas</title><pre>Missing file.\nCreate ${rootDir}/index.html</pre>`,
          );
          return true;
        }
        res.statusCode = 404;
