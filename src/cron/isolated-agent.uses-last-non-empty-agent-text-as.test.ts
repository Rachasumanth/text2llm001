...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeBase(fn, { prefix: "text2llm-cron-" });
}
...
async function writeSessionStore(
  ...
  const dir = path.join(home, ".text2llm", "sessions");
...
function makeCfg(
  home: string,
  storePath: string,
  overrides: Partial<TEXT2LLMConfig> = {},
): TEXT2LLMConfig {
  const base: TEXT2LLMConfig = {
    agents: {
      defaults: {
...
describe("runCronIsolatedAgentTurn", () => {
  ...
  it("uses agentId for workspace, session key, and store paths", async () => {
    await withTempHome(async (home) => {
      ...
      const cfg = makeCfg(
        home,
        path.join(home, ".text2llm", "agents", "{agentId}", "sessions", "sessions.json"),
        {
          agents: {
