...
export function renderRuntimeHints(
  ...
  if (runtime.missingUnit) {
    hints.push(`Service not installed. Run: ${formatCliCommand("text2llm gateway install", env)}`);
    if (fileLog) {
  ...
  if (runtime.status === "stopped") {
    ...
    if (process.platform === "darwin") {
      ...
      const task = resolveGatewayWindowsTaskName(env.TEXT2LLM_PROFILE);
...
export function renderGatewayServiceStartHints(env: NodeJS.ProcessEnv = process.env): string[] {
  const base = [
    formatCliCommand("text2llm gateway install", env),
    formatCliCommand("text2llm gateway", env),
  ];
