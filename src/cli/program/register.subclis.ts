...
const shouldRegisterPrimaryOnly = (argv: string[]) => {
  if (isTruthyEnvValue(process.env.TEXT2LLM_DISABLE_LAZY_SUBCOMMANDS)) {
    return false;
  }
...
const shouldEagerRegisterSubcommands = (_argv: string[]) => {
  return isTruthyEnvValue(process.env.TEXT2LLM_DISABLE_LAZY_SUBCOMMANDS);
};

const loadConfig = async (): Promise<TEXT2LLMConfig> => {
