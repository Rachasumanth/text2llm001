...
describe("registerSubCliCommands", () => {
  ...
  beforeEach(() => {
    ...
    delete process.env.TEXT2LLM_DISABLE_LAZY_SUBCOMMANDS;
  ...
  it("registers only the primary placeholder and dispatches", async () => {
    process.argv = ["node", "text2llm", "acp"];
  ...
  it("re-parses argv for lazy subcommands", async () => {
    process.argv = ["node", "text2llm", "nodes", "list"];
    const program = new Command();
    program.name("text2llm");
    registerSubCliCommands(program, process.argv);
  ...
  it("replaces placeholder when registering a subcommand by name", async () => {
    process.argv = ["node", "text2llm", "acp", "--help"];
    const program = new Command();
    program.name("text2llm");
    registerSubCliCommands(program, process.argv);
    ...
    await program.parseAsync(["node", "text2llm", "acp"], { from: "user" });
    expect(registerAcpCli).toHaveBeenCalledTimes(1);
    expect(acpAction).toHaveBeenCalledTimes(1);
  });
});
