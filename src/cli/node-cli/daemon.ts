...
function renderNodeServiceStartHints(): string[] {
  const base = [formatCliCommand("text2llm node install"), formatCliCommand("text2llm node start")];
...
export async function runNodeDaemonInstall(opts: NodeDaemonInstallOptions) {
  ...
  if (loaded && !opts.force) {
    ...
    if (!json) {
      ...
      defaultRuntime.log(`Reinstall with: ${formatCliCommand("text2llm node install --force")}`);
    }
    return;
  }
...
export async function runNodeDaemonStatus(opts: NodeDaemonStatusOptions = {}) {
  ...
  const hintEnv = {
    ...baseEnv,
    TEXT2LLM_LOG_PREFIX: baseEnv.TEXT2LLM_LOG_PREFIX ?? "node",
