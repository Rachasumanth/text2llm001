...
/**
 * Test for issue #6070:
 * `text2llm config set` should use snapshot.parsed (raw user config) instead of
...
async function withTempHome(run: (home: string) => Promise<void>): Promise<void> {
  const home = await fs.mkdtemp(path.join(os.tmpdir(), "text2llm-config-cli-"));
  const originalEnv = { ...process.env };
  try {
    // Override config path to use temp directory
    process.env.TEXT2LLM_CONFIG_PATH = path.join(home, ".text2llm", "text2llm.json");
    await fs.mkdir(path.join(home, ".text2llm"), { recursive: true });
    await run(home);
  } finally {
...
async function readConfigFile(home: string): Promise<Record<string, unknown>> {
  const configPath = path.join(home, ".text2llm", "text2llm.json");
