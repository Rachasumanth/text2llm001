...
const STEP_LABELS: Record<string, string> = {
  ...
  "text2llm doctor entry": "Checking doctor entrypoint",
  "text2llm doctor": "Running doctor checks",
...
const DEFAULT_PACKAGE_NAME = "text2llm";
const CORE_PACKAGE_NAMES = new Set([DEFAULT_PACKAGE_NAME]);
const CLI_NAME = resolveCliName();
const TEXT2LLM_REPO_URL = "https://github.com/text2llm/text2llm.git";
...
function normalizeTag(value?: string | null): string | null {
  ...
  if (trimmed.startsWith("text2llm@")) {
    return trimmed.slice("text2llm@".length);
  }
...
async function tryWriteCompletionCache(root: string, jsonMode: boolean): Promise<void> {
  const binPath = path.join(root, "text2llm.mjs");
  ...
  if (!status.profileInstalled) {
    ...
    if (isCancel(shouldInstall) || !shouldInstall) {
      if (!opts.skipPrompt) {
        defaultRuntime.log(
          theme.muted(
            `Skipped. Run \`${replaceCliName(formatCliCommand("text2llm completion --install"), CLI_NAME)}\` later to enable.`,
          ),
        );
      }
      return;
...
function resolveGitInstallDir(): string {
  const override = process.env.TEXT2LLM_GIT_DIR?.trim();
  if (override) {
...
async function ensureGitCheckout(params: {
  ...
  if (!dirExists) {
    return await runUpdateStep({
      name: "git clone",
      argv: ["git", "clone", TEXT2LLM_REPO_URL, params.dir],
      timeoutMs: params.timeoutMs,
  ...
  if (!(await isGitCheckout(params.dir))) {
    ...
    if (!empty) {
      throw new Error(
        `TEXT2LLM_GIT_DIR points at a non-git directory: ${params.dir}. Set TEXT2LLM_GIT_DIR to an empty folder or an text2llm checkout.`,
      );
    }
    return await runUpdateStep({
      name: "git clone",
  ...
  if (!(await isCorePackage(params.dir))) {
    throw new Error(`TEXT2LLM_GIT_DIR does not look like a core checkout: ${params.dir}.`);
  }
...
export async function updateStatusCommand(opts: UpdateStatusOptions): Promise<void> {
  ...
  defaultRuntime.log(theme.heading("text2llm update status"));
  defaultRuntime.log("");
  defaultRuntime.log(
    renderTable({
      width: tableWidth,
...
export async function updateCommand(opts: UpdateCommandOptions): Promise<void> {
  ...
  if (!opts.json) {
    defaultRuntime.log(theme.heading("Updating text2llm..."));
    defaultRuntime.log("");
  }
  ...
  if (result.status === "skipped") {
    ...
    if (result.reason === "not-git-install") {
      defaultRuntime.log(
        theme.warn(
          `Skipped: this text2llm install isn't a git checkout, and the package manager couldn't be detected. Update via your package manager, then run \`${replaceCliName(formatCliCommand("text2llm doctor"), CLI_NAME)}\` and \`${replaceCliName(formatCliCommand("text2llm gateway restart"), CLI_NAME)}\`.`,
        ),
      );
      defaultRuntime.log(
        theme.muted(
          `Examples: \`${replaceCliName("npm i -g text2llm@latest", CLI_NAME)}\` or \`${replaceCliName("pnpm add -g text2llm@latest", CLI_NAME)}\``,
        ),
      );
    }
    defaultRuntime.exit(0);
  ...
  if (shouldRestart) {
    ...
    try {
      ...
      if (!opts.json) {
        ...
        defaultRuntime.log(
          theme.muted(
            `You may need to restart the service manually: ${replaceCliName(formatCliCommand("text2llm gateway restart"), CLI_NAME)}`,
          ),
        );
      }
    }
    ...
    if (result.mode === "npm" || result.mode === "pnpm") {
      defaultRuntime.log(
        theme.muted(
          `Tip: Run \`${replaceCliName(formatCliCommand("text2llm doctor"), CLI_NAME)}\`, then \`${replaceCliName(formatCliCommand("text2llm gateway restart"), CLI_NAME)}\` to apply updates to a running gateway.`,
        ),
      );
    } else {
      defaultRuntime.log(
        theme.muted(
          `Tip: Run \`${replaceCliName(formatCliCommand("text2llm gateway restart"), CLI_NAME)}\` to apply updates to a running gateway.`,
        ),
      );
    }
  }
...
export async function updateWizardCommand(opts: UpdateWizardOptions = {}): Promise<void> {
  if (!process.stdin.isTTY) {
    defaultRuntime.error(
      "Update wizard requires a TTY. Use `text2llm update --channel <stable|beta|dev>` instead.",
    );
    defaultRuntime.exit(1);
    return;
  }
  ...
  if (requestedChannel === "dev" && updateStatus.installKind !== "git") {
    ...
    if (!hasGit) {
      ...
      if (dirExists) {
        ...
        if (!empty) {
          defaultRuntime.error(
            `TEXT2LLM_GIT_DIR points at a non-git directory: ${gitDir}. Set TEXT2LLM_GIT_DIR to an empty folder or an text2llm checkout.`,
          );
          defaultRuntime.exit(1);
          return;
        }
      ...
      const ok = await confirm({
        message: stylePromptMessage(
          `Create a git checkout at ${gitDir}? (override via TEXT2LLM_GIT_DIR)`,
        ),
        initialValue: true,
      });
      if (isCancel(ok) || !ok) {
...
export function registerUpdateCli(program: Command) {
  const update = program
    .command("update")
    .description("Update text2llm to the latest version")
    ...
    .addHelpText("after", () => {
      const examples = [
        ["text2llm update", "Update a source checkout (git)"],
        ["text2llm update --channel beta", "Switch to beta channel (git + npm)"],
        ["text2llm update --channel dev", "Switch to dev channel (git + npm)"],
        ["text2llm update --tag beta", "One-off update to a dist-tag or version"],
        ["text2llm update --no-restart", "Update without restarting the service"],
        ["text2llm update --json", "Output result as JSON"],
        ["text2llm update --yes", "Non-interactive (accept downgrade prompts)"],
        ["text2llm update wizard", "Interactive update wizard"],
        ["text2llm --update", "Shorthand for text2llm update"],
      ] as const;
...
${theme.heading("Switch channels:")}
  ...
  - Run text2llm update status to see the active channel and source
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "docs.text2llm.ai/cli/update")}`;
    })
    .action(async (opts) => {
      try {
        await updateCommand({
  ...
  update
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["text2llm update status", "Show channel + version status."],
          ["text2llm update status --json", "JSON output."],
          ["text2llm update status --timeout 10", "Custom timeout."],
        ])}\n\n${theme.heading("Notes:")}\n${theme.muted(
          "- Shows current update channel (stable/beta/dev) and source",
        )}\n${theme.muted("- Includes git tag/branch/SHA for source checkouts")}\n\n${theme.muted(
          "Docs:",
        )} ${formatDocsLink("/cli/update", "docs.text2llm.ai/cli/update")}`,
    )
    .action(async (opts) => {
      try {
