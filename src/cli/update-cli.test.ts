...
vi.mock("../infra/text2llm-root.js", () => ({
  resolveTEXT2LLMPackageRoot: vi.fn(),
}));
...
describe("update-cli", () => {
  ...
  beforeEach(async () => {
    vi.clearAllMocks();
    const { resolveTEXT2LLMPackageRoot } = await import("../infra/text2llm-root.js");
    const { readConfigFileSnapshot } = await import("../config/config.js");
    const { checkUpdateStatus, fetchNpmTagVersion, resolveNpmChannelTag } =
      await import("../infra/update-check.js");
    const { runCommandWithTimeout } = await import("../process/exec.js");
    vi.mocked(resolveTEXT2LLMPackageRoot).mockResolvedValue(process.cwd());
  ...
  it("updateStatusCommand prints table output", async () => {
    ...
    expect(logs.join("\n")).toContain("text2llm update status");
  });
  ...
  it("defaults to stable channel for package installs when unset", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "text2llm-update-"));
    try {
      await fs.writeFile(
      ...
      vi.mocked(resolveTEXT2LLMPackageRoot).mockResolvedValue(tempDir);
      vi.mocked(checkUpdateStatus).mockResolvedValue({
        root: tempDir,
        installKind: "package",
        packageManager: "npm",
  ...
  it("updateWizardCommand offers dev checkout and forwards selections", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "text2llm-update-wizard-"));
    const previousGitDir = process.env.TEXT2LLM_GIT_DIR;
    try {
      setTty(true);
      process.env.TEXT2LLM_GIT_DIR = tempDir;
      ...
      process.env.TEXT2LLM_GIT_DIR = previousGitDir;
      await fs.rm(tempDir, { recursive: true, force: true });
    }
  });
});
