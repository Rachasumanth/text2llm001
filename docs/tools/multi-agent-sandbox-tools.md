...
For debugging “why is this blocked?”, see [Sandbox vs Tool Policy vs Elevated](/gateway/sandbox-vs-tool-policy-vs-elevated) and `text2llm sandbox explain`.
...
Legacy `agent.*` configs are migrated by `text2llm doctor`; prefer `agents.defaults` + `agents.list` going forward.
...
2. **Verify sandbox containers:**
   ...
   ```exec
   docker ps --filter "name=text2llm-sbx-"
   ```
...
4. **Monitor logs:**
   ...
   ```exec
   tail -f "${TEXT2LLM_STATE_DIR:-$HOME/.text2llm}/logs/gateway.log" | grep -E "routing|sandbox|tools"
   ```
