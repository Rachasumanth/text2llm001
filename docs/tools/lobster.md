---
title: Lobster
summary: "Typed workflow runtime for text2llm with resumable approval gates."
description: Typed workflow runtime for text2llm — composable pipelines with approval gates.
read_when:
...
Lobster is a workflow shell that lets text2llm run multi-step tool sequences as a single, deterministic operation with explicit approval checkpoints.
...
- **One call instead of many**: text2llm runs one Lobster tool call and gets a structured result.
...
text2llm launches the local `lobster` CLI in **tool mode** and parses a JSON envelope from stdout.
...
gog.gmail.search --query 'newer_than:1d' \
  | text2llm.invoke --tool message --action send --each --item-key message --args-json '{"provider":"telegram","to":"..."}'
```
...
```lobster
text2llm.invoke --tool llm-task --action json --args-json '{
...
Lobster can run YAML/JSON workflow files with `name`, `args`, `steps`, `env`, `condition`, and `approval` fields. In text2llm tool calls, set `pipeline` to the file path.
...
Install the Lobster CLI on the **same host** that runs the text2llm Gateway (see the [Lobster repo](https://github.com/text2llm/lobster)), and ensure `lobster` is on `PATH`.
...
plugin tools (like `lobster`), text2llm keeps core tools enabled. To restrict core
...
→ text2llm calls gmail.list
→ LLM summarizes
→ User: "draft replies to #2 and #5"
→ LLM drafts
→ User: "send #2"
→ text2llm calls gmail.send
...
- **No secrets** — Lobster doesn't manage OAuth; it calls text2llm tools that do.
