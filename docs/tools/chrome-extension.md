---
summary: "Chrome extension: let text2llm drive your existing Chrome tab"
read_when:
...
The text2llm Chrome extension lets the agent control your **existing Chrome tabs** (your normal Chrome window) instead of launching a separate text2llm-managed Chrome profile.
...
text2llm then controls the attached tab through the normal `browser` tool surface (selecting the right profile).
...
The extension ships inside the text2llm release (npm package) as static files. There is no separate “build” step.

After upgrading text2llm:

- Re-run `text2llm browser extension install` to refresh the installed files under your text2llm state directory.
- Chrome → `chrome://extensions` → click “Reload” on the extension.

## Use it (no extra config)

text2llm ships with a built-in browser profile named `chrome` that targets the extension relay on the default port.
...
- Open the tab you want text2llm to control.
...
- `ON`: attached; text2llm can drive that tab.
...
Debugging: `text2llm sandbox explain`
...
`text2llm browser extension path` prints the **installed** on-disk directory containing the extension files.

The CLI intentionally does **not** print a `node_modules` path. Always run `text2llm browser extension install` first to copy the extension to a stable location under your text2llm state directory.
...
- **This is not isolated** like the dedicated text2llm-managed profile.
