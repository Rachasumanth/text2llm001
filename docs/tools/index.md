---
summary: "Agent tool surface for text2llm (browser, canvas, nodes, message, cron) replacing legacy `text2llm-*` skills"
read_when:
  - Adding or modifying agent tools
  - Retiring or changing `text2llm-*` skills
title: "Tools"
---

# Tools (text2llm)

text2llm exposes **first-class agent tools** for browser, canvas, nodes, and cron.
These replace the old `text2llm-*` skills: the tools are typed, no shelling,
and the agent should rely on them directly.

## Disabling tools

You can globally allow/deny tools via `tools.allow` / `tools.deny` in `text2llm.json`
...
- If `tools.allow` only references unknown or unloaded plugin tool names, text2llm logs a warning and ignores the allowlist so core tools stay available.
...
- `group:nodes`: `nodes`
- `group:text2llm`: all built-in text2llm tools (excludes provider plugins)
...
- `host=node` can target a macOS companion app or a headless node host (`text2llm node run`).
...
- Requires a Brave API key (recommended: `text2llm configure --section web`, or set `BRAVE_API_KEY`).
...
Control the dedicated text2llm-managed browser.
...
- Quick smoke: `text2llm nodes canvas a2ui push --node <id> --text "Hello from A2UI"`.
...
- `restart` (authorizes + sends `SIGUSR1` for in-process restart; `text2llm gateway` restart in-place)
