...
- CLI 辅助工具：`text2llm onboard`、`text2llm models list`、`text2llm models set <provider/model>`。

## 内置提供商（pi-ai 目录）

text2llm 附带 pi-ai 目录。这些提供商**不需要** `models.providers` 配置；只需设置认证 + 选择模型。
...
- CLI：`text2llm onboard --auth-choice token`（粘贴 setup-token）或 `text2llm models auth paste-token --provider anthropic`
...
- CLI：`text2llm onboard --auth-choice openai-codex` 或 `text2llm models auth login --provider openai-codex`
...
- Antigravity OAuth 作为捆绑插件提供（`google-antigravity-auth`，默认禁用）。
  - 启用：`text2llm plugins enable google-antigravity-auth`
  - 登录：`text2llm models auth login --provider google-antigravity --set-default`
- Gemini CLI OAuth 作为捆绑插件提供（`google-gemini-cli-auth`，默认禁用）。
  - 启用：`text2llm plugins enable google-gemini-cli-auth`
  - 登录：`text2llm models auth login --provider google-gemini-cli --set-default`
  - 注意：你**不需要**将客户端 ID 或密钥粘贴到 `text2llm.json` 中。CLI 登录流程将令牌存储在 Gateway 网关主机的认证配置文件中。
...
text2llm models set opencode/claude-opus-4-5
