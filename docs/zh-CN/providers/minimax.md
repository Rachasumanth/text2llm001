---
read_when:
  - 你想在 text2llm 中使用 MiniMax 模型
  - 你需要 MiniMax 设置指南
summary: 在 text2llm 中使用 MiniMax M2.1
title: MiniMax
x-i18n:
...
```bash
text2llm plugins enable minimax-portal-auth  # 如果已加载则跳过
text2llm gateway restart  # 如果 Gateway 网关已在运行则重启
...
详情参见 [MiniMax OAuth 插件 README](https://github.com/text2llm/text2llm/tree/main/extensions/minimax-portal-auth)。
...
- 使用 `text2llm models list` 和 `text2llm models set minimax/MiniMax-M2.1` 切换模型。
...
- 运行 `text2llm configure` 并选择 **MiniMax M2.1**，或
