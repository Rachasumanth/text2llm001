...
summary: 设置指南：在保持最新的同时保持你的 text2llm 设置个性化
title: 设置
x-i18n:
...
- **个性化设置存放在仓库之外：** `~/.text2llm/workspace`（工作区）+ `~/.text2llm/text2llm.json`（配置）。
...
- **配置：** `~/.text2llm/text2llm.json`（JSON/JSON5 格式）
- **工作区：** `~/.text2llm/workspace`（Skills、提示、记忆；将其设为私有 git 仓库）
...
如果你还没有全局安装，通过 `pnpm text2llm setup` 运行它。

## 稳定工作流（macOS 应用优先）

1. 安装并启动 **text2llm.app**（菜单栏）。
...
- 运行 `text2llm setup`，然后 `text2llm channels login`，然后手动启动 Gateway 网关（`text2llm gateway`）。
...
- **配对允许列表**：`~/.text2llm/credentials/<channel>-allowFrom.json`
- **模型认证配置文件**：`~/.text2llm/agents/<agentId>/agent/auth-profiles.json`
- **旧版 OAuth 导入**：`~/.text2llm/credentials/oauth.json`
  更多详情：[安全](/gateway/security#credential-storage-map)。

## 更新（不破坏你的设置）

- 将 `~/.text2llm/workspace` 和 `~/.text2llm/` 保持为"你的东西"；不要将个人提示/配置放入 `text2llm` 仓库。
