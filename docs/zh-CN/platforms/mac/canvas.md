...
- `~/Library/Application Support/text2llm/canvas/<session>/...`

Canvas 面板通过**自定义 URL 方案**提供这些文件：

- `text2llm-canvas://<session>/<path>`

示例：

- `text2llm-canvas://main/` → `<canvasRoot>/main/index.html`
- `text2llm-canvas://main/assets/app.css` → `<canvasRoot>/main/assets/app.css`
- `text2llm-canvas://main/widgets/todo/` → `<canvasRoot>/main/widgets/todo/index.html`
...
```bash
text2llm nodes canvas present --node <id>
text2llm nodes canvas navigate --node <id> --url "/"
text2llm nodes canvas eval --node <id> --js "document.title"
text2llm nodes canvas snapshot --node <id>
```
...
```
http://<gateway-host>:18793/__text2llm__/a2ui/
```
...
text2llm nodes canvas a2ui push --jsonl /tmp/a2ui-v0.8.jsonl --node <id>
```

快速测试：

```bash
text2llm nodes canvas a2ui push --node <id> --text "Hello from A2UI"
```
...
```js
window.location.href = "text2llm://agent?message=Review%20this%20design";
```
