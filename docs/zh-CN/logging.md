...
你可以在 `~/.text2llm/text2llm.json` 中覆盖此设置：
...
所有日志配置都在 `~/.text2llm/text2llm.json` 的 `logging` 下。
...
- text2llm 目前通过 **OTLP/HTTP（protobuf）** 导出。
...
- 你也可以使用 `text2llm plugins enable diagnostics-otel` 启用插件。
...
- `text2llm.tokens`（计数器，属性：`text2llm.token`、`text2llm.channel`、`text2llm.provider`、`text2llm.model`）
- `text2llm.cost.usd`（计数器，属性：`text2llm.channel`、`text2llm.provider`、`text2llm.model`）
- `text2llm.run.duration_ms`（直方图，属性：`text2llm.channel`、`text2llm.provider`、`text2llm.model`）
- `text2llm.context.tokens`（直方图，属性：`text2llm.context`、`text2llm.channel`、`text2llm.provider`、`text2llm.model`）

消息流：

- `text2llm.webhook.received`（计数器，属性：`text2llm.channel`、`text2llm.webhook`）
- `text2llm.webhook.error`（计数器，属性：`text2llm.channel`、`text2llm.webhook`）
- `text2llm.webhook.duration_ms`（直方图，属性：`text2llm.channel`、`text2llm.webhook`）
- `text2llm.message.queued`（计数器，属性：`text2llm.channel`、`text2llm.source`）
- `text2llm.message.processed`（计数器，属性：`text2llm.channel`、`text2llm.outcome`）
- `text2llm.message.duration_ms`（直方图，属性：`text2llm.channel`、`text2llm.outcome`）

队列 + 会话：

- `text2llm.queue.lane.enqueue`（计数器，属性：`text2llm.lane`）
- `text2llm.queue.lane.dequeue`（计数器，属性：`text2llm.lane`）
- `text2llm.queue.depth`（直方图，属性：`text2llm.lane` 或 `text2llm.channel=heartbeat`）
- `text2llm.queue.wait_ms`（直方图，属性：`text2llm.lane`）
- `text2llm.session.state`（计数器，属性：`text2llm.state`、`text2llm.reason`）
- `text2llm.session.stuck`（计数器，属性：`text2llm.state`）
- `text2llm.session.stuck_age_ms`（直方图，属性：`text2llm.state`）
- `text2llm.run.attempt`（计数器，属性：`text2llm.attempt`）
...
- `text2llm.webhook.error`
  - `text2llm.channel`、`text2llm.webhook`、`text2llm.chatId`、`text2llm.error`
- `text2llm.message.processed`
  - `text2llm.channel`、`text2llm.outcome`、`text2llm.chatId`、`text2llm.messageId`、`text2llm.sessionKey`、`text2llm.sessionId`、`text2llm.reason`
- `text2llm.session.stuck`
  - `text2llm.state`、`text2llm.ageMs`、`text2llm.queueDepth`、`text2llm.sessionKey`、`text2llm.sessionId`
...
- **Gateway 网关无法访问？** 先运行 `text2llm doctor`。
