...
LINE 通过 LINE Messaging API 连接到 text2llm。该插件作为 webhook 接收器在 Gateway 网关上运行，使用你的 channel access token + channel secret 进行身份验证。
...
```bash
text2llm plugins install @text2llm/line
```

本地检出（从 git 仓库运行时）：

```bash
text2llm plugins install ./extensions/line
```
...
```bash
text2llm pairing list line
text2llm pairing approve line <CODE>
```
