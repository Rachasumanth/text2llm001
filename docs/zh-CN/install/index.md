...
read_when:
  - 安装 text2llm
  - 你想从 GitHub 安装
summary: 安装 text2llm（推荐安装器、全局安装或从源代码安装）
title: 安装
x-i18n:
...
pnpm approve-builds -g                # 批准 text2llm、node-llama-cpp、sharp 等
pnpm add -g text2llm@latest           # 重新运行以执行 postinstall 脚本
```
...
提示：如果你还没有全局安装，请通过 `pnpm text2llm ...` 运行仓库命令。
...
- 运行新手引导：`text2llm onboard --install-daemon`
- 快速检查：`text2llm doctor`
- 检查 Gateway 网关健康状态：`text2llm status` + `text2llm health`
- 打开仪表板：`text2llm dashboard`
...
```bash
# 显式 npm
curl -fsSL https://text2llm.ai/install.sh | bash -s -- --install-method npm
...
- `TEXT2LLM_GIT_DIR=...`
- `TEXT2LLM_GIT_UPDATE=0|1`
- `TEXT2LLM_NO_PROMPT=1`
- `TEXT2LLM_DRY_RUN=1`
- `TEXT2LLM_NO_ONBOARD=1`
- `SHARP_IGNORE_GLOBAL_LIBVIPS=0|1`（默认：`1`；避免 `sharp` 针对系统 libvips 构建）

## 故障排除：找不到 `text2llm`（PATH）
...
如果 `$(npm prefix -g)/bin`（macOS/Linux）或 `$(npm prefix -g)`（Windows）**不**在 `echo "$PATH"` 的输出中，你的 shell 无法找到全局 npm 二进制文件（包括 `text2llm`）。
