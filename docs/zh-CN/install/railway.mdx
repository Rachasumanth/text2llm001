...
通过一键模板在 Railway 上部署 text2llm，并在浏览器中完成设置。
...
- `https://<your-railway-domain>/text2llm` — 控制面板 UI
...
- `TEXT2LLM_STATE_DIR=/data/.text2llm`（推荐）
- `TEXT2LLM_WORKSPACE_DIR=/data/workspace`（推荐）
- `TEXT2LLM_GATEWAY_TOKEN`（推荐；请视为管理员密钥）
...
这会导出你的 text2llm 状态和工作区，方便你迁移到其他主机而不丢失配置或记忆。
