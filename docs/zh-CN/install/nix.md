...
使用 Nix 运行 text2llm 的推荐方式是通过 **[nix-text2llm](https://github.com/text2llm/nix-text2llm)** — 一个开箱即用的 Home Manager 模块。
...
> **📦 完整指南：[github.com/text2llm/nix-text2llm](https://github.com/text2llm/nix-text2llm)**
>
> nix-text2llm 仓库是 Nix 安装的权威来源。本页只是一个快速概述。
...
当设置 `TEXT2LLM_NIX_MODE=1` 时（nix-text2llm 会自动设置）：

text2llm 支持 **Nix 模式**，使配置确定性并禁用自动安装流程。
通过导出以下环境变量启用：
...
text2llm 从 `TEXT2LLM_CONFIG_PATH` 读取 JSON5 配置，并将可变数据存储在 `TEXT2LLM_STATE_DIR` 中。

- `TEXT2LLM_STATE_DIR`（默认：`~/.text2llm`）
- `TEXT2LLM_CONFIG_PATH`（默认：`$TEXT2LLM_STATE_DIR/text2llm.json`）
...
[`scripts/package-mac-app.sh`](https://github.com/text2llm/text2llm/blob/main/scripts/package-mac-app.sh) 将此模板复制到应用包中并修补动态字段（bundle ID、版本/构建号、Git SHA、Sparkle 密钥）。这使 plist 对于 SwiftPM 打包和 Nix 构建保持确定性（它们不依赖完整的 Xcode 工具链）。

## 相关内容

- [nix-text2llm](https://github.com/text2llm/nix-text2llm) — 完整设置指南
