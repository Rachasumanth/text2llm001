...
Matrix is an open, decentralized messaging protocol. text2llm connects as a Matrix **user**
...
```bash
text2llm plugins install @text2llm/matrix
```

Local checkout (when running from a git repo):

```bash
text2llm plugins install ./extensions/matrix
```
...
1. Install the Matrix plugin:
   - From npm: `text2llm plugins install @text2llm/matrix`
   - From a local checkout: `text2llm plugins install ./extensions/matrix`
...
3. Get an access token for the bot account:
   ...
   - Or set `channels.matrix.userId` + `channels.matrix.password`: text2llm calls the same
     login endpoint, stores the access token in `~/.text2llm/credentials/matrix/credentials.json`,
...
- On first connection, text2llm requests device verification from your other sessions.
- Verify the device in another Matrix client (Element, etc.) to enable key sharing.
- If the crypto module cannot be loaded, E2EE is disabled and encrypted rooms will not decrypt;
  text2llm logs a warning.
...
`~/.text2llm/matrix/accounts/<account>/<homeserver>__<user>/<token-hash>/crypto/`
...
- Approve via:
  - `text2llm pairing list matrix`
  - `text2llm pairing approve matrix <CODE>`
...
- On startup, text2llm resolves room/user names in allowlists to IDs and logs the mapping; unresolved entries are ignored for allowlist matching.
