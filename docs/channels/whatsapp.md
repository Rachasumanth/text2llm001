...
```bash
text2llm channels login --channel whatsapp --account work
```
...
<Note>
text2llm recommends running WhatsApp on a separate number when possible. (The channel metadata and onboarding flow are optimized for that setup, but personal-number setups are also supported.)
</Note>
...
<AccordionGroup>
  <Accordion title="Dedicated number (recommended)">
    ...
    - separate WhatsApp identity for text2llm
  ...
  <Accordion title="WhatsApp Web-only channel scope">
    The messaging platform channel is WhatsApp Web-based (`Baileys`) in current text2llm channel architecture.
...
- if `messages.responsePrefix` is unset, self-chat replies default to `[{identity.name}]` or `[text2llm]`
...
<AccordionGroup>
  ...
  <Accordion title="Credential paths and legacy compatibility">
    - current auth path: `~/.text2llm/credentials/whatsapp/<accountId>/creds.json`
    - backup file: `creds.json.bak`
    - legacy default auth in `~/.text2llm/credentials/` is still recognized/migrated for default-account flows
  </Accordion>

  <Accordion title="Logout behavior">
    `text2llm channels logout --channel whatsapp [--account <id>]` clears WhatsApp auth state for that account.
