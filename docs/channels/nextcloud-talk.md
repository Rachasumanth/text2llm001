...
```bash
text2llm plugins install @text2llm/nextcloud-talk
```

Local checkout (when running from a git repo):

```bash
text2llm plugins install ./extensions/nextcloud-talk
```
...
2. On your Nextcloud server, create a bot:
   ...
   ```bash
   ./occ talk:bot:install "text2llm" "<shared-secret>" "<webhook-url>" --feature reaction
   ```
...
- Approve via:
  - `text2llm pairing list nextcloud-talk`
  - `text2llm pairing approve nextcloud-talk <CODE>`
