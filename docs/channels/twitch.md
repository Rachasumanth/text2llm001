...
read_when:
  - Setting up Twitch chat integration for text2llm
title: "Twitch"
---

# Twitch (plugin)

Twitch chat support via IRC connection. text2llm connects as a Twitch user (bot account) to receive and send messages in channels.
...
```bash
text2llm plugins install @text2llm/twitch
```

Local checkout (when running from a git repo):

```bash
text2llm plugins install ./extensions/twitch
```
...
4. Configure the token:
   - Env: `TEXT2LLM_TWITCH_ACCESS_TOKEN=...` (default account only)
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "text2llm", // Bot's Twitch account
      accessToken: "oauth:abc123...", // OAuth Access Token (or use TEXT2LLM_TWITCH_ACCESS_TOKEN env var)
...
```bash
TEXT2LLM_TWITCH_ACCESS_TOKEN=oauth:abc123...
```
