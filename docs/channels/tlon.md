...
Tlon is a decentralized messenger built on Urbit. text2llm connects to your Urbit ship and can
...
```bash
text2llm plugins install @text2llm/tlon
```

Local checkout (when running from a git repo):

```bash
text2llm plugins install ./extensions/tlon
```
...
Use these with `text2llm message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, text2llm replies in-thread.
