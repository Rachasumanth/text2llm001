...
```
    ...
    - or read `chat.id` from `text2llm logs --follow`
...
- DM messages can carry `message_thread_id`; text2llm routes them with thread-aware session keys and preserves thread ID for replies.
...
<AccordionGroup>
  <Accordion title="Draft streaming in Telegram DMs">
    text2llm can stream partial replies with Telegram draft bubbles (`sendMessageDraft`).
  ...
  <Accordion title="Formatting and HTML fallback">
    ...
    - If Telegram rejects parsed HTML, text2llm retries as plain text.
...
```
    ...
    - `~/.text2llm/telegram/sticker-cache.json`
...
```
  ...
  <Accordion title="Reaction notifications">
    ...
    When enabled, text2llm enqueues system events like:
...
```bash
text2llm message send --channel telegram --target 123456789 --message "hi"
text2llm message send --channel telegram --target @name --message "hi"
```
...
<AccordionGroup>
  <Accordion title="Bot does not respond to non mention group messages">
    ...
    - `text2llm channels status` warns when config expects unmentioned group messages.
    - `text2llm channels status --probe` can check explicit numeric group IDs; wildcard `"*"` cannot be membership-probed.
  ...
  <Accordion title="Bot not seeing group messages at all">
    ...
    - review logs: `text2llm logs --follow` for skip reasons
