...
Feishu (Lark) is a team chat platform used by companies for messaging and collaboration. This plugin connects text2llm to a Feishu/Lark bot using the platform’s WebSocket event subscription so messages can be received without exposing a public webhook URL.
...
```bash
text2llm plugins install @text2llm/feishu
```

Local checkout (when running from a git repo):

```bash
text2llm plugins install ./extensions/feishu
```
...
If you just installed text2llm, run the wizard:
...
2. Configuring app credentials in text2llm
...
1. You already ran `text2llm channels add` for Feishu
2. The gateway is running (`text2llm gateway status`)
...
```bash
text2llm pairing approve feishu <CODE>
```
...
2. Run `text2llm logs --follow` and look for `chat_id`
...
2. Run `text2llm logs --follow` and look for `open_id`
...
| Command                    | Description                   |
| -------------------------- | ----------------------------- |
| `text2llm gateway status`  | Show gateway status           |
| `text2llm gateway install` | Install/start gateway service |
| `text2llm gateway stop`    | Stop gateway service          |
| `text2llm gateway restart` | Restart gateway service       |
| `text2llm logs --follow`   | Tail gateway logs             |
...
4. Check logs: `text2llm logs --follow`
...
5. Ensure the gateway is running: `text2llm gateway status`
6. Check logs: `text2llm logs --follow`
  ...
  agents: {
    list: [
      { id: "main" },
      {
        ...
        agentDir: "/home/user/.text2llm/agents/clawd-fan/agent",
      },
      {
        id: "clawd-xi",
        workspace: "/home/user/clawd-xi",
        agentDir: "/home/user/.text2llm/agents/clawd-xi/agent",
      },
    ],
  },
  bindings: [
