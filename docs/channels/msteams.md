...
```bash
text2llm plugins install @text2llm/msteams
```

Local checkout (when running from a git repo):

```bash
text2llm plugins install ./extensions/msteams
```

If you choose Teams during configure/onboarding and a git checkout is detected,
text2llm will offer the local install path automatically.
...
3. Configure text2llm with those credentials.
...
- Talk to text2llm via Teams DMs, group chats, or channels.
...
- On startup, text2llm resolves team/channel and user allowlist names to IDs (when Graph permissions allow)
...
5. Configure `msteams` in `~/.text2llm/text2llm.json` (or env vars) and start the gateway.
6. The gateway listens for Bot Framework webhook traffic on `/api/messages` by default.

## Azure Bot Setup (Prerequisites)

Before configuring text2llm, you need to create an Azure Bot resource.
...
2. Fill in the **Basics** tab:
   ...
   | Field              | Value                                                    |
   | ------------------ | -------------------------------------------------------- |
   | **Bot handle**     | Your bot name, e.g., `text2llm-msteams` (must be unique) |
...
1. **Install the Microsoft Teams plugin**
   - From npm: `text2llm plugins install @text2llm/msteams`
   - From a local checkout: `text2llm plugins install ./extensions/msteams`
...
```json
{
  ...
  "description": { "short": "text2llm in Teams", "full": "text2llm in Teams" },
...
text2llm handles this by returning quickly and sending replies proactively, but very slow responses may still cause issues.
...
By default, text2llm only downloads media from Microsoft/Teams hostnames. Override with `channels.msteams.mediaAllowHosts` (use `["*"]` to allow any host).
...
Uploaded files are stored in a `/Text2llmShared/` folder in the configured SharePoint site's default document library.

## Polls (Adaptive Cards)

text2llm sends Teams polls as Adaptive Cards (there is no native Teams poll API).

- CLI: `text2llm message poll --channel msteams --target conversation:<id> ...`
- Votes are recorded by the gateway in `~/.text2llm/msteams-polls.json`.
...
```bash
text2llm message send --channel msteams \
...
text2llm message send --channel msteams --target "user:40a1a0ed-..." --message "Hello"

# Send to a user by display name (triggers Graph API lookup)
text2llm message send --channel msteams --target "user:John Smith" --message "Hello"

# Send to a group chat or channel
text2llm message send --channel msteams --target "conversation:19:abc...@thread.tacv2" --message "Hello"

# Send an Adaptive Card to a conversation
text2llm message send --channel msteams --target "conversation:19:abc...@thread.tacv2" \
  --card '{"type":"AdaptiveCard","version":"1.5","body":[{"type":"TextBlock","text":"Hello"}]}'
```
