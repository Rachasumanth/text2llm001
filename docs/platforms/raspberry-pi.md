---
summary: "text2llm on Raspberry Pi (budget self-hosted setup)"
read_when:
  - Setting up text2llm on a Raspberry Pi
  - Running text2llm on ARM devices
  - Building a cheap always-on personal AI
title: "Raspberry Pi"
---

# text2llm on Raspberry Pi

## Goal

Run a persistent, always-on text2llm Gateway on a Raspberry Pi for **~$35-80** one-time cost (no monthly fees).
...
Most text2llm features work on ARM64, but some external binaries may need ARM builds:
...
sudo systemctl is-enabled text2llm

# Enable if not
sudo systemctl enable text2llm

# Start on boot
sudo systemctl start text2llm
```
...
```bash
# Check logs
journalctl -u text2llm --no-pager -n 100

# Common fix: rebuild
cd ~/text2llm  # if using hackable install
npm run build
sudo systemctl restart text2llm
```
