...
service via the external `text2llm` CLI (no embedded runtime). This gives you
...
- The app installs a per‑user LaunchAgent labeled `bot.molt.gateway`
  (or `bot.molt.<profile>` when using `--profile`/`TEXT2LLM_PROFILE`; legacy `com.text2llm.*` is supported).
