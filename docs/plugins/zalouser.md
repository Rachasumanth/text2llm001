...
read_when:
  - You want Zalo Personal (unofficial) support in text2llm
...
Zalo Personal support for text2llm via a plugin, using `zca-cli` to automate a normal Zalo user account.
...
```bash
text2llm plugins install @text2llm/zalouser
```
...
```bash
text2llm plugins install ./extensions/zalouser
