...
read_when:
  - You want to place an outbound voice call from text2llm
...
Voice calls for text2llm via a plugin. Supports outbound notifications and
...
- Use `text2llm voicecall ...` or the `voice_call` tool
...
```bash
text2llm plugins install ./extensions/voice-call
