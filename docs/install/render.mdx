...
Deploy text2llm on Render using Infrastructure as Code. The included `render.yaml` Blueprint defines your entire stack declaratively, service, disk, environment variables, so you can deploy with a single click and version your infrastructure alongside your code.
...
[Deploy to Render](https://render.com/deploy?repo=https://github.com/text2llm/text2llm)
...
repository configures everything needed to run text2llm:
...
services:
  - type: web
    ...
    envVars:
      ...
      - key: TEXT2LLM_STATE_DIR
        value: /data/.text2llm
      - key: TEXT2LLM_WORKSPACE_DIR
        value: /data/workspace
      - key: TEXT2LLM_GATEWAY_TOKEN
...
The web dashboard is available at `https://<your-service>.onrender.com/text2llm`.
...
If you use the original text2llm repository, Render will not auto-deploy your text2llm. To update it, run a manual Blueprint sync from the dashboard.
...
For text2llm, vertical scaling is usually sufficient. Horizontal scaling requires sticky sessions or external state management.
...
This downloads a portable backup you can restore on any text2llm host.
