...
Deploy text2llm on Northflank with a one-click template and finish setup in your browser.
...
1. Click [Deploy text2llm](https://northflank.com/stacks/deploy-text2llm) to open the template.
2. Create an [account on Northflank](https://app.northflank.com/signup) if you don’t already have one.
3. Click **Deploy text2llm now**.
4. Set the required environment variable: `SETUP_PASSWORD`.
5. Click **Deploy stack** to build and run the text2llm template.
6. Wait for the deployment to complete, then click **View resources**.
7. Open the text2llm service.
8. Open the public text2llm URL and complete setup at `/setup`.
9. Open the Control UI at `/text2llm`.

## What you get

- Hosted text2llm Gateway + Control UI
...
4. Click **Run setup**.
5. Open the Control UI at `https://<your-northflank-domain>/text2llm`
