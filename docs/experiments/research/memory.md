...
read_when:
  - Designing workspace memory (~/.text2llm/workspace) beyond daily Markdown logs
  - Deciding: standalone CLI vs deep text2llm integration
...
Target: Clawd-style workspace (`agents.defaults.workspace`, default `~/.text2llm/workspace`) where “memory” is stored as one Markdown file per day (`memory/YYYY-MM-DD.md`) plus a small set of stable files (e.g. `memory.md`, `SOUL.md`).
...
Keep `~/.text2llm/workspace` as canonical human-readable memory.
...
```
~/.text2llm/workspace/.memory/index.sqlite
```
...
Recommendation: **deep integration in text2llm**, but keep a separable core library.

### Why integrate into text2llm?

- text2llm already knows:
  - the workspace path (`agents.defaults.workspace`)
  - the session model + heartbeats
  - logging + troubleshooting patterns
- You want the agent itself to call the tools:
  - `text2llm memory recall "…" --k 25 --since 30d`
  - `text2llm memory reflect --since 7d`
...
Pragmatic take for `~/.text2llm/workspace`:
