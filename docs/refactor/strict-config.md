...
- Plugin manifests (`text2llm.plugin.json`) are required for all plugins.

## Plugin schema enforcement

- Each plugin provides a strict JSON Schema for its config (inline in the manifest).
- Plugin load flow:
  1. Resolve plugin manifest + schema (`text2llm.plugin.json`).
...
Everything else must hard-fail with: “Config invalid. Run `text2llm doctor --fix`.”
