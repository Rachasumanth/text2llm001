...
- Publish as `text2llm/plugin-sdk` (or export from core under `text2llm/plugin-sdk`).
- Semver with explicit stability guarantees.

### 2) Plugin Runtime (execution surface, injected)

Scope: everything that touches core runtime behavior.
Accessed via `Text2llmPluginApi.runtime` so plugins never import `src/**`.
...
export type PluginRuntime = {
  channel: {
    text: {
      ...
      resolveTextChunkLimit(cfg: Text2llmConfig, channel: string, accountId?: string): number;
      hasControlCommand(text: string, cfg: Text2llmConfig): boolean;
    };
    reply: {
    ...
    mentions: {
      buildMentionRegexes(cfg: Text2llmConfig, agentId?: string): RegExp[];
    ...
    debounce: {
      ...
      resolveInboundDebounceMs(cfg: Text2llmConfig, channel: string): number;
    };
    commands: {
  ...
  state: {
    resolveStateDir(cfg: Text2llmConfig): string;
  };
};
```
...
- Introduce `text2llm/plugin-sdk`.
- Add `api.runtime` to `Text2llmPluginApi` with the surface above.
...
- Plugins declare a required runtime range (e.g., `text2llmRuntime: ">=2026.2.0"`).
