...
- **Ask default:** `on-miss`.
- **Approvals store:** `~/.text2llm/exec-approvals.json` (JSON, no legacy migration).
- **Runner:** headless system service; UI app hosts a Unix socket for approvals.
- **Node identity:** use existing `nodeId`.
- **Socket auth:** Unix socket + token (cross-platform); split later if needed.
- **Node host state:** `~/.text2llm/node.json` (node id + pairing token).
...
Path: `~/.text2llm/exec-approvals.json`
...
```json
{
  "version": 1,
  "socket": {
    "path": "~/.text2llm/exec-approvals.sock",
...
- Unix socket at `~/.text2llm/exec-approvals.sock` (0600).
