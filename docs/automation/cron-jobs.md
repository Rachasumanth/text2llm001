...
- Jobs persist under `~/.text2llm/cron/` so restarts don’t lose schedules.
...
text2llm cron list
text2llm cron run <job-id>
text2llm cron runs --id <job-id>
```
...
Cron jobs are persisted on the Gateway host at `~/.text2llm/cron/jobs.json` by default.
The Gateway loads the file into memory and writes it back on changes, so manual edits
are only safe when the Gateway is stopped. Prefer `text2llm cron add/edit` or the cron
...
If `delivery` is omitted for isolated jobs, text2llm defaults to `announce`.
...
- Job store: `~/.text2llm/cron/jobs.json` (Gateway-managed JSON).
- Run history: `~/.text2llm/cron/runs/<jobId>.jsonl` (JSONL, auto-pruned).
...
text2llm cron add --name "Ops sweep" --cron "0 6 * * *" --session isolated --message "Check ops queue" --agent ops

# Switch or clear the agent on an existing job
text2llm cron edit <jobId> --agent ops
text2llm cron edit <jobId> --clear-agent
```

Manual run (force is the default, use `--due` to only run when due):

```bash
text2llm cron run <jobId>
text2llm cron run <jobId> --due
```

Edit an existing job (patch fields):

```bash
text2llm cron edit <jobId> \
...
```bash
text2llm system event --mode now --text "Next heartbeat: check battery."
```
...
- `cron.run` (force or due), `cron.runs`
  For immediate system events without a job, use [`text2llm system event`](/cli/system).

## Troubleshooting

### “Nothing runs”

- Check cron is enabled: `cron.enabled` and `TEXT2LLM_SKIP_CRON`.
- Check the Gateway is running continuously (cron runs inside the Gateway process).
- For `cron` schedules: confirm timezone (`--tz`) vs the host timezone.

### A recurring job keeps delaying after failures

- text2llm applies exponential retry backoff for recurring jobs after consecutive errors:
