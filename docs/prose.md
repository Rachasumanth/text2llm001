---
summary: "OpenProse: .prose workflows, slash commands, and state in text2llm"
read_when:
...
OpenProse is a portable, markdown-first workflow format for orchestrating AI sessions. In text2llm it ships as a plugin that installs an OpenProse skill pack plus a `/prose` slash command. Programs live in `.prose` files and can spawn multiple sub-agents with explicit control flow.
...
```bash
text2llm plugins enable open-prose
```

Restart the Gateway after enabling the plugin.

Dev/local checkout: `text2llm plugins install ./extensions/open-prose`

Related docs: [Plugins](/tools/plugin), [Plugin manifest](/plugins/manifest), [Skills](/tools/skills).

## Slash command

OpenProse registers `/prose` as a user-invocable skill command. It routes to the OpenProse VM instructions and uses text2llm tools under the hood.
...
## text2llm runtime mapping

OpenProse programs map to text2llm primitives:

| OpenProse concept         | text2llm tool    |
| ------------------------- | ---------------- |
...
Treat `.prose` files like code. Review before running. Use text2llm tool allowlists and approval gates to control side effects.
