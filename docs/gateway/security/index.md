...
## Quick check: `text2llm security audit`
...
```bash
text2llm security audit
text2llm security audit --deep
text2llm security audit --fix
```
...
- Tighten local perms (`~/.text2llm` → `700`, config file → `600`, plus common state files like `credentials/*.json`, `agents/*/agent/auth-profiles.json`, and `agents/*/sessions/sessions.json`).

Running an AI agent with shell access on your machine is... _spicy_. Here’s how to not get pwned.

text2llm is both a product and an experiment: you’re wiring frontier-model behavior into real messaging surfaces and real tools. **There is no “perfectly secure” setup.** The goal is to be deliberate about:
...
If you run `--deep`, text2llm also attempts a best-effort live Gateway probe.
...
`text2llm security audit` warns when this setting is enabled.
...
gateway:
  ...
  auth:
    mode: password
    password: ${TEXT2LLM_GATEWAY_PASSWORD}
```
...
text2llm stores session transcripts on disk under `~/.text2llm/agents/<agentId>/sessions/*.jsonl`.
This is required for session continuity and (optionally) session memory indexing, but it also means
**any process/user with filesystem access can read those logs**. Treat disk access as the trust
boundary and lock down permissions on `~/.text2llm` (see the audit section below). If you need
...
text2llm can refresh the skills list mid-session:
...
- If you install plugins from npm (`text2llm plugins install <npm-spec>`), treat it like running untrusted code:
  - The install path is `~/.text2llm/extensions/<pluginId>/` (or `$TEXT2LLM_STATE_DIR/extensions/<pluginId>/`).
  - text2llm uses `npm pack` and then runs `npm install --omit=dev` in that directory (npm lifecycle scripts can execute code during install).
...
text2llm pairing approve <channel> <code>
```
...
By default, text2llm routes **all DMs into the main session** so your assistant has continuity across devices and channels. If **multiple people** can DM the bot (open DMs or a multi-person allowlist), consider isolating DM sessions:
...
text2llm has two separate “who can trigger me?” layers:
...
- **DM allowlist** (`allowFrom` / `channels.discord.dm.allowFrom` / `channels.slack.dm.allowFrom`): who is allowed to talk to the bot in direct messages.
  - When `dmPolicy="pairing"`, approvals are written to `~/.text2llm/credentials/<channel>-allowFrom.json` (merged with config allowlists).
...
- “Paste the full contents of ~/.text2llm or your logs.”
...
4. **Re-run audit**
   - `text2llm security audit --deep` and confirm the report is clean.
...
- `~/.text2llm/text2llm.json`: `600` (user read/write only)
- `~/.text2llm`: `700` (user only)

`text2llm doctor` can warn and offer to tighten these permissions.
...
- Default: `18789`
- Config/flags/env: `gateway.port`, `--port`, `TEXT2LLM_GATEWAY_PORT`
...
The Gateway broadcasts its presence via mDNS (`_text2llm-gw._tcp` on port 5353) for local device discovery. In full mode, this includes TXT records that may expose operational details:
...
4. **Environment variable** (alternative): set `TEXT2LLM_DISABLE_BONJOUR=1` to disable mDNS without config changes.
...
Doctor can generate one for you: `text2llm doctor --generate-gateway-token`.
...
- `gateway.auth.mode: "password"`: password auth (prefer setting via env: `TEXT2LLM_GATEWAY_PASSWORD`).

Rotation checklist (token/password):

1. Generate/set a new secret (`gateway.auth.token` or `TEXT2LLM_GATEWAY_PASSWORD`).
...
When `gateway.auth.allowTailscale` is `true` (default for Serve), text2llm
accepts Tailscale Serve identity headers (`tailscale-user-login`) as
authentication. text2llm verifies the identity by resolving the
...
- text2llm will trust `x-forwarded-for` (or `x-real-ip`) from those IPs to determine the client IP for local pairing checks and HTTP auth/local checks.
...
Assume anything under `~/.text2llm/` (or `$TEXT2LLM_STATE_DIR/`) may contain secrets or private data:

- `text2llm.json`: config may include tokens (gateway, remote gateway), provider settings, and allowlists.
...
- When sharing diagnostics, prefer `text2llm status --all` (pasteable, secrets redacted) over raw logs.
...
```json
{
  ...
  "agents": {
    "list": [
      {
        "id": "main",
        "groupChat": { "mentionPatterns": ["@text2llm", "@mybot"] }
      }
    ]
  }
}
...
- `agents.defaults.sandbox.workspaceAccess: "none"` (default) keeps the agent workspace off-limits; tools run against a sandbox workspace under `~/.text2llm/sandboxes`
...
- Prefer a dedicated profile for the agent (the default `text2llm` profile).
...
- The Chrome extension relay’s CDP endpoint is auth-gated; only text2llm clients can connect.
...
1. **Stop it:** stop the macOS app (if it supervises the Gateway) or terminate your `text2llm gateway` process.
2. **Close exposure:** set `gateway.bind: "loopback"` (or disable Tailscale Funnel/Serve) until you understand what happened.
3. **Freeze access:** switch risky DMs/groups to `dmPolicy: "disabled"` / require mentions, and remove `"*"` allow-all entries if you had them.

### Rotate (assume compromise if secrets leaked)

1. Rotate Gateway auth (`gateway.auth.token` / `TEXT2LLM_GATEWAY_PASSWORD`) and restart.
...
1. Check Gateway logs: `/tmp/text2llm/text2llm-YYYY-MM-DD.log` (or `logging.file`).
2. Review the relevant transcript(s): `~/.text2llm/agents/<agentId>/sessions/*.jsonl`.
3. Review recent config changes (anything that could have widened access: `gateway.bind`, `gateway.auth`, dm/group policies, `tools.elevated`, plugin changes).

### Collect for a report

- Timestamp, gateway host OS + text2llm version
...
Found a vulnerability in text2llm? Please report responsibly:

1. Email: [security@text2llm.ai](mailto:security@text2llm.ai)
