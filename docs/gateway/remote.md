...
- Use the macOS app’s **Remote over SSH** mode (Settings → General → “text2llm runs”).
...
- `text2llm health` and `text2llm status --deep` now reach the remote gateway via `ws://127.0.0.1:18789`.
- `text2llm gateway {status,health,send,agent,call}` can also target the forwarded URL via `--url` when needed.

Note: replace `18789` with your configured `gateway.port` (or `--port`/`TEXT2LLM_GATEWAY_PORT`).
