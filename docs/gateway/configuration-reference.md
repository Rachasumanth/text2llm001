...
description: "Complete field-by-field reference for ~/.text2llm/text2llm.json"
---

# Configuration Reference

Every field available in `~/.text2llm/text2llm.json`. For a task-oriented overview, see [Configuration](/gateway/configuration).

Config format is **JSON5** (comments + trailing commas allowed). All fields are optional — text2llm uses safe defaults when omitted.
...
- Legacy single-account Baileys auth dir is migrated by `text2llm doctor` into `whatsapp/default`.
...
Mattermost ships as a plugin: `text2llm plugins install @text2llm/mattermost`.
...
text2llm spawns `imsg rpc` (JSON-RPC over stdio). No daemon or port required.
...
```json5
{
  ...
  agents: {
    list: [
      {
        id: "main",
        groupChat: { mentionPatterns: ["reisponde", "@text2llm"] },
      },
    ],
  },
}
...
- `config: true` enables `/config` (reads/writes `text2llm.json`).
...
Optional repository root shown in the system prompt's Runtime line. If unset, text2llm auto-detects by walking upward from the workspace.
...
- `model.primary`: format `provider/model` (e.g. `anthropic/claude-opus-4-6`). If you omit the provider, text2llm assumes `anthropic` (deprecated).
...
- `none`: per-scope sandbox workspace under `~/.text2llm/sandboxes`
    ...
    list: [
      {
        ...
        agentDir: "~/.text2llm/agents/main/agent",
        ...
        groupChat: { mentionPatterns: ["@text2llm"] },
        sandbox: { mode: "off" },
...
| `group:nodes`      | `nodes`                                                                                  |
| `group:text2llm`   | All built-in tools (excludes provider plugins)                                           |
...
text2llm uses the pi-coding-agent model catalog. Add custom providers via `models.providers` in config or `~/.text2llm/agents/<agentId>/agent/models.json`.
...
- Override agent config root with `TEXT2LLM_AGENT_DIR` (or `PI_CODING_AGENT_DIR`).
...
Set `OPENCODE_API_KEY` (or `OPENCODE_ZEN_API_KEY`). Shortcut: `text2llm onboard --auth-choice opencode-zen`.
...
Set `ZAI_API_KEY`. `z.ai/*` and `z-ai/*` are accepted aliases. Shortcut: `text2llm onboard --auth-choice zai-api-key`.
...
For the China endpoint: `baseUrl: "https://api.moonshot.cn/v1"` or `text2llm onboard --auth-choice moonshot-api-key-cn`.
...
Anthropic-compatible, built-in provider. Shortcut: `text2llm onboard --auth-choice kimi-code-api-key`.
...
Base URL should omit `/v1` (Anthropic client appends it). Shortcut: `text2llm onboard --auth-choice synthetic-api-key`.
...
Set `MINIMAX_API_KEY`. Shortcut: `text2llm onboard --auth-choice minimax-api`.
...
- Loaded from `~/.text2llm/extensions`, `<workspace>/.text2llm/extensions`, plus `plugins.load.paths`.
...
```json5
{
  gateway: {
    ...
    auth: {
      ...
      token: "your-token",
      // password: "your-password", // or TEXT2LLM_GATEWAY_PASSWORD
      allowTailscale: true,
    },
    tailscale: {
...
- `port`: single multiplexed port for WS + HTTP. Precedence: `--port` > `TEXT2LLM_GATEWAY_PORT` > `gateway.port` > `18789`.
...
Convenience flags: `--dev` (uses `~/.text2llm-dev` + port `19001`), `--profile <name>` (uses `~/.text2llm-<name>`).
...
Auth: `Authorization: Bearer <token>` or `x-text2llm-token: <token>`.
...
- Gateway auto-starts `gog gmail watch serve` on boot when configured. Set `TEXT2LLM_SKIP_GMAIL_WATCHER=1` to disable.
...
```json5
{
  canvasHost: {
    ...
    port: 18793,
    liveReload: true,
    // enabled: false, // or TEXT2LLM_SKIP_CANVAS_HOST=1
  },
}
```
...
- Also serves A2UI at `/__text2llm__/a2ui/`.
...
- Hostname defaults to `text2llm`. Override with `TEXT2LLM_MDNS_HOSTNAME`.
...
Writes a unicast DNS-SD zone under `~/.text2llm/dns/`. For cross-network discovery, pair with a DNS server (CoreDNS recommended) + Tailscale split DNS.

Setup: `text2llm dns setup --apply`.
...
- `.env` files: CWD `.env` + `~/.text2llm/.env` (neither overrides existing vars).
...
{
  gateway: {
    auth: { token: "${TEXT2LLM_GATEWAY_TOKEN}" },
  },
}
```
...
- Legacy OAuth imports from `~/.text2llm/credentials/oauth.json`.
...
- Default log file: `/tmp/text2llm/text2llm-YYYY-MM-DD.log`.
...
Current builds no longer include the TCP bridge. Nodes connect over the Gateway WebSocket. `bridge.*` keys are no longer part of the config schema (validation fails until removed; `text2llm doctor --fix` can strip unknown keys).
