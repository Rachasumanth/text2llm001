...
- During heartbeat runs, text2llm treats `HEARTBEAT_OK` as an ack when it appears
...
- `session`: optional session key for heartbeat runs.
  ...
  - Explicit session key (copy from `text2llm sessions --json` or the [sessions CLI](/cli/sessions)).
...
If **all three** are false, text2llm skips the heartbeat run entirely (no model call).
...
```bash
text2llm system event --text "Check for urgent follow-ups" --mode now
```
