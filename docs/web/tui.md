...
```bash
text2llm tui --url ws://<host>:<port> --token <gateway-token>
```
...
- Turn delivery on:
  ...
  - or start with `text2llm tui --deliver`
...
- Check the Gateway logs: `text2llm logs --follow`.
- Confirm the agent can run: `text2llm status` and `text2llm models status`.
...
- No agents in picker: check `text2llm agents list` and your routing config.
