...
- optional prefix: set `gateway.controlUi.basePath` (e.g. `/text2llm`)
...
If the page fails to load, start the Gateway first: `text2llm gateway`.
...
you revoke it with `text2llm devices revoke --device <id> --role <role>`. See
...
- Config: view/edit `~/.text2llm/text2llm.json` (`config.get`, `config.set`)
...
(`tailscale-user-login`) when `gateway.auth.allowTailscale` is `true`. text2llm
...
text2llm **blocks** Control UI connections without device identity.
