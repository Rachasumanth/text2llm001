---
summary: "Use Venice AI privacy-focused models in text2llm"
read_when:
  - You want privacy-focused inference in text2llm
...
```bash
text2llm onboard --auth-choice venice-api-key
```
...
```bash
text2llm chat --model venice/llama-3.3-70b "Hello, are you working?"
```

## Model Selection

After setup, text2llm shows all available Venice models. Pick based on your needs:
...
```bash
text2llm models set venice/claude-opus-45
text2llm models set venice/llama-3.3-70b
```

List all available models:

```bash
text2llm models list | grep venice
```

## Configure via `text2llm configure`

1. Run `text2llm configure`
2. Select **Model/auth**
3. Choose **Venice AI**
...
text2llm automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
text2llm chat --model venice/llama-3.3-70b

# Use Claude via Venice (anonymized)
text2llm chat --model venice/claude-opus-45

# Use uncensored model
text2llm chat --model venice/venice-uncensored

# Use vision model with image
text2llm chat --model venice/qwen3-vl-235b-a22b

# Use coding model
text2llm chat --model venice/qwen3-coder-480b-a35b-instruct
```
...
The Venice model catalog updates dynamically. Run `text2llm models list` to see currently available models. Some models may be temporarily offline.
