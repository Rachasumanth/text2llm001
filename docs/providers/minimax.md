---
summary: "Use MiniMax M2.1 in text2llm"
read_when:
  - You want MiniMax models in text2llm
...
```bash
text2llm plugins enable minimax-portal-auth  # skip if already loaded.
text2llm gateway restart  # restart if gateway is already running
text2llm onboard --auth-choice minimax-portal
```
...
See [MiniMax OAuth plugin README](https://github.com/text2llm/text2llm/tree/main/extensions/minimax-portal-auth) for details.
...
Configure manually via `text2llm.json`:
...
- Use `text2llm models list` and `text2llm models set minimax/MiniMax-M2.1` to switch.
...
- Running `text2llm configure` and selecting **MiniMax M2.1**, or
