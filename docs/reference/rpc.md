...
text2llm integrates external CLIs via JSON-RPC. Two patterns are used today.
...
- text2llm owns lifecycle when `channels.signal.autoStart=true`.
...
- text2llm spawns `imsg rpc` as a child process (legacy iMessage integration).
