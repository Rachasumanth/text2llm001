...
This document explains how text2llm manages sessions end-to-end:
...
text2llm is designed around a single **Gateway process** that owns session state.
...
text2llm persists sessions in two layers:
...
- Transcripts: `~/.text2llm/agents/<agentId>/sessions/<sessionId>.jsonl`
  - Telegram topic sessions: `.../<sessionId>-topic-<threadId>.jsonl`

text2llm resolves these via `src/config/sessions.ts`.
...
text2llm intentionally does **not** “fix up” transcripts; the Gateway uses `SessionManager` to read/write them.
...
These are Pi runtime semantics (text2llm consumes the events, but Pi decides when to compact).
...
text2llm also enforces a safety floor for embedded runs:

- If `compaction.reserveTokens < reserveTokensFloor`, text2llm bumps it.
- Default floor is `20000` tokens.
- Set `agents.defaults.compaction.reserveTokensFloor: 0` to disable the floor.
- If it’s already higher, text2llm leaves it alone.
...
- `text2llm status` (CLI)
- `text2llm sessions` / `sessions --json`
...
text2llm supports “silent” turns for background tasks where the user should not see intermediate output.

Convention:

- The assistant starts its output with `NO_REPLY` to indicate “do not deliver a reply to the user”.
- text2llm strips/suppresses this in the delivery layer.

As of `2026.1.10`, text2llm also suppresses **draft/typing streaming** when a partial chunk begins with `NO_REPLY`, so silent operations don’t leak partial output mid-turn.
...
text2llm uses the **pre-threshold flush** approach:
...
Pi also exposes a `session_before_compact` hook in the extension API, but text2llm’s
...
- Store vs transcript mismatch? Confirm the Gateway host and the store path from `text2llm status`.
