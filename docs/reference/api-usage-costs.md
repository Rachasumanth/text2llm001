...
text2llm features that can generate provider usage or paid API calls.
...
- `text2llm status --usage` and `text2llm channels list` show provider **usage windows**
...
text2llm can pick up credentials from:
...
- `text2llm status --usage`
- `text2llm models status --json`
...
`text2llm models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
