...
- `/usage cost` in chats: local cost summary aggregated from text2llm session logs.
- CLI: `text2llm status --usage` prints a full per-provider breakdown.
- CLI: `text2llm channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
