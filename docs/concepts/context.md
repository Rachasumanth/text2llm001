...
read_when:
  - You want to understand what “context” means in text2llm
...
“Context” is **everything text2llm sends to the model for a run**. It is bounded by the model’s **context window** (token limit).

Beginner mental model:

- **System prompt** (text2llm-built): rules, tools, skills list, time/runtime, and injected workspace files.
...
## How text2llm builds the system prompt

The system prompt is **text2llm-owned** and rebuilt each run. It includes:
...
By default, text2llm injects a fixed set of workspace files (if present):
