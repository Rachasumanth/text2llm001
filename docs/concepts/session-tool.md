...
- When given a `sessionId`, text2llm resolves it to the corresponding session key (missing ids error).
...
- After the primary run completes, text2llm runs a **reply-back loop**:
  - Round 2+ alternates between requester and target agents.
  - Reply exactly `REPLY_SKIP` to stop the ping‑pong.
  - Max turns is `session.agentToAgent.maxPingPongTurns` (0–5, default 5).
- Once the loop ends, text2llm runs the **agent‑to‑agent announce step** (target agent only):
...
- After completion, text2llm runs a sub-agent **announce step** and posts the result to the requester chat channel.
