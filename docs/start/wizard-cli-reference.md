...
read_when:
  - You need detailed behavior for text2llm onboard
...
This page is the full reference for `text2llm onboard`.
...
<Steps>
  <Step title="Existing config detection">
    - If `~/.text2llm/text2llm.json` exists, choose Keep, Modify, or Reset.
    - Re-running the wizard does not wipe anything unless you explicitly choose Reset (or pass `--reset`).
    - If config is invalid or contains legacy keys, the wizard stops and asks you to run `text2llm doctor` before continuing.
  ...
  <Step title="Channels">
    ...
    - DM security: default is pairing. First DM sends a code; approve via
      `text2llm pairing approve <channel> <code>` or use allowlists.
  </Step>
  ...
  <Step title="Health check">
    - Starts gateway (if needed) and runs `text2llm health`.
    - `text2llm status --deep` adds gateway health probes to status output.
  </Step>
  <Step title="Skills">
...
<AccordionGroup>
  ...
  <Accordion title="OpenAI API key">
    ...
    `~/.text2llm/.env` so launchd can read it.
...
- OAuth credentials: `~/.text2llm/credentials/oauth.json`
- Auth profiles (API keys + OAuth): `~/.text2llm/agents/<agentId>/agent/auth-profiles.json`

<Note>
Headless and server tip: complete OAuth on a machine with a browser, then copy
`~/.text2llm/credentials/oauth.json` (or `$TEXT2LLM_STATE_DIR/credentials/oauth.json`)
to the gateway host.
</Note>
...
- Command reference: [`text2llm onboard`](/cli/onboard)
