...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in text2llm
...
Running text2llm via Node with `tsx` fails at startup with:

```
[text2llm] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `text2llm status` and `gateway:watch` worked.
...
- Use Node + tsc watch, then run compiled output:
  ...
  node --watch text2llm.mjs status
  ```

- Confirmed locally: `pnpm exec tsc -p tsconfig.json` + `node text2llm.mjs status` works on Node 25.
