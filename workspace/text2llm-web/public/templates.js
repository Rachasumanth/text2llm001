...
let nbTplCurrentPage = 1;
const NB_TPL_ITEMS_PER_PAGE = 200;
...
function nbRenderTemplateCards() {
  ...
  const totalPages = Math.ceil(totalFiltered / NB_TPL_ITEMS_PER_PAGE);
  
  if (nbTplCurrentPage > totalPages && totalPages > 0) {
    nbTplCurrentPage = totalPages;
  }

  const startIndex = (nbTplCurrentPage - 1) * NB_TPL_ITEMS_PER_PAGE;
  const endIndex = startIndex + NB_TPL_ITEMS_PER_PAGE;
  const toRender = filtered.slice(startIndex, endIndex);
  ...
  let paginationContainer = document.getElementById("nb-tpl-pagination-container");
  if (!paginationContainer) {
    paginationContainer = document.createElement("div");
    paginationContainer.id = "nb-tpl-pagination-container";
    paginationContainer.className = "nb-tpl-pagination-container";
    grid.parentNode.insertBefore(paginationContainer, grid.nextSibling);
  }
  
  if (totalFiltered > NB_TPL_ITEMS_PER_PAGE) {
    paginationContainer.style.display = "flex";
    paginationContainer.innerHTML = `
      <button class="nb-tpl-page-btn" id="nb-tpl-prev-page" ${nbTplCurrentPage === 1 ? 'disabled' : ''}>&lt; Previous</button>
      <span class="nb-tpl-page-info">Page ${nbTplCurrentPage} of ${totalPages} (${totalFiltered} templates)</span>
      <button class="nb-tpl-page-btn" id="nb-tpl-next-page" ${nbTplCurrentPage === totalPages ? 'disabled' : ''}>Next &gt;</button>
    `;

    const prevBtn = document.getElementById("nb-tpl-prev-page");
    if (prevBtn) {
      prevBtn.addEventListener("click", () => {
        if (nbTplCurrentPage > 1) {
          nbTplCurrentPage--;
          nbRenderTemplateCards();
        }
      });
    ...
    const nextBtn = document.getElementById("nb-tpl-next-page");
    if (nextBtn) {
      nextBtn.addEventListener("click", () => {
        if (nbTplCurrentPage < totalPages) {
          nbTplCurrentPage++;
          nbRenderTemplateCards();
        }
      });
...
function askTemplateInsertMode() {
  return new Promise((resolve) => {
    const modal = document.getElementById("nb-tpl-confirm-modal");
    if (!modal) return resolve("continue"); // fallback

    const btnCancel = document.getElementById("nb-tpl-confirm-cancel");
    const btnReplace = document.getElementById("nb-tpl-confirm-replace");
    const btnContinue = document.getElementById("nb-tpl-confirm-continue");
...
async function nbInsertTemplate(tpl) {
  try {
    const mode = await askTemplateInsertMode();
    ...
    if (mode === "replace") {
      nbSetStatus("Clearing existing cells...");
      const existingCells = [...nbState.cells];
      for (const c of existingCells) {
        try {
          await fetch(`/api/notebook/cells/${c.id}?projectId=${getActiveProjectIdOrDefault()}`, { method: "DELETE" });
        } catch(e) {}
      }
      nbState.cells = [];
    }
...
// Initialize templates feature when the page is ready
