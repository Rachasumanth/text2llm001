...
const workspaceConfig = (() => {
  ...
  if (!configuredPath || !configuredPath.trim()) {
    return path.resolve(repoRoot, "workspace", "openclaw.json");
  }
...
function runOpenClawAgent(message) {
  return new Promise((resolve, reject) => {
    ...
    const env = {
      ...process.env,
      OPENCLAW_CONFIG_PATH: process.env.OPENCLAW_CONFIG_PATH || workspaceConfig,
...
function runOpenClawCommand(args, extraEnv = {}, options = {}) {
  return new Promise((resolve, reject) => {
    const env = {
      ...process.env,
      OPENCLAW_CONFIG_PATH: process.env.OPENCLAW_CONFIG_PATH || workspaceConfig,
...
app.get("/api/health", (_req, res) => {
  res.json({
    ok: true,
    configPath: process.env.OPENCLAW_CONFIG_PATH || workspaceConfig,
  });
});
...
function ensureNotebookCells(config) {
  ...
  if (!Array.isArray(config.notebook.cells)) {
    config.notebook.cells = [
      makeCell("markdown", "# Project Notebook\nCode cells generated by OpenClaw.", "default"),
      makeCell("code", "# Ready for GPU inference\nprint('Hello from OpenClaw')", "default"),
    ];
  }
...
app.post("/api/auth/test", async (req, res) => {
  try {
    ...
    const result = await runOpenClawCommand(args, { ...process.env });
...
app.post("/api/chat", async (req, res) => {
  ...
  child.stderr.on("data", (chunk) => {
    ...
    // Keep stderr only for debug fallback on hard failure.
    // Do not forward raw stderr into user-visible chat stream because
    // shell/tool errors can pollute natural-language replies.
  });
