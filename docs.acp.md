...
This document describes how the text2llm ACP (Agent Client Protocol) bridge works,
how it maps ACP sessions to Gateway sessions, and how IDEs should invoke it.

## Overview

`text2llm acp` exposes an ACP agent over stdio and forwards prompts to a running
text2llm Gateway over WebSocket. It keeps ACP session ids mapped to Gateway
...
drive a text2llm Gateway session.

Quick steps:

1. Run a Gateway (local or remote).
2. Configure the Gateway target (`gateway.remote.url` + auth) or pass flags.
3. Point the IDE to run `text2llm acp` over stdio.

Example config:

```bash
text2llm config set gateway.remote.url wss://gateway-host:18789
text2llm config set gateway.remote.token <token>
```

Example run:

```bash
text2llm acp --url wss://gateway-host:18789 --token <token>
```
...
```bash
text2llm acp --session agent:main:main
text2llm acp --session agent:design:main
text2llm acp --session agent:qa:bug-123
```
...
In Zed, open the Agent panel and select “text2llm ACP” to start a thread.

## Execution Model

- ACP client spawns `text2llm acp` and speaks ACP messages over stdio.
...
```bash
text2llm acp --session agent:main:main
text2llm acp --session-label "support inbox"
text2llm acp --reset-session
```
...
`text2llm acp` resolves the Gateway URL and auth from CLI flags or config:
